...
PROD_PORNMASTER_FLUX2_MULTI_EDIT_BF16_TASK_TYPE = (
...
class RunPodProdWorkerCanaryCaseBuilder:
    ...
    def dry_run_steps(self) -> list[str]:
        if self.config.profile == "i2i_pro":
            ...
            task_summary = (
                "submit prod Web pornmaster_flux2_edit_bf16 and "
                "pornmaster_flux2_multi_edit_bf16 tasks serially"
            )
        else:
            task_summary = (
    ...
    def pornmaster_flux2_edit_bf16_task_cases(
        ...
        return [
            ...
            {
                "label": "prod_pornmaster_flux2_multi_edit_bf16_canary",
                "expected_central_task_type": (
                    PROD_PORNMASTER_FLUX2_MULTI_EDIT_BF16_TASK_TYPE
                ),
                "payload": {
                    "task_type": PROD_PORNMASTER_FLUX2_MULTI_EDIT_BF16_TASK_TYPE,
                    "inputs": {
                        "images": [image_object_key, image_object_key],
                        "image": image_object_key,
                        "image2": image_object_key,
                    ...
                    "prompt": self.config.prompt
                    or "combine both references, high quality",
                    "negative_prompt": self.config.negative_prompt,
                    "priority": 0,
                },
                "result_kind": "image",
