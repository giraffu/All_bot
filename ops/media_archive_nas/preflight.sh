...
printf '%s\n' "${MINIO_IMAGE:-}" | grep -Eq '@sha256:[0-9a-f]{64}$' || { echo "MINIO_IMAGE must be digest-pinned" >&2; exit 2; }
printf '%s\n' "${MINIO_MC_IMAGE:-}" | grep -Eq '@sha256:[0-9a-f]{64}$' || { echo "MINIO_MC_IMAGE must be digest-pinned" >&2; exit 2; }
