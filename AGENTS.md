...
* **动态优先级衰减机制**：`最终优先级 = 修为优先级 + 身份优先级`。
  ...
  * **防霸占衰减**：无论境界多高，优先级都会随当日生成次数阶梯式衰减。
    * **付费身份保底**：真传弟子/核心弟子/内门弟子在衰减到最后阶段时，将保持 `+1` 的保底优先级，不再降为 0。
    * **修为境界归零**：金丹/筑基/练气等境界，超过单日最大生成次数后，优先级将降为 0。

## 4. 经济与审计模型
* **单轨制代币**：
  * `credits`（灵石）：充值、邀请（基础5 + 进群10 = 15灵石）或每日签到获得。永不过期。
  * *注：曾经的 `temp_credits`（临时灵石）系统已被**完全废弃**，现在所有途径获取的均为永久灵石。所有的代码中均不应再使用 `temp_credits`。*
* **强制数据流审计**：
