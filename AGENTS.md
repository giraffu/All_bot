...
5.  **Payment Agent (账房)**：负责处理与 TON 区块链的交互、充值验证与自动发货。
6.  **Data Steward Agent**：处理数据持久化和日志记录。
...
### 2.5 Payment Agent (账房)
-   **代码库**: `src/services/payment_validator.py`, `frontend/`
-   **角色**: TON 区块链支付与财务对账系统。
-   **架构**: 
    -   **前端 (Mini App)**: 基于 React 渲染“修仙”套餐（内门/核心/真传弟子），计算价格，并生成格式为 `ORDER:{tgUserId}:{planId}:{timestamp}` 的专属 BOC Payload，拉起钱包签名。
    -   **后端 (Bot 轮询任务)**: 通过 `TonPaymentValidator` 每 15 秒主动轮询 TON Center RPC 节点。
-   **职责**:
    -   监听指定收款地址 (`VITE_MERCHANT_ADDRESS`) 的链上交易。
    -   解析 BOC 格式提取订单 Payload，验证转账金额是否满足要求（防篡改）。
    -   一旦交易确认，自动发放“灵石”、更新用户身份 (`current_identity`) 并向用户发送 Telegram 充值成功通知。

### 2.6 Data Steward Agent
-   **代码库**: `src/database/*`
-   **角色**: 持久化管理者。
-   **职责**:
...
本项目配置了**双机器人（双环境）**机制，以确保生产环境的稳定，同时方便进行新功能（如支付系统）的测试。两个环境**共享同一个底层数据库**。

- **正式服 (PROD)**：使用 `BOT_TOKEN`，对应配置文件 `deploy/docker-compose.yml`。
- **测试服 (TEST)**：使用 `BOT_TOKEN_TEST`，对应配置文件 `deploy/docker-compose-test.yml`。

#### 本地与容器部署规范
1.  **前提条件**: Python 3.10+, PostgreSQL, MinIO, AI Backend。
2.  **环境变量**: 确保 `.env` 文件中同时配置了 `BOT_TOKEN` 和 `BOT_TOKEN_TEST`（或 `BOT_TOKEN_test`）。
3.  **日常开发与测试工作流（核心规范）**:
    - **任何代码更改后，必须首先重启测试 Bot (`tg-bot-test`) 进行验证。**
    - 测试无误后，再重启正式 Bot。
    ```bash
    cd deploy
    # 1. 重新构建并启动测试服
