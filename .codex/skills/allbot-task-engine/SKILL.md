...
- SCAIL-2 用户侧任务类型包括 `scail2_action_transfer`、`scail2_video_replacement`、`scail2_face_swap_v2`；内部仍保留 `scail2_action_transfer_long` 执行类型承接动作迁移 10/15/20s。视频换脸是版本化两阶段 continuation：先抽驱动首帧并以优先级 100 提交标准 `face_swap_v2`，意图持久化且切换 registry 后，才以确定性 backend ID、根任务正常身份优先级提交 `reference_preprocessed=true` 的 SCAIL-2 阶段；第二阶段不可取消、不得重复扣费或暴露中间图。公开动作迁移允许 5/8/10/15/20s，业务/History 仍记 `scail2_action_transfer`；dispatcher 按时长选择短 workflow 或隐藏 Context Windows workflow。
