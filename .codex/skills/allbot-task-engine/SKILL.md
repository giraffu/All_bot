...
- 正式 LAN AIO Worker 与统一 create request 后续新建的 RunPod Worker 使用按 profile 版本化的有界流水线；存量 RunPod 不原地修改。`img2img/img2img_lora`、`i2i_pro`、`pornmaster_flux2_edit_bf16` 使用 `image_claim3_comfy2_delivery1_v1`，有效上限为 claimed/Comfy/delivery=`3/2/1`；`image_to_video`、`ltx_video`、`scail2`、`wan22_video_v2` 使用 `media_claim2_comfy1_delivery1_v1`，有效上限为 `2/1/1`，确保视频类只有一个 GPU/Comfy 执行槽，但前一单交付时下一单可开始计算。`PREFETCH_TASK_TYPES` 必须跟随 `SUPPORTED_TASK_TYPES`；预接候选取 supported、prefetch 与 pipeline 类型交集并由 Central 在整个交集中按队列 score 选择，禁止按当前 task type 黏性续接。预接等待期间用 `set_current=false` 续 heartbeat，不能覆盖当前任务或扩成无界本地队列。占单计数覆盖 execution、delivery 与 reserved task；`gpu_done` 只释放 Comfy 槽，结果必须在上传成功且 Central `/complete` 确认后才终态。render/create request 始终显式保留数字回滚默认 `1/2/1`；旧镜像不认识版本策略时继续串行，精确旧 digest 回滚不会意外继承新深度。旧 `bf16_lan_claim3_comfy2_delivery1` 仅作已发布 BF16 镜像兼容别名，不得用于新 profile。
...
- SCAIL-2 用户侧任务类型包括 `scail2_action_transfer`、`scail2_video_replacement`、`scail2_face_swap_v2`；内部仍保留 `scail2_action_transfer_long` 执行类型承接动作迁移 10/15/20s。视频换脸是版本化两阶段 continuation：先抽取驱动首帧并以优先级 100 提交标准 `face_swap_v2`，意图持久化且切换 registry 后，才以确定性 backend ID、根任务正常身份优先级提交 `reference_preprocessed=true` 的 SCAIL-2 阶段；第二阶段不可取消、不得重复扣费或暴露中间图。公开动作迁移允许 5/8/10/15/20s，业务/History 仍记 `scail2_action_transfer`；dispatcher 按时长选择短 workflow 或隐藏 Context Windows workflow。时长、成本、驱动视频大小、默认 prompt、History type 长度和 LAN/RunPod 承接差异都必须按文档核对。
