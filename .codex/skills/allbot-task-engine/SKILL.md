...
- 若用户报告任务失败、卡住、结果不可见或性能退化，叠加 `allbot-diagnosing-bugs` 先建立反馈环；若新增任务能力或修复回归，叠加 `allbot-tdd` 一次一个行为切片。
...
- **节点层**：`workers/comfy_agent/agent_main.py` 是启动、shutdown、loop orchestration 与依赖组装 shell；健康/隔离与控制面恢复在 `agent_health.py`，Central 上报与 retry 在 `agent_reporting_client.py`，输入预取生命周期在 `agent_prefetch_manager.py`，双槽 pop/prepare/submit 与后台 finalizer 调度在 `agent_pipeline_coordinator.py`，等待完成、quality retry、结果物化、sidecar/R2 上传、complete/fail/cancel 回报和 timeout interrupt 在 `agent_finalizer.py`。旧 `process_task(...)` 保留串行兼容路径，旧 `_launch_pipeline_task(...)`、`_prepare_and_submit_task(...)` 与 `_finalize_execution(...)` 保留为委托方法。`workers/local_relay/relay_main.py` 是本地 worker 网关与上传 sidecar：非终态 status 可合并转发，`pop/check/complete/failed/cancelled` 必须同步转发，`/health` 是轻量存活检查，`/ready` 会探测 Central 与上传 client。新增输出类型、失败补偿、取消检查、重试策略、预取、pipeline、健康检查或上报语义时，优先下沉到对应阶段模块，并补 Worker focused tests。
- **Worker 健康态**：Comfy Agent heartbeat 状态包含 `idle`、`running`、`error`、`quarantined`；`active_workers` 只表示有心跳，`healthy_workers` 表示 heartbeat 健康态为 `idle/running`，`accepting_workers` 才表示同时处于健康态且 agent control 为 `enabled`、可接新单。Comfy 探活持续失败进入 `error`，连续基础设施类任务失败进入 `quarantined`。Agent 到 relay/Central 的控制面请求连续失败默认达到 12 次且持续 300 秒时会以退出码 75 退出，让 Docker restart 接管；这不替代 task heartbeat zombie 清理。Dashboard 必须同时展示健康字段和 `control_state`，避免把 `disabled/draining` worker 当作可接单容量。
- **Worker drain 控制**：新版 worker 会在 `/api/agent/task/pop` 携带 `agent_id`，Central 可通过 agent control 键将单个 worker 置为 `draining/disabled`，使其不再接新单但不影响其它 worker；旧 worker 不传 `agent_id` 时保持兼容。heartbeat 可选携带 GPU pool 元数据用于资源池观测，但这些字段只描述 Worker Agent 层，不表示底层 ComfyUI runtime 已被容器化或 Controller 接管；`cloud_prod_worker_01` 的 agent 已支持新协议，但目标 `gpu-226:8188` 仍是宿主机 ComfyUI。
- **Central Redis 瞬时断线韧性**：Redis 写连接偶发 reset 曾导致 `/status/{task_id}` 或 worker heartbeat/status 短暂 500；当前 QueueManager 已对安全读写和幂等更新做有限 transient retry，覆盖 `/status`、agent `/task/check`、`task_heartbeat`、运行态 `status`、worker heartbeat 和 agent control。真实出队 `zpopmin` 与会改变候选任务的队列移除不做盲 retry。排障时先看 worker 重试、task heartbeat 和队列事实是否恢复，不要把一次 Redis 连接重置直接解读成队列丢失。
...
- `src/domain_config/task_type_registry.py` 是任务类型只读事实表、查询 helper 和一致性门禁，记录 public type、legacy alias、execution type、Central type、workflow filename、RunPod profile、视频/Gallery/apply 与成本。当前已驱动 Gallery 可投稿类型、Gallery 展示配置与 apply 输入复用白名单；dispatcher、Central map、workflow mapping 和 worker `SUPPORTED_TASK_TYPES` 仍由既有事实源驱动。新增任务类型必须同步 registry 并跑 `tests/config/test_task_type_registry.py`。
...
- LTX 高级图生视频用户侧仍提交并展示为 `ltx_video`；当前 Bot/Web 入口只开放单首帧和首尾帧，dispatcher 分别进入执行面 `ltx_video`、`ltx_video_flf2v`。底层 `ltx_video_v2v_audio` 仍作为历史/队列兼容执行面保留，但不再由用户入口提交。LTX worker 建议声明三个执行类型，结果 `extra_outputs.last_frame` 可被 Web/Bot 扩展生成复用；Web/Bot 续段会携带 `ltx_prev_task_id` / `ltx_chain_task_ids` 并由 Web finalizer 或 Bot completion 持久化到 `extra_outputs._ltx_context`，续段结果可通过 `/users/history/{task_id}/ltx-chain/stitch` 或 Bot 结果按钮 `ltx_stitch_chain:<task_id>` 生成 `extra_outputs.ltx_chain_stitch` 拼接记录；LoRA/附加模型仍沿用现有 LTX `lora_items` 规则，不新增模型选择体系。
- SCAIL-2 已有 `scail2_action_transfer`、`scail2_video_replacement`、`scail2_face_swap_v2`。Web payload 使用 `inputs.images=[reference_image_key, motion_video_key]`、可选 `prompt`、`negative_prompt`、`duration`；Bot “视频生视频”二级菜单下“视频换人 / 动作迁移 / 视频换脸”均收集参考图、驱动视频和可选正向提示词，Bot 可跳过、Web 可留空，空 prompt 由 `normalize_scail2_positive_prompt(...)` 按 task type 补齐默认值。负面词固定使用默认值，驱动视频限制 40MB。`Scail2VideoStrategy` 严格只允许 5s/8s，计费 40/80 灵石，提交 Central simple route 时转换为 `image`、`video`、`prompt`、`negative_prompt`、`length`；`scail2_video_replacement` 与 `scail2_face_swap_v2` 都按 replacement mode 执行。Web/Bot 成功结果可投稿；Web 一键应用 SCAIL-2 投稿时只复用原 motion/driving video，复用者重新上传 reference image，衍生任务保持 `allow_contribute=false`。云测试 worker8 与云正式 LAN slot0 都可接三类 audio/v10 workflow，视频换脸覆盖到 `SCAIL-2_FaceSwap_v10_firstframe_faceswap_replacement_audio.api.json`，并通过 `SCAIL2_FACE_SWAP_V10_*` 先调用 `face_swap_v2.json` 对驱动视频第一帧做图片换脸，再提交 SCAIL-2 replacement workflow；正式 RunPod `scail2` profile 仍只声明 `scail2_action_transfer,scail2_video_replacement`，不承接 `scail2_face_swap_v2`。
...
- 看 worker heartbeat、`healthy_workers`、`accepting_workers`、节点 `error/quarantined` 状态、`control_state` 与 `SUPPORTED_TASK_TYPES`
