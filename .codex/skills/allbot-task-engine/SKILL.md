...
- LTX 高级图生视频用户侧仍提交并展示为 `ltx_video`；dispatcher 按 `inputs.ltx_mode`、`use_end_frame` 或 `video` 分流到执行面 `ltx_video`、`ltx_video_flf2v`、`ltx_video_v2v_audio`。LTX worker 必须声明三个执行类型，结果 `extra_outputs.last_frame` 可被 Web/Bot 扩展生成复用；LoRA/附加模型仍沿用现有 LTX `lora_items` 规则，不新增模型选择体系。
