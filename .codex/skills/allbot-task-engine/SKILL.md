...
- SCAIL-2 已有 `scail2_action_transfer`、`scail2_video_replacement`，云测试还接入了 `scail2_face_swap_v2`。Web payload 使用 `inputs.images=[reference_image_key, motion_video_key]`、可选 `prompt`、`negative_prompt`、`duration`；Bot “视频生视频”二级菜单下“视频换人 / 动作迁移 / 视频换脸”均收集参考图、驱动视频和可选正向提示词，Bot 可跳过、Web 可留空，空 prompt 由 `normalize_scail2_positive_prompt(...)` 按 task type 补齐默认值。负面词固定使用默认值，驱动视频限制 40MB。`Scail2VideoStrategy` 严格只允许 5s/8s，计费 40/80 灵石，提交 Central simple route 时转换为 `image`、`video`、`prompt`、`negative_prompt`、`length`；`scail2_video_replacement` 与 `scail2_face_swap_v2` 都按 replacement mode 执行。Web/Bot 成功结果可投稿；Web 一键应用 SCAIL-2 投稿时只复用原 motion/driving video，复用者重新上传 reference image，衍生任务保持 `allow_contribute=false`。云测试 worker8 可接三类 audio workflow，当前视频换脸覆盖到 `SCAIL-2_FaceSwap_v10_firstframe_faceswap_replacement_audio.api.json`，且 worker8 通过 `CLOUD_TEST_WORKER_08_FACE_SWAP_V10_*` 先调用 `face_swap_v2.json` 对驱动视频第一帧做图片换脸，再提交 SCAIL-2 replacement workflow；云正式 SCAIL-2 LAN/RunPod 当前仍只声明 `scail2_action_transfer,scail2_video_replacement`，正式扩展 FaceSwap v2 需另行发布确认。
