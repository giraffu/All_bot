...
- SCAIL-2 已新增 `scail2_action_transfer` 与 `scail2_video_replacement` 两个真实业务 task type。Web payload 使用 `inputs.images=[reference_image_key, motion_video_key]`、`prompt`、`negative_prompt`、`duration`；Bot 入口在“视频生视频”二级菜单下，按“视频换人 / 动作迁移”收集参考图、驱动视频和正向提示词，负面词固定使用 SCAIL-2 默认值，驱动视频限制 40MB。`Scail2VideoStrategy` 严格只允许 5s/8s，缺省 5s，计费 40/80 灵石，提交 Central simple route 时转换为 `image`、`video`、`prompt`、`negative_prompt`、`length`。非法时长必须返回 400 或在 Bot FSM 内拒绝，不允许静默拉长成长视频。Web/Bot 成功结果可投稿；Web 一键应用 SCAIL-2 投稿时只复用原 motion/driving video，复用者重新上传 reference image，提交仍为原 task type 且 `is_template=true/source_post_id=...`，模板衍生结果必须保持 `allow_contribute=false`。云测试 runtime 可以是 gpu-002 LAN AIO SCAIL-2 容器 `http://192.168.1.2:8190` + `cloud_worker_test_08`，也可以是 cloud-test RunPod `scail2` profile；云正式主 runtime 是 gpu-002 slot0 的 `lan_aio_prod_gpu002_gpu0_scail2_01`，手动正式 RunPod `runpod_prod_scail2_manual_NN` 只是可选备用/临时扩容 worker。正式排队排障时必须先看 `/system/workers` 是否真的存在 healthy 且 enabled 的 `scail2` worker；没有 heartbeat 或已删除的 RunPod slot 不算容量。正式 SCAIL-2 worker 都必须写 `user-data-prod` 且不得复用测试 worker/桶。正式发布或强制 RunPod canary 前用 `scripts/cloud_prod_generation_release_gate.py` 冻结 Web/Bot 新生成、等待 SCAIL-2 pending 清空或总 pending 低于阈值，再只取消退款仍在 pending 的任务；running 任务不强杀。
