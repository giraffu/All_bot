...
- SCAIL-2 云测试新增 `scail2_action_transfer` 与 `scail2_video_replacement` 两个真实业务 task type，当前接 Web 测试站与测试 Bot，不进云正式。Web payload 使用 `inputs.images=[reference_image_key, motion_video_key]`、`prompt`、`negative_prompt`、`duration`；测试 Bot 入口在“视频生视频”二级菜单下，按“视频换人 / 动作迁移”收集参考图、驱动视频和正向提示词，负面词固定使用 SCAIL-2 默认值，驱动视频限制 40MB。`Scail2VideoStrategy` 严格只允许 5s/8s，缺省 5s，计费 40/80 灵石，提交 Central simple route 时转换为 `image`、`video`、`prompt`、`negative_prompt`、`length`。非法时长必须返回 400 或在 Bot FSM 内拒绝，不允许静默拉长成长视频。执行 runtime 可以是 gpu-002 LAN AIO SCAIL-2 容器 `http://192.168.1.2:8190` + `cloud_worker_test_08`，也可以是 cloud-test RunPod `scail2` profile；RunPod canary 会临时 disable 同环境支持 SCAIL-2 的非 RunPod worker，确保两个 5s Web 任务由 `runpod_test_scail2_*` 接单，结束后恢复 control。SCAIL-2 runtime 不进入云正式。
