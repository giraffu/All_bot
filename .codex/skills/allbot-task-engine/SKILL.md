...
- **Central Redis 短暂 500 风险**：2026-06-10 巡检发现 Redis 写连接偶发 reset 可让 `/status/{task_id}` 或 worker heartbeat/status 短暂 500。排障时先看 worker 重试、task heartbeat 和队列事实是否恢复；后续修复应在 Central Redis 关键读写路径加有限 retry/reconnect。
