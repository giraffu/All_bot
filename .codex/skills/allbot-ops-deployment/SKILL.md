...
- `scripts/classify_ci_change.py` 对纯 docs/Skills/tests/仓库治理与 CI 门禁元数据提供 fail-closed 轻量分类；精确白名单包含 `scripts/release.py` 与 `deploy/test-acceptance.example.json`。全体路径均命中窄白名单时，单独 PR 可直接进入受保护 main 或兼容分支，跳过全量模块 CI、release bundle、测试部署和验收；任一运行时代码、migration、Compose、env/配置、白名单外发布执行器或未知路径都恢复完整流程。轻量分类不授权 direct push main 或任何环境 mutation。
