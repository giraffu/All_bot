...
## 1. 模块功能描述
- **Wan22 RunPod Web 验收状态**：2026-06-13 已用 4090-only Pod `tyejnfyigvwc68` 完成 cloud-test `wan22_video_v2` Web 端真实闭环；任务 `0f170dec-a48f-4f3c-9393-e1dcd053937f` 由 `runpod_test_wan22_video_v2_tyejnfyigvwc68` 接取，Central `done`、Web result `success`，结果在项目根目录 `wan22_video_v2.mp4` 与 `wan22_video_v2_last_frame.png`。验收后该 Pod 已手动删除，RunPod managed count/reconcile 均为 0。
...
- **GPU Pool Controller v1 / RunPod Provider v0**：本地 GPU 资源池第一阶段入口为 `ops/gpu_pool_controller/`、`scripts/gpu_pool_controller.py`、`deploy/docker-compose-local-registry.yml` 与 `scripts/manage_local_registry.sh`。默认 dry-run/canary，不自动重启生产 worker、ComfyUI 或 GPU 节点；`runtime-plan` 只输出 runtime/image/model/worker-env diff，`runtime-render` 只渲染标准 Comfy runtime compose，二者已支持 `--host-port`/`--container-name`/`--api-url`/`--ws-url` 备用端口 canary 覆盖；`runtime-apply`/`switch-profile`/`rollback-profile --execute` 当前会拒绝真实执行。`gpu-226` 是 `host_service`，Controller 不得为它生成 Docker 操作；`gpu-002` 是 Phase 1 试点。RunPod v0 位于 `ops/gpu_pool_controller/providers/runpod.py`，CLI 为 `python scripts/gpu_pool_controller.py runpod ...`，当前云测试主路径支持 `img2img,img2img_lora` canary 与拆分后的视频 profile `image_to_video`、`wan22_video_v2`；`wan22_aio_video` 仅保留为兼容/回滚 profile，不再作为新视频测试或扩容主口径。手动正式 RunPod 代码入口已支持 `--profile img2img`、`--profile image_to_video` 与 `--profile wan22_video_v2`：分别使用 `runpod_prod_img2img_manual_01..NN`、`runpod_prod_image_to_video_manual_01..NN`、`runpod_prod_wan22_video_v2_manual_01..NN`；真实生产创建/启用仍必须用户明确确认并通过门禁。
- **RunPod split video profile 口径**：2026-06-13 起 cloud-test RunPod 视频主路径已拆成 `image_to_video` 与 `wan22_video_v2` 两个 profile，分别渲染 `SUPPORTED_TASK_TYPES`、`POOL_RUNTIME_PROFILE`、`AGENT_ID_PREFIX`、profile-specific manifest 与 profile-specific env；二者暂时复用 Wan22 template `77gi0wqo8x` 和同一个 GHCR image。视频 profile 默认 GPU 调度口径为 `NVIDIA GeForce RTX 5090,NVIDIA GeForce RTX 4090`，5090 优先、4090 回退；若用户要求只用 4090，显式设置 `RUNPOD_GPU_TYPE_IDS_WAN22_VIDEO_V2='NVIDIA GeForce RTX 4090'`。新增 CLI：`runpod workers render-scale|scale --profile image_to_video|wan22_video_v2`、`runpod split-video-manifests`、`runpod split-video-canary`。真实 split canary 只允许 cloud-test；默认双 profile 需临时设置 `RUNPOD_DRY_RUN=false`、`RUNPOD_AUTOSCALER_ENABLED=true`、`RUNPOD_MAX_PODS_TOTAL=2`、`RUNPOD_MAX_PODS_PER_TYPE=1` 和 `--execute`，完成或失败后都要恢复 worker control、删除两个 Pod 并核验 managed count 为 0；分阶段验证时可传 `--profile wan22_video_v2` 或 `--profile image_to_video`，此时只创建 1 个 Pod，必须把 `RUNPOD_MAX_PODS_TOTAL` 收窄为 `1`，并只禁用支持该 profile 任务类型的非 RunPod worker。现场排障时可显式传 `--no-cleanup` 保留 Pod；已创建 Pod 可用 `--reuse-pod-id wan22_video_v2=<pod_id>` 接回 canary，继续等待 worker、提交 Web 任务和下载结果。若第二个 Pod 创建失败，必须立即删除第一个已创建 Pod，避免计费残留。
- **RunPod 通用门禁与清理**：真实 transfer/build/canary 仍只允许 cloud-test 且完成即删 Pod；真实 create/start/stop/delete/scale 必须同时显式设置 `RUNPOD_DRY_RUN=false`、`RUNPOD_AUTOSCALER_ENABLED=true`、对应 Pod 上限和 `--execute`。`runpod canary --env-file .env.cloud.test` 仍是图生图默认 dry-run 一键预检；视频验证应使用 `split-video-canary`。正式手动入口为 `python scripts/gpu_pool_controller.py runpod prod-worker render|status|up|enable|disable|down|canary|scale [--profile img2img|image_to_video|wan22_video_v2]`；`up --execute` 必须先写目标 agent 的 Central control `disabled` 再创建 Pod，`scale --execute` 扩容按 `disabled -> create -> readiness -> heartbeat -> enabled`，缩容从最高 slot 往下 `disabled -> drain -> delete`，忙碌 worker 有 `current_task_id` 时必须失败而非强杀；`down --execute` 必须确认目标 worker 无 `current_task_id` 才删除，`canary --execute` 不禁用现有正式 worker，完成后恢复目标 RunPod worker 为 `disabled`；`image_to_video` canary 会提交正式 Web `image_to_video` 任务并使用 `wan22_model_profile=legacy_image_to_video`，`wan22_video_v2` canary 会提交正式 Web `wan22_video_v2` 任务并使用 `wan22_model_profile=wan22_video_v2`，二者都会下载 MP4 与 `last_frame` PNG。正式 RunPod profile 会强制渲染 `RUNPOD_KEEPALIVE_ON_BOOTSTRAP_FAILURE=false`，避免生产 bootstrap 失败后意外长时间保活计费。RunPod R2 manifest 同步入口 `remote_workers/scripts/runpod_sync_models_from_r2.py` 已支持 `.partial` 断点续传、有限重试和后续新 Pod 的进度日志；已经创建的 Pod 不会热更新 `dockerStartCmd`，需删除重建才会拿到新 bootstrap。RunPod SSH 只用于云测试/失败现场短时诊断，需人工从 RunPod UI 提供当次 proxy SSH 信息；生产路径不依赖 SSH，也不得要求生产 Pod 暴露永久 SSH。详细口径见 `docs/子模块_GPU算力资源池控制器_gpu_pool_controller.md`。
- **RunPod / R2 变量分层**：`MINIO_*` / `R2_*` 始终表示用户数据桶，云测试为 `user-data-test` + `https://r2-test.aivison.it.com`，云正式为 `user-data-prod` + `https://r2.aivison.it.com`；`RUNPOD_MODEL_*` 始终表示模型缓存桶 `allbot-model-cache`，`img2img_lora` 默认 `img2img_lora/2026-06-10`，当前视频主路径必须使用 split profile 前缀 `image_to_video/2026-06-13-test` 与 `wan22_video_v2/2026-06-13-test`；`wan22_aio_video/2026-06-12-test` 只作为兼容/回滚全集 manifest，不再作为新视频主口径。`RUNPOD_MODEL_ACCESS_KEY` / `RUNPOD_MODEL_SECRET_KEY` 是模型桶真实 S3 凭据名，`RUNPOD_MODEL_ACCESS_KEY_REF` / `RUNPOD_MODEL_SECRET_KEY_REF` 只是 RunPod Secret reference 字符串；RunPod 用户数据桶 secret 分测试 `allbot_cloud_test_*` 和正式 `allbot_cloud_prod_*` 两套。Cloudflare `cfat_...` API token 不用于 S3 客户端，不写入 `.env.cloud.*`、日志或知识库；完整变量字典见 `docs/子模块_GPU算力资源池控制器_gpu_pool_controller.md`。
...
- **GitHub/GHCR token 口径**：Wan22 RunPod 共享镜像优先通过 GitHub Actions `.github/workflows/runpod_wan22_profile_image.yml` 手动构建/推送，workflow 使用仓库 `GITHUB_TOKEN` 登录 GHCR 并做匿名 manifest 检查；本机 `scripts/build_runpod_profile_image.sh --profile wan22_aio_video` 只作为共享镜像目录名对应的调试/兜底入口，不代表运行时继续使用 AIO profile。GitHub token 只用于 `docker login ghcr.io`、GHCR push 或 GitHub package 管理，不是 `RUNPOD_API_KEY`、R2 S3 key、agent token，也不进入 RunPod Pod env。`.env.cloud.test` / `.env.cloud.prod` 可保存真实值但不得提交或输出；当前 `all-github-token` 带中划线，不能通过 `source` 直接导出，本机推镜像前需手工映射到合法 shell 变量 `GHCR_TOKEN` / `GITHUB_TOKEN`。GHCR push 后必须用空 `DOCKER_CONFIG` 匿名 pull/inspect 验证 package public。
- **Wan22 RunPod 冷拉状态**：2026-06-13 旧 `wan22_aio_video` GHCR 镜像为 28 层、压缩约 6.59 GiB，不包含业务大模型；两次 5090 cloud-test 真实 canary 均卡在 RunPod `Downloading your container...` 冷拉阶段并中止，未进入模型同步或任务执行，Pod 已清理。第一轮启动优化切到 RunPod 官方 PyTorch/CUDA base `runpod/pytorch:1.0.2-cu1281-torch280-ubuntu2404` 后，GitHub Actions 成功发布 `20260613-wan22aio-runpodbase-89eca6c`，但 manifest 为 36 层、压缩约 11.94 GiB、最大单层约 4.0 GiB；真实 canary Pod `cqghk2kaa1x5dq` 约 10 分钟仍停在 `runtime=false` / `pod_readiness`，已中断并清理到 managed Pod=0。RunPod 官方 ComfyUI worker base 也很大，不适合作为默认提速路线。当前 Wan22 GitHub Actions 默认 base 回到局域网成功环境 `192.168.1.2:8189` 同款 `yanwk/comfyui-boot:cu128-slim`，该实例为 ComfyUI `0.22.0`、PyTorch `2.11.0+cu128`，`/object_info` 已验证 V82 需要的 `FL_RIFE`、`LTXVSpatioTemporalTiledVAEDecode`、`WanImageToVideo`、`WanFirstLastFrameToVideo`、DaSiWa、rgthree、VHS 等节点全部存在；Dockerfile pin 关键 custom node commit。不要直接 `docker commit`/push 局域网 `comfy1` 容器，因为 `/root` 是 Docker volume，`models/custom_nodes/workflows/input/output/temp` 是 bind mount，commit 会漏掉关键内容且可能混入运行残留。后续 `20260613-wan22aio-fillnodes-layer-8ea2c33` 通过 template 创建 4090 Pod `g7ni1ty514pbm7`，约 12 分钟后完成 worker heartbeat 并接到 `wan22_video_v2` 任务，但 `/prompt` 失败于 `missing_node_type: LTXVSpatioTemporalTiledVAEDecode`；修复口径是只从 `Lightricks/ComfyUI-LTXVideo` 注册 `tiled_vae_decode.py` 的最小 `LTXVSpatioTemporalTiledVAEDecode` 子集，并在 build smoke 中实际导入 `NODE_CLASS_MAPPINGS`。后续 `20260613-wan22aio-lanbase-ab9b7ea` 通过 template `77gi0wqo8x` 在 4090-only Pod `tyejnfyigvwc68` 上完成 cloud-test `wan22_video_v2` Web 端闭环：任务 `0f170dec-a48f-4f3c-9393-e1dcd053937f` 由 `runpod_test_wan22_video_v2_tyejnfyigvwc68` 接取，Central `done`、Web result `success`，结果下载为项目根目录 `wan22_video_v2.mp4` 与 `wan22_video_v2_last_frame.png`。DaSiWa installer 可参考 custom node/依赖清单，但当前官方 Linux installer 仍是 wizard/self-updating 口径，不适合直接放进每次 RunPod cold-start 的 `dockerStartCmd`。Wan22 后续启动优化优先评估 RunPod Network Volume/High-performance storage 预热 `/workspace/ComfyUI/models`，避免每次从 R2 重拉几十 GiB。
