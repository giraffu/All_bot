...
- 控制面中需要视频探测、尾帧提取或拼接的 `qqcc-bot`、`private-bot-worker`、`qqcc-config-backend`、`dashboard-backend` 继承窄基础产物 `python-media-runtime-base`。该基础产物只提供 `ffmpeg` / `ffprobe`，本身不可部署、不可做服务身份启动检查；其 Dockerfile 变化只重建真实 descendants。full-validation 必须在四个最终 digest 分别执行双工具 smoke，不能只验证基础镜像或其它服务。
