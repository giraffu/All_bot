...
- **SCAIL-2 RunPod 云测试 profile**：`RUNPOD_TASK_PROFILES["scail2"]` 只用于 cloud-test，声明 `SUPPORTED_TASK_TYPES=scail2_action_transfer,scail2_video_replacement`、`POOL_RUNTIME_PROFILE=scail2`、agent 前缀 `runpod_test_scail2`，默认 `containerDiskInGb=120`、GPU 优先 `NVIDIA GeForce RTX 5090,NVIDIA GeForce RTX 4090`。镜像由 `.github/workflows/runpod_scail2_profile_image.yml` 构建为 `ghcr.io/giraffu/allbot-comfy-runpod-scail2:<tag>`，不 baked 模型，默认 `dockerStartCmd=["bash","-lc","exec bash /opt/allbot/runpod_bootstrap_from_git.sh"]`。模型桶固定 `allbot-model-cache`，manifest 为 `scail2/2026-06-17-test/manifest.json`，用户输入/结果桶仍是 `user-data-test`。`runpod canary --task-type scail2` 会上传/复用 Nomadoor 样例参考图与驱动视频，临时 disable 支持 SCAIL-2 的非 RunPod cloud-test worker（通常是 `cloud_worker_test_08`），串行提交 `scail2_action_transfer` 与 `scail2_video_replacement` 两个 5s Web 任务，结束后恢复 worker control 并删除 Pod。该 profile 仍不作为云正式 RunPod 能力；云正式本轮使用 LAN slot0 runtime。
