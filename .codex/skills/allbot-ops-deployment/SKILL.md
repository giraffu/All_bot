...
- schema v2 将不可变产物拆为 `control-plane`、`test-execution`、`gpu-execution` 三条环境无关发布链；`release-index.json` 引用三份 manifest，test/prod 只选择模块并注入配置。已有增量基线时，普通自动发布只从策略影响集合中选择 bundle 内 `source_sha` 等于目标 SHA 的运行时 artifact；全部 artifact 均复用旧 `source_sha` 时选择集必须为空，风险 level/matched rules 只作为审计元数据，不得触发空重建。可信上游 full CI 的手动 replay 可用受限 `release_artifact` 单独重建一个已登记 control-plane artifact 及其 base，并继承完整 GPU baseline；累计 diff 中其它 GPU 变化不得把该控制面定向构建重新扩成 GPU canary 门禁，GPU/test/external/未知 artifact 选择仍 fail closed。`--services` 仍只能扩大影响集合；显式 `--modules dashboard|qqcc-bot|qqcc-config` 是三个受控独立模块边界，一次只能选择一个完整组，并从组内每个 artifact 自己的 `source_sha` 分别计算差异。旧版局部 `current.json` 缺失的 artifact 只从按时间排序的成功 history 在内存恢复，不现场改状态；组内混合版本允许保留各自真实基线。
...
- 控制面中需要视频探测、尾帧提取、拼接或 QQCC 智能画幅适配的 `qqcc-bot`、`private-bot-worker`、`qqcc-config-backend`、`dashboard-backend` 继承窄基础产物 `python-media-runtime-base`。该基础产物提供 `ffmpeg` / `ffprobe`、固定版本 OpenCV headless/SmartCrop，以及用 SHA-256 锁定的 YuNet ONNX；本身不可部署、不可做服务身份启动检查，其 Dockerfile或媒体智能依赖清单变化只重建真实 descendants。full-validation 必须在四个最终 digest 分别执行双工具 smoke，并验证 `cv2` / `smartcrop` 导入和模型存在，不能只验证基础镜像或其它服务。
...
- 并发研发的 handoff 默认进入本机持久化队列；用户级单写者把当前等待的精确 head 冻结为一个 `release-batch -> main` PR，后到 handoff 排入下一批，失败批次阻断后续。批次 PR 不构建发布容器；main push 的上游 CI 成功后，`modular-release-v2.yml` 才为该 main SHA 构建一次 main-channel bundle，随后协调器仅串行部署 test/control-plane；lightweight 跳过 bundle和环境。协调器不接受 prod 参数。若 GitHub 没有投递 push workflow，可对当前 main 精确 head 手动重跑 `Immutable control-plane release`；模块化 workflow 只有在 GitHub API 证明该 run 的 repository/workflow/path/main SHA、成功结论和全部预期 test jobs 完整匹配时才按 `full` 消费。模块化 workflow 自己的手工入口仍只能生成 `build-only`。`codex/test-train`、test-candidate 和 approval/promotion workflow 只作历史兼容，不再生成新候选。
- `scripts/classify_ci_change.py` 提供四类 fail-closed scope。`lightweight` 覆盖纯 docs/Skills/tests/仓库治理；`release-tooling` 精确覆盖发布器、配置契约、CI run 校验、classifier 与自动集成协调器，只跑发布专项回归；两者均不创建 release bundle。`operator` 仅覆盖明确列出的 GPU/RunPod/LAN operator/helper 并运行 operator 回归；其它路径、空集或聚焦 scope 与业务 runtime 混合均为 `runtime` 并恢复全量流程。任何 scope 都不授权 direct push main 或环境 mutation。
- 发布策略是 `--strategy auto|standard|direct|emergency`。核心用户链路与已有专属测试实例的 QQCC Config 默认 standard；Dashboard 与 GPU 执行面默认 direct；公共 Web 默认 standard、可显式 direct；emergency 与跳过测试只要求明确 `--reason`，不再要求单人开发者提供 `--approved-by`。云测试拓扑不存在 `payment-api` 与 `paid-group-bot`，日常 `promote` 对这两个 prod-only artifact 使用 direct assurance（受保护 main CI、不可变 digest、正式健康与回滚），不得伪造测试 state；同批次的 `central-api`、`web-api`、`main-bot` 等测试可用核心 artifact 仍必须命中 exact-digest 测试 evidence。普通混合变更取最高风险。独立模块发布只重建所选服务；migration、未知共享 Compose/env 或未审计跨模块契约仍 fail closed。两类 migration 窄例外都必须按模块、文件和内容 SHA256 精确固定：reviewed additive migration 仍执行备份、单 head 与 `--confirm-db-upgrade`；reviewed non-target migration 明确属于其它模块，不设置 `requires_db_upgrade`、不执行 Alembic，只允许显式模块的高级 `deploy --policy <已提交main策略> --no-maintenance`。任一内容或模块漂移立即恢复阻断。已审阅并固定内容 SHA256 的 owner-only Compose/env 与向后兼容 schema/config snapshot 可继续独立发布；worker-only `deploy/docker-compose-worker-base.yml` 也只能以当前精确快照跨越，且不得因此选择或变更任何 Worker/GPU 服务。日常 `promote` 必须从候选不可变 SHA 读取默认策略，不能信任调用工作目录的旧策略；任一文件内容变化即恢复阻断，不能把 snapshot 当通配 allowlist。
- `--skip-gate` 只允许 `ci-tests|test-deploy|test-acceptance|gpu-business-canary`，并受策略约束。固定 observation gate 已取消；CI `validation_mode=build-only` 仍必须从受保护 main 完整 SHA 构建 digest 产物，发布时显式跳过 `ci-tests` 且记录 reason；任何 execute 都禁止 `--skip-ci-checks`。main 血缘、成功构建、digest/checksum/OCI revision、配置、目标健康、事务/回滚和非目标服务不重建永久保留。
- 云测试默认部署 main bundle 中所需的 control-plane/公共 Web artifact；专属测试 QQCC Config 前后端固定使用测试配置及 8045/8088，Dashboard 仍不进入测试站。test-execution 只在专项诊断时显式选择。验收状态按 track + artifact digest 写入 history；streamlined smoke 写 `verified`，strict direct/emergency 可继续写 `waived`，GPU direct 写 `attested`，不得覆盖其它核心 artifact 的 tested 证据。
- `release.py` 在同一 CLI 门面内自动输出并选择 `execution_profile=streamlined|strict`。普通 schema-v2 main control-plane、bundle `validation.mode=full/tests=passed`、已知影响且目标配置投影无漂移时走 streamlined；migration、Compose/env/共享发布契约、数据库/Redis、首次切换、未知影响、GPU/test-execution 或任一 strict artifact 使整个混合事务进入 strict。两种 control-plane profile 的代码发布都只读核验目标环境实际存在服务的 current projection；strict 不得因此退回全 catalog 配置检查，但继续保留完整备份、Alembic、维护、排空、回滚与恢复能力。streamlined 不做完整 rollback preflight、旧 checkout 准备、旧镜像预拉、非目标快照、全局 queue drain 或固定 observation，只单次 pull/`up --no-deps --wait` 目标服务，并用本机已有旧 ref 做目标回切；回切失败才保留 maintenance 和 recovery transaction。独立模块 artifact 的 `source_sha` 只表示镜像来源，不能推导当前 Compose checkout；streamlined 必须分别从每个目标运行容器的 Compose labels 核验其指向受控 `/releases/<sha>/deploy` 且配置文件精确为当前环境的 base + overlay。独立模块分次晋级后，不同目标可合法来自不同历史 checkout；发布器选择其中一个已核验 checkout 作为本次 Compose seed，并逐目标保存当前 exact image ref 用于回切，不得因此触碰非目标容器。
- `plan` 与 `preflight` 会输出 10 分钟有效的本机 `plan_token`；后续 preflight/deploy 传 `--plan-token` 可复用候选、GitHub CI 与已通过 evidence，参数、输入 checksum、SHA 或目标变化立即失效，执行前仍重新读取目标配置 revision。strict 远端阶段实时输出非敏感心跳并记录 candidate/config/pull/backup/migration/replace/health/state 耗时；失败 journal 保留脱敏阶段与原因。Alembic `heads/upgrade` 临时容器固定 `--no-deps`。test/prod 普通 deploy 若 exact ref、RepoDigest、OCI revision、逐服务 config revision 与健康均一致，直接返回 `no-change`，不进入 preflight、pull 或替换。
- 日常正式发布唯一门面是 `scripts/release.py promote [--modules <逗号列表>] [--sha <完整SHA>] [--no-maintenance] [--confirm-prod]`。缺省只锁定一次最新受保护 main 第一父链 bundle，并按正式运行 digest/Pages 自动选择不一致模块；无确认只输出精简预览，一次 `--confirm-prod` 直接完成预检、发布、验收与失败回滚。用户明确决定所选模块不进入维护时，operator 可把该决定翻译为 `--no-maintenance`，不再要求第二次确认；该参数必须有显式模块，且不能跳过未审阅 migration、首次切换、部署契约、blocker 或未知路径，失败补偿仍可为安全恢复启用维护。候选旧策略尚未包含已提交 main 的精确 non-target migration 审批时，使用高级 `deploy --policy <clean-main-policy> --modules ... --no-maintenance`，仍须先 dry-run 并保留 main/CI/digest/config/health/rollback/non-target 门禁。`plan|preflight|deploy|deploy-module|rollback|recover|config-plan|config-apply|credential-isolation-complete` 其余场景继续作为高级兼容入口。云端不 build、不挂载源码、不接收代码/env rsync。
- 云控制面运行配置的唯一事实源是目标主机 `/etc/allbot/test.env` 或 `/etc/allbot/prod.env`。发布器合并版本化非敏感默认值后，只生成权限 `600` 的 `/var/lib/allbot/config/<env>/<revision>/<service>.env` 逐服务投影；`release.env` 不保存秘密。服务契约用 `environments` 声明 test/prod 拓扑，测试全量计划不得构造 prod-only Dashboard/Payment/Paid Group/Support 投影，正式也不得构造 test-only PostgreSQL/Redis 投影。主 Bot 的 `REQUIRED_CHANNEL_ID` 是频道成员同步与签到资格的精确必填投影，只属于 `main-bot`，不得作为可忽略 legacy key；缺失时配置阶段 fail closed。正式主 Bot 懒人入口使用 `MAIN_BOT_LAZY_BOT_ENABLED` 与 `MAIN_BOT_LAZY_BOT_USERNAME`，同样只允许进入 `main-bot` 投影；旧 `QQCC_LAZY_BOT_*` 仅作运行时兼容回退，不得用宽泛 `QQCC_*` 规则把新键投给 QQCC 服务。全局 `config-plan/config-apply` 校验当前环境完整契约，并报告/原子移除历史 `retired_services`；模块级 apply 不清理历史非目标投影：先验证完整 active state，再将当前环境已退役或契约未知的非目标投影按原始字节和 revision 复制到新快照，只重建目标与当前环境仍适用的 active 投影。所有具有容器 env 契约的独立模块均可用 `--module` 只暂存本模块投影，Public Web 没有容器 env 投影、继续单独注入 runtime config。若同 digest 发布会被判定 no-change，可在模块投影已暂存且 drift-free 后使用 `config-apply --module <name> --activate-staged --service <exact-service>`；该入口只滚动重建显式服务、核对全部非目标容器身份、不开维护，失败时回滚投影和旧 `release.env`。目标缺键、可重建非目标 active 服务变化、闭包逃逸、投影篡改或影响目标的未知键均 fail closed。
...
- 秘密隔离状态只作运行风险审计，不再阻断日常 `promote` 或要求额外接受参数。轮换完成仍可通过 `credential-isolation-complete --evidence <json> --confirm-prod --execute` 写入状态；证据必须在一小时内、覆盖全部目标键、证明 test/prod 不复用、所有目标健康且旧凭据已撤销。跨环境比较使用随机 challenge HMAC，只输出键名、是否相同和证据摘要；GPU token 切换继续走 LAN AIO/RunPod 专用 operator。
- 云测试：先执行 `scripts/release.py plan --env test --sha <sha>` 核对 execution profile，再以同一 SHA 执行 `deploy --env test --sha <sha> --execute`。streamlined 成功 smoke 后自动按 artifact + exact digest 写 `verified` evidence 和真实开始/完成时间；`verify-test` 只作专项人工补充。影响集合由 `deploy/release-policy.yml` 计算，`--services` 只能扩大，不能缩小。
...
- 事务 journal 复制 previous state 时必须保留 `pending_secret_rotation_acceptance` 这一非敏感审计记录；secret-free 校验只允许这个精确字段名并继续递归拒绝其内部任何 token/secret/password/env-values 字段。不得为消除误报删除历史接受记录，也不得放宽为通用 secret 字段白名单。
- 测试站不运行 Dashboard，但 Dashboard-only candidate 仍可能以携带全部继承 artifact 的完整 bundle 成为 control-plane 记录基线。若其 rollback checkout/release.env 缺失，`recover --modules dashboard --repair-rollback-materials` 在全局 `current.json.git_sha` 等于该 bundle SHA 时不得要求 `current.json.artifacts` 存在 Dashboard 条目，而必须从同一完整 bundle 展开实际启用的测试服务，逐项核对这些服务的 `current.json` digest 和运行容器 image digest 后才物化完整回滚合约；不得寻找不存在的测试 Dashboard 容器，也不得只写 Dashboard 变量或跳过运行服务核验。
...
- 环境中立门禁按 exact image ref 去重；共享 GPU profile 只扫描一次。文件系统检查通过停止容器的 `docker cp` tar 流限定读取 `/app`、`/opt/allbot`、`/usr/src/app`，禁止整镜像 `docker export` 把模型层纳入扫描；`.env*`、`.pem`、`.key` 仍递归 fail closed。pull、配置、文件树和 runtime identity 分阶段计时并有固定超时。每个可运行 Python service artifact 仍分别注入 test/prod 哨兵身份并真实导入；`python-runtime-base`、`python-media-runtime-base` 和 `python-worker-base` 不携应用源码，只执行 Config.Env 与文件系统扫描，不得对其执行服务身份启动检查。
...
- `config-apply` 和 migration 的 `pg_dump` 在容器 POSIX `/bin/sh` 中运行；migration 的临时 PostgreSQL client 必须通过 `--network container:$web_container` 共享当前 Web 容器网络，不能让 Compose DNS 名称在孤立 `docker run` 中解析失败。`postgresql+asyncpg:` 转换必须使用 `case` + `${VAR#prefix}`，禁止 Bash-only 替换语法。未知 scheme、空备份或非单 Alembic head 都必须在 projection/容器变更前 fail closed，不得人工跳过。
...
- standard 测试验收只要求本次选择 artifact 对应的检查和精确 digest；streamlined 容器 health、解析后 API_BASE 和 single polling/getUpdates conflict smoke 成功后自动落 verified evidence，公共 Web 仍校验 tar checksum，测试 Worker 仅在显式 track 中检查 heartbeat。evidence 记录真实开始/完成时间与实际持续秒数，不设固定最短 observation，也不要求 rollback drill。
- 正式 streamlined 晋级不重新查询 GitHub CI：direct artifact 只校验 bundle `validation.mode=full/tests=passed`；standard 从云测试 retained history 按 artifact + exact digest 查找 main-channel verified 证据。只 inspect 目标容器和目标配置投影；非目标历史未知服务或 drift 不阻断，目标自身缺键、篡改/revision drift 必须先显式 `config-plan/config-apply --module`。Dashboard 只有 LAN runner 相关影响规则命中才探测 runner；Public Web 未选中不得初始化 Wrangler/Pages。每次 execute 仍必须 `--confirm-prod`。
- `central-api`、`web-api`、主 Bot、QQCC Bot、私有 Bot worker 任一被选中时，默认把混合事务提升为生成维护；Dashboard、QQCC 配置后台、Payment、Paid Group Bot、Public Web 单独发布 rolling 替换。用户明确决定后，显式模块 `promote --no-maintenance` 可把 forward rollout 改为 rolling，并在预览、事务 journal 中记录 `maintenance_required=true` / `maintenance_waived=true`；migration、Compose/发布契约、首次切换、blocker 和未知影响仍不可豁免。`no-change` 只有实际容器 RepoDigest、健康和 config revision 全部一致才能报告；该只读运行态核验必须早于 mutation rollback preflight，同 SHA 已精确运行时不得因本地缺另一份 previous manifest 而误阻断。
...
- 共享云测试站只有一个写入者。A-H 功能 AI 不得部署；启用 `allbot-ai-integration-queue.timer` 后，由自动协调器在 main bundle 成功后通过固定的 `release.py ... --env test --track control-plane` 串行切换 control-plane/公共 Web。禁止同时启动另一条共享测试发布；test-execution 仅用于专项诊断，显式启用时按 control-plane → test-execution 顺序切换。
...
- Dashboard autoscaler 基于预计清空时间、profile 阈值、Redis leader lease 与 operation store 做 add/down/restart/enable；不直接操作本地 worker，不绕过 RunPod 门禁；Dashboard Backend 的最小服务投影必须显式包含 `AGENT_SECRET_TOKEN`，因为 RunPod down/delete 会先调用 Central agent control，缺失时必须在投影阶段 fail closed，不能靠整份宿主 env 或镜像内 `.env` 补齐；RunPod Worker 卡片的 `锁定/解锁` 会让手动删除、autoscaler down 和 add cleanup 跳过该 worker。
...
5. migration、共享/未知契约、emergency 与跳门禁必须阻断 `promote` 并改用相应高级入口；秘密轮换状态仅记录审计，不要求额外确认。目标配置缺键或漂移时先停止，只有用户另行要求配置收敛才使用 `config-plan/config-apply`。
...
- 定向任务在 push 后、assemble 前失败时，同 SHA 重试只可在 registry tag 的 OCI revision 精确等于目标 SHA 后复用其 digest；缺失或不匹配必须 fail closed，不得覆盖不可变 tag。
