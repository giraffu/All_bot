...
- Dashboard Backend 镜像 smoke 必须真实 import `dashboard.backend.main` 和 `dashboard.backend.qqcc_config_main`；镜像依赖闭包至少包含根 `config.py`、`src`、`shared`、`paid_group_guard_bot`、Dashboard 运维路由依赖的 `ops`，以及 LAN AIO 发布链路需要的 `scripts/gpu_release_rollout.py`、`release_manifest_v2.py`、`release_strategy.py`；禁止用单个文件存在检查代替导入闭包。
