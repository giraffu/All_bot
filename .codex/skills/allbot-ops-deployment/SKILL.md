...
- 发布策略是 `--strategy auto|standard|direct|emergency`。核心用户链路与已有专属测试实例的 QQCC Config 默认 standard；Dashboard 与 GPU 执行面默认 direct；公共 Web 默认 standard、可显式 direct；emergency 与跳过测试只要求明确 `--reason`，不再要求单人开发者提供 `--approved-by`。普通混合变更取最高风险。独立模块发布只重建所选服务；migration、未知共享 Compose/env 或未审计跨模块契约仍 fail closed。已审阅并固定内容 SHA256 的 owner-only Compose/env 与向后兼容 schema/config snapshot 可继续独立发布；日常 `promote` 必须从候选不可变 SHA 读取默认策略，不能信任调用工作目录的旧策略；任一文件内容变化即恢复阻断，不能把 snapshot 当通配 allowlist。
