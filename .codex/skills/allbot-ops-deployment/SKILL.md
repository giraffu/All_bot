...
- **SCAIL-2 History 恢复**：若 SCAIL-2 backend 生成成功但 Web History/闪回瓶缺失，先确认云正式 Alembic 已迁到 `history.type VARCHAR(64)`，再用 `scripts/recover_scail2_history_delivery.py` 按 `snapshot --dry-run`、`recover --execute`、`send --execute` 分步处理。该脚本只处理快照内 `scail2_action_transfer` / `scail2_video_replacement` 且 backend `done + result_path` 的任务；backend error/cancelled 只审计，不退款、不重启 GPU/RunPod、不手工写 History。
...
- **RunPod 正式按需容量口径**：正式手动 RunPod 池的容量和 profile 组合按当次运维目标决定，不写死为固定台数、固定日期或固定类型组合；某次实操的 Pod 数量/profile 组合只应进入运维日志或工单，不作为长期 SOP。启动/恢复优先用 `scripts/runpod_prod_ops.sh add --profile ... --count N --execute`；真实执行时显式设置 `RUNPOD_DRY_RUN=false`、`RUNPOD_AUTOSCALER_ENABLED=true`，必要时设置 `RUNPOD_PROD_MAX_MANUAL_SLOTS=<slot命名空间上限>`，默认建议 `100`。`prod-worker` 的真实 `up/down/add/scale` 会按 profile 持有文件锁，默认锁目录 `/tmp/allbot_runpod_locks`，并在 create 前复查目标 slot 仍空闲；RunPod 返回 `There are no instances currently available` 时可用 wrapper 的 `--retry-unavailable` 做有界轮询；不要并发启动相同 profile/count 的重复创建循环。
...
- **LAN AIO fleet 泛化接管**：gpu-252 后续用 `ops/gpu_pool_controller/config/lan_aio_prod_slots.yml` + `scripts/lan_aio_fleet_prod_ops.py` 逐 slot 灰度，不再复制 gpu-002 专用 helper。`gpu-177` 已整机 AIO 正式接管，配置阶段为 `prod_enabled`：GPU0 `lan_aio_prod_gpu177_gpu0_image_to_video_01` 提供 `image_to_video`，GPU1 `lan_aio_prod_gpu177_gpu1_ltx_video_01` 提供 `ltx_video`；旧 `cloud_prod_worker_02/03` 与 `comfy0/comfy1` 只作为 stopped rollback baseline 保留。`gpu-252-gpu1-wan22_video_v2` 已替换 `cloud_prod_worker_05`，AIO agent `lan_aio_prod_gpu252_gpu1_wan22_video_v2_01` 只声明 `SUPPORTED_TASK_TYPES=wan22_video_v2`，旧 `comfy1` 与 `cloud-prod-comfy-agent-5` stopped rollback；该节点当前 Docker/NVIDIA runtime 对 worker05 原设备暴露为 device `0`，slot 配置需保留 `gpu_index: 0`。当前 LAN AIO 正式能力包括 `image_to_video`（兼容 `video_insert` / `video_edit` alias）、`ltx_video`、`wan22_video_v2`、SCAIL-2 的 `scail2_action_transfer` / `scail2_video_replacement`；canary-ready 但未接管的 fleet 候选是 `gpu-252-gpu0-img2img_lora`；`gpu-226` 是 host_service，不能直接接管。真实顺序固定为 `preflight --execute`（只读，接受 Docker insecure registry 已配置或目标镜像已预置）-> 维护窗口 `configure-registry --execute`（会重启目标 Docker daemon；无免密 sudo 时可改用 `docker save ... | ssh ... docker load`）-> `pull-image --execute` -> `start-disabled --execute` -> 验收 disabled heartbeat 元数据 -> 小窗口 `enable-aio --execute` -> `drain-aio`/观察 -> 按接管目标 `stop-old --execute`。`enable-aio` 会先 disable legacy，并拒绝在 legacy 仍 running、AIO disabled heartbeat 不可见或旧 runtime 仍占 GPU 显存时放开 AIO，避免同卡双 ComfyUI 抢单。所有 mutation 必须显式 `--slot` 和 `--execute`，禁止跨节点批量启用；compose 必须是 `cloud-prod`、正式 Central、`user-data-prod`，且 remote_workers 必须挂载当前仓库 bundle。LAN AIO 的 `image_to_video` / `wan22_video_v2` split profile必须由 runtime-render 写入 `COMFY_EXTRA_ARGS=... --disable-dynamic-vram`，避免 32G 5090/RunPod cu128 DynamicVRAM 触发概率性 OOM。`start-disabled` 还会按 slot 的 `legacy_hot_cache_copies` 预置旧 ComfyUI 热缓存；`gpu-177-gpu0-image_to_video` 从旧 `comfy0` 复制 `rife49.pth`，`gpu-252-gpu1-wan22_video_v2` 从宿主机旧 `inst1` 路径复制同一文件，目标都是 AIO 内 `ComfyUI_Fill-Nodes` 与 `ComfyUI-Frame-Interpolation` 缓存路径，不能依赖运行时访问 HuggingFace。
...
- **SCAIL-2 LAN AIO 测试 runtime**：`scripts/lan_scail2_aio_test.sh` 用于 gpu-002 GPU0/`8190:8188` 的 SCAIL-2 LAN AIO runtime，可同时服务 ComfyUI 手工测试、Web 测试站和测试 Bot 的“动作迁移 / 视频换人”两类任务。`start --execute` 会先 drain/disable `lan_aio_prod_gpu002_gpu0_img2img_lora_01` 并保持 `cloud_prod_worker_06=disabled`，slot1 保持运行。容器 `allbot-lan-aio-gpu-002-gpu0-scail2-test` 仍不设置 `AGENT_ID` / `CENTRAL_API_URL` / `SUPPORTED_TASK_TYPES`，只启动 ComfyUI UI、LAN model sync、四个 Nomadoor UI workflow、两个业务 API workflow 和样例素材；它本身不直接接 Central 队列。完成 smoke 后视频复制到 `gpu-002:/root/scail2-test-results/<timestamp>/`，恢复 slot0 用同一 helper `restore --execute`。
- **SCAIL-2 云测试 worker**：业务接单层在本地主 `workers/docker-compose-cloud-worker-test.yml` 的 `cloud-comfy-agent-test-8` / `cloud_worker_test_08`，指向上述 LAN AIO runtime `http://192.168.1.2:8190`，声明 `SUPPORTED_TASK_TYPES=scail2_action_transfer,scail2_video_replacement`、`POOL_RUNTIME_PROFILE=scail2`、`POOL_NODE_ID=gpu-002`、`POOL_GPU_INDEX=0`。`scripts/start_cloud_worker_test.sh` 启动后会把该 agent control 置为 `disabled`，验证 `/object_info`、workflow mapping、Web 5s smoke 和测试 Bot 5s smoke 后才可手动 enable；该能力只属于云测试，不同步云正式 worker compose。
- **SCAIL-2 云正式 slot0 runtime**：正式低影响发布不重建 `cloud-prod-comfy-agent-1..7`，不创建/启停 RunPod，只用 `scripts/lan_scail2_aio_prod.sh` 接管 gpu-002 slot0/`8190:8188`。新 agent 固定 `lan_aio_prod_gpu002_gpu0_scail2_01`，容器固定 `allbot-lan-aio-gpu-002-gpu0-scail2-prod`，支持 `scail2_action_transfer,scail2_video_replacement`，运行环境必须是 `cloud-prod`、正式 Central `https://worker-central.aivison.it.com`、R2 `user-data-prod`，且 compose/env 不得出现 `cloud-test` / `user-data-test`。`start-disabled --execute` 只 drain 旧 slot0 AIO agent `lan_aio_prod_gpu002_gpu0_img2img_lora_01` 并等待自然空闲，停止旧 slot0 容器后启动 SCAIL-2 disabled heartbeat；验收 `/system_stats`、`/object_info`、模型枚举和 disabled heartbeat 后才 `enable --execute`。rollback 只停 SCAIL-2 slot0 并恢复旧 slot0 img2img_lora AIO，slot1/其它 worker/RunPod 全程不动。
