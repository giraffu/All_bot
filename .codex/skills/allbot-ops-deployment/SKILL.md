...
- standard 测试验收只要求本次选择 artifact 对应的检查和精确 digest；公共 Web 校验 tar checksum，核心按选中服务检查，测试 Worker 仅在显式 track 中检查 heartbeat。evidence 记录真实开始/完成时间与实际持续秒数，但不设固定最短观察时长，也不再接受或要求短观察 override。
