...
- `scripts/classify_ci_change.py` 提供三类 fail-closed scope。`lightweight` 仅覆盖纯 docs/Skills/tests/仓库治理、release-batch JSON 审计记录与 CI 门禁元数据，跳过全量模块 CI 和 release bundle；`operator` 仅覆盖 `ops/gpu_pool_controller/**` 与明确列出的 RunPod/LAN operator/helper，单独跑 `tests/ops tests/scripts`，仍允许 modular workflow 为实际命中的 artifact 构建 bundle；其它路径、空集或 operator 与业务 runtime 混合均为 `runtime` 并恢复全量流程。LAN 主机 helper 变化没有 control-plane artifact；Dashboard 镜像内置 controller/rollout 依赖只重建 `dashboard-backend`。`remote_workers/**`、`deploy/release-artifacts-v2.json` 中的 GPU release artifact/profile、镜像 Dockerfile/基础依赖继续走完整 CI 与 GPU attestation/canary/operator 门禁。任何 scope 都不授权 direct push main 或环境 mutation。
