...
- **GPU Pool Controller v1**：本地 GPU 资源池第一阶段入口为 `ops/gpu_pool_controller/`、`scripts/gpu_pool_controller.py`、`deploy/docker-compose-local-registry.yml` 与 `scripts/manage_local_registry.sh`。默认 dry-run/canary，不自动重启生产 worker、ComfyUI 或 GPU 节点；详细口径见 `docs/子模块_GPU算力资源池控制器_gpu_pool_controller.md`。
