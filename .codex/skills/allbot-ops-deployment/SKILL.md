...
- **旧本地分阶段重建**：旧本地正式栈才按 workers -> central api -> 主服务群 -> dashboard -> 旧边缘静态站的顺序重建/发布；云正式不走该路径。
...
- 功能研发默认目标环境是云测试控制面：`.env.cloud.test` + `deploy/docker-compose-cloud-test.yml`；旧本地隔离测试栈 `.env.test`、`backend/docker-compose-test.yml`、`workers/docker-compose-test.yml`、`deploy/docker-compose-test.yml` 仅作兼容/回滚路径。
...
- 云测试公网 Web 入口继续使用 `web-test.aivison.it.com` 的边缘 VPS 静态站；前端静态资源由 `frontend npm run deploy:edge-test` 发布到 `web` VPS `/root/dist-test`，VPS Nginx 的 `/api/` 必须反代到云测试 Web API `http://100.82.124.91:8001`。云测试服务端口绑定到该 Tailscale IP；公网 eth0 上的 `8001/8004/8044/8084` 由 `allbot-cloud-test-firewall.service` drop。
...
- 云正式 Dashboard 卡顿要统计 `cloud-dashboard-backend-prod` 中 `Circuit Breaker is OPEN`，并区分本地 Dashboard 网关、Central 观测接口、外部余额接口和 Dashboard stats 缓存失效；不要把 Dashboard 熔断直接当作任务调度失败。本地网关可短缓存 `/api/stats*` 与 `/api/system/status|workers|concurrency_stats`，但登录、退款、封禁、删除和清理类写操作不得缓存。
...
- 云正式 worker compose 包含本地 relay/上传 sidecar。更新 `workers/comfy_agent`、`workers/local_relay`、`worker_requirements.txt`、`remote_workers/` 或 worker compose 时，测试 canary 需额外验证 relay `/health`、`relay_forward_failed`/`sidecar_upload_failed` 日志、R2 上传成功后才 `/complete`，以及 Central `/system/workers` 无 error/quarantined。
...
若目标旧本地 worker 正在处理任务，非紧急情况下应先告知用户会中断该 worker 当前任务，并尽量等待任务完成或确认可以中断；旧本地正式全量发布才走 `safe_deploy.sh` 的队列门禁流程。
