...
- **GPU Pool Controller v1**：本地 GPU 资源池第一阶段入口为 `ops/gpu_pool_controller/`、`scripts/gpu_pool_controller.py`、`deploy/docker-compose-local-registry.yml` 与 `scripts/manage_local_registry.sh`。默认 dry-run/canary，不自动重启生产 worker、ComfyUI 或 GPU 节点；`runtime-plan` 只输出 runtime/image/model/worker-env diff，`runtime-render` 只渲染标准 Comfy runtime compose，`runtime-apply`/`switch-profile`/`rollback-profile --execute` 当前会拒绝真实执行。`gpu-226` 是 `host_service`，Controller 不得为它生成 Docker 操作；`gpu-002` 是 Phase 1 试点。详细口径见 `docs/子模块_GPU算力资源池控制器_gpu_pool_controller.md`。
