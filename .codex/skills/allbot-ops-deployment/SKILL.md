...
- **LAN AIO fleet 泛化接管**：gpu-252 后续用 `ops/gpu_pool_controller/config/lan_aio_prod_slots.yml` + `scripts/lan_aio_fleet_prod_ops.py` 逐 slot 灰度，不再复制 gpu-002 专用 helper。`gpu-177` 已整机 AIO 正式接管，配置阶段为 `prod_enabled`：GPU0 `lan_aio_prod_gpu177_gpu0_image_to_video_01` 提供 `image_to_video`，GPU1 `lan_aio_prod_gpu177_gpu1_ltx_video_01` 提供 `ltx_video`；旧 `cloud_prod_worker_02/03` 与 `comfy0/comfy1` 只作为 stopped rollback baseline 保留。当前 LAN AIO 正式能力包括 `image_to_video`（兼容 `video_insert` / `video_edit` alias）、`ltx_video`、SCAIL-2 的 `scail2_action_transfer` / `scail2_video_replacement`；canary-ready 但未接管的 fleet 候选是 `gpu-252-gpu0-img2img_lora` 与 `gpu-252-gpu1-wan22_video_v2`；`gpu-226` 是 host_service，不能直接接管。真实顺序固定为 `preflight --execute`（只读）-> 维护窗口 `configure-registry --execute`（会重启目标 Docker daemon）-> `pull-image --execute` -> `start-disabled --execute` -> 验收 disabled heartbeat 元数据 -> 小窗口 `enable-aio --execute` -> `drain-aio`/观察 -> 按接管目标 `stop-old --execute`。`enable-aio` 会先 disable legacy，并拒绝在 legacy 仍 running、AIO disabled heartbeat 不可见或旧 runtime 仍占 GPU 显存时放开 AIO，避免同卡双 ComfyUI 抢单。所有 mutation 必须显式 `--slot` 和 `--execute`，禁止跨节点批量启用；compose 必须是 `cloud-prod`、正式 Central、`user-data-prod`，且 remote_workers 必须挂载当前仓库 bundle。LAN AIO 的 `image_to_video` / `wan22_video_v2` split profile必须由 runtime-render 写入 `COMFY_EXTRA_ARGS=... --disable-dynamic-vram`，避免 32G 5090/RunPod cu128 DynamicVRAM 触发概率性 OOM。`start-disabled` 还会按 slot 的 `legacy_hot_cache_copies` 预置旧 ComfyUI 热缓存；`gpu-177-gpu0-image_to_video` 需要把旧 `comfy0` 的 `rife49.pth` 复制到 AIO 内 `ComfyUI_Fill-Nodes` 与 `ComfyUI-Frame-Interpolation` 缓存路径，不能依赖运行时访问 HuggingFace。
