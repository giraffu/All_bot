...
- schema v2 将不可变产物拆为 `control-plane`、`test-execution`、`gpu-execution` 三条环境无关发布链；`release-index.json` 引用三份 manifest，test/prod 只选择模块并注入配置。已有增量基线时，普通自动发布只从策略影响集合中选择 bundle 内 `source_sha` 等于目标 SHA 的运行时 artifact；全部 artifact 均复用旧 `source_sha` 时选择集必须为空，风险 level/matched rules 只作为审计元数据，不得触发空重建。`--services` 仍只能扩大影响集合；显式独立模块包括 `central-api|web-api|payment-api|bot|qqcc-private-bot-worker|paid-group-guard-bot|public-web|dashboard|qqcc-bot|qqcc-config`，一次只能选择一个完整组，并从组内每个 artifact 自己的 `source_sha` 分别计算差异。旧版局部 `current.json` 缺失的 artifact 只从按时间排序的成功 history 在内存恢复，不现场改状态；组内混合版本允许保留各自真实基线。
...
- 唯一代码发布入口是 `scripts/release.py plan|preflight|deploy|deploy-module|rollback|recover`。`deploy-module --module <name>` 刷新并解析最新 `origin/main`，随后只委托一次 `deploy --env prod --modules <name>`；它不会执行 test deploy。目标必须是可从 `origin/main` 到达的完整 40 位 SHA，并使用 CI 生成的 `release.json`、Web checksum 和 digest-pinned 镜像；云端不 build、不挂载源码、不接收代码/env rsync。`preflight` 与 `deploy` 不自动拉 bundle，所有材料必须预先可读。
...
- 正式 standard 晋级在对应 track 的 retained history 中按 artifact 名称和精确 digest 查找 main-channel tested 证据，不再由单个全局 current/目标 SHA 阻塞，也不得为了正式发布自动重新部署测试环境；缺证据只报告 blocker。空 artifact/service 选择或全部目标 digest 已在线时，preflight/deploy 必须拒绝 mutation 和状态推进。direct/emergency 跳过测试晋级但仍必须 `--execute --confirm-prod`。`--dashboard-fast-track` 仅作兼容别名，新增操作使用通用 `--strategy`/`--skip-gate`/风险审计字段。
...
4. 先审阅 plan/preflight 的 `risk_class/strategy/validation_mode/skipped_gates/gates`。常规单模块正式更新优先使用 `scripts/release.py deploy-module --module <name> --execute --confirm-prod`；支持的名称为 `central-api|web-api|payment-api|bot|qqcc-private-bot-worker|paid-group-guard-bot|public-web|dashboard|qqcc-bot|qqcc-config`，不得在一次独立事务中混选。standard 只读取 history 中同 artifact digest 的 tested 证据，不重跑测试部署；QQCC Config auto standard，Dashboard auto direct。公共 Web direct 和核心 emergency 必须显式风险接受。确认计划只重建目标服务；若提示 already deployed，按无变化收口，不得伪报已更新。
