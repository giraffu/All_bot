...
- `release.py` 在同一 CLI 门面内自动输出并选择 `execution_profile=streamlined|strict`。普通 schema-v2 main control-plane、bundle `validation.mode=full/tests=passed`、已知影响且目标配置投影无漂移时走 streamlined；migration、Compose/env/共享发布契约、数据库/Redis、首次切换、未知影响、GPU/test-execution 或任一 strict artifact 使整个混合事务进入 strict。streamlined 不做完整 rollback preflight、旧 checkout 准备、旧镜像预拉、非目标快照、全局 queue drain 或固定 observation，只单次 pull/`up --no-deps --wait` 目标服务，并用本机已有旧 ref 做目标回切；回切失败才保留 maintenance 和 recovery transaction。strict 保留原完整备份、Alembic、维护、排空、回滚与恢复能力。
...
- 环境中立门禁对每个可运行 Python service artifact 分别注入 test/prod 哨兵身份并真实导入；`python-runtime-base`、`python-media-runtime-base` 和 `python-worker-base` 不携应用源码，只执行 Config.Env 与文件系统扫描，不得对其执行服务身份启动检查。
