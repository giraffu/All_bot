...
- **Wan22 RunPod 冷拉状态**：2026-06-13 旧 `wan22_aio_video` GHCR 镜像为 28 层、压缩约 6.59 GiB，不包含业务大模型；两次 5090 cloud-test 真实 canary 均卡在 RunPod `Downloading your container...` 冷拉阶段并中止，未进入模型同步或任务执行，Pod 已清理。第一轮启动优化已切到 RunPod 官方 PyTorch/CUDA base `runpod/pytorch:1.0.2-cu1281-torch280-ubuntu2404`，Dockerfile 在 base 不含 ComfyUI 时安装到 `/opt/ComfyUI`，通过 `/opt/allbot-comfyui-dir` 让 bootstrap/entrypoint 启动，并清理 custom node `.git`、pip cache 和临时文件；业务大模型仍只走 `allbot-model-cache` manifest。后续真实 canary 前先用 GitHub Actions 构建新 tag 并做匿名 manifest 检查，再评估是否还需要 disabled warm standby。
