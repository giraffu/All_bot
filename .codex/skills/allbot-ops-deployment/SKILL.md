...
- 唯一代码发布入口是 `scripts/release.py plan|preflight|deploy|deploy-module|rollback|recover`。`deploy-module --module <name>` 默认一次锁定最新受保护 main，可显式传完整 SHA；只接受带 promotion 批准记录的 main bundle 精确 digest，机器依赖集合只能扩大。普通 `preflight/deploy` 不拉材料；快捷入口可在 mutation 前只读拉取 main bundle。云端不 build、不挂载源码、不接收代码/env rsync。
