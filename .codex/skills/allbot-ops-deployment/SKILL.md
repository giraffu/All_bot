...
- 完整 GPU manifest 优先由 `scripts/gpu_profile_release_v2.py --publish-ref` 发布；本机 token 缺少 `write:packages` 时，只能改用受保护的 `.github/workflows/publish-gpu-release-manifest.yml`。workflow 输入必须是 main 可达完整 SHA、完整 manifest base64 与文件 SHA256，并继续执行 exact catalog、artifact 同 SHA、SHA tag 和远端不可覆盖校验；禁止未审查临时 workflow、tag 转写或 shell 参数注入。
- test-train promotion approval 仍必须先由 `test_train_release.py approve-release --execute` 在集成机核对真实测试运行态并生成；若仅 GHCR push 因本机 token 缺少 `write:packages` 失败，使用受保护 `modular-release-v2.yml` 的 approval-only dispatch 原样发布该批准文件。Actions 必须锁定当前 train 头、校验输入 SHA256、candidate OCI descriptor 和逐 artifact 证据，且该 job 不得 build、部署测试或覆盖既有 tag。
- 并发研发使用受保护 `codex/test-train` 的独立 `test-candidate` bundle；candidate bundle 只能由集成 AI 部署 test，不能直接用于 prod。最终候选必须 `freeze`，按真实测试运行 digest 生成 `verified`/`approved-direct` OCI 批准记录；tree-identical main promotion 只复制原 artifact/Public Web 字节，禁止重新 build、重新部署测试或改写 artifact source SHA。
...
- 唯一代码发布入口是 `scripts/release.py plan|preflight|deploy|deploy-module|config-plan|config-apply|rollback|recover`。`deploy-module --module <name>` 支持重复选择固定模块，默认一次锁定最新受保护 main，可显式传完整 SHA；只接受带 promotion 批准记录的 main bundle 精确 digest，机器依赖集合只能扩大。普通 `preflight/deploy` 不拉材料；快捷入口可在 mutation 前只读拉取 main bundle。云端不 build、不挂载源码、不接收代码/env rsync。
- 云控制面环境事实源只在目标机 `/etc/allbot/<env>.env`。代码发布先远端计算逐服务投影 revision；发现漂移必须先 `config-plan`，再由当次明确授权执行 `config-apply`。投影路径为 `/var/lib/allbot/config/<env>/<revision>/<service>.env`、权限 `600`，禁止整份 prod env 注入全部容器。未知键/契约变化与首次切换强制完整维护、env/数据库备份和单 Alembic head。
- `credential-isolation-complete` 前，任何正式 `deploy --execute` 或 `deploy-module --execute` 都必须额外带 `--accept-pending-secret-rotation --reason --approved-by` 并写入状态；这不授权轮换秘密。完成状态只能由独立秘密轮换任务在 test Worker→prod control-plane→prod Worker 验证后写入。
- 完成状态禁止手工写 marker；必须使用 `release.py credential-isolation-complete --evidence <json> --approved-by <name> --confirm-prod --execute`。证据只允许记录检查键名、复用结论、健康布尔值和旧凭据撤销状态，且必须在一小时内生成；命令会在 test/prod 配置根原子记录审计，跨主机写入失败时恢复为 pending。
- `scripts/safe_deploy_cloud_test.sh` 与 `scripts/migrate_local_test_to_cloud_containers.sh` 已固定 fail closed；旧 `.env.cloud.test` / cloud-test compose 只能历史取证，不能作为测试发布或回滚入口。
- test/prod 是否复用 `AGENT_SECRET_TOKEN|API_TOKEN|AUTH_TOKEN` 只用 `scripts/verify_remote_secret_isolation.py` 的远端 HMAC challenge 核对；输出只能含键名和复用结论。实际 Agent Token 切换仍使用既有 GPU operator，禁止由控制面快捷发布器重建 GPU 镜像或批量改 Worker。
...
- 正式 standard 晋级优先使用 main promotion bundle 内逐 artifact exact-digest approval；不得要求把 main 再部署测试。direct artifact 必须在批准记录中明确为 `approved-direct`，不能伪装 tested。每次 execute 仍必须 `--confirm-prod`。
...
- test/prod 共用镜像；环境身份、DB/Redis、Token、对象存储、bucket、域名、Bot username 和开关只允许来自宿主 env/overlay/Web runtime config。candidate CI 必须执行 `validate_release_environment_neutral.py` 检查 build context、Dockerfile、image Config.Env 和 Public Web dist，且不得输出秘密值。
...
4. 先审阅 plan/preflight 的 `risk_class/strategy/validation_mode/skipped_gates/gates`。快捷入口使用可重复的 `deploy-module --module central-api|web-api|payment-api|dashboard|main-bot|qqcc-bot|qqcc-config|private-bot-worker|paid-group-bot|public-web`；standard 只晋级批准记录中的同 digest 证据，Dashboard/管理面 rolling，生成入口统一进入生成维护。所有正式执行仍带 `--execute --confirm-prod`，并确认非目标容器 ID/digest/config revision/启动时间不变。
