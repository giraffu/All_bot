...
- 正式 immutable Payment 必须固定 `PAYMENT_API_PORT=8021` 与 host/container `8021:8021`，并由公共 cloud compose 的 `/pay/result` healthcheck 进入 `docker compose up --wait` 门禁；禁止把容器 `running` 当作支付入口健康，也禁止恢复历史 `8002` 映射。
