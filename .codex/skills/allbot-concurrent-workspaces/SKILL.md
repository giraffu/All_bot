...
- 同一 candidate 的 control-plane 与 test-execution journal/staged state 及云端非敏感 `release.env` 必须按 track 分目录；云端合约路径为 `/var/lib/allbot/releases/<track>/<sha>/release.env`。Worker 首次切换不得覆盖已提交的控制面 journal/合约，也不得重跑 Postgres/Redis 控制面 cutover。发现升级前无 track 的失败 journal 时，只能通过带精确 `--track` 的 `recover` 兼容收口，禁止手工删 journal 或维护标记。
- 当 control-plane 的可信计划为 `level=none` 且无 artifacts/services 时，`deploy` 只记录 `ready-for-acceptance` / `non-runtime`；Dashboard/QQCC 管理面候选记录 `deployment_mode=test-not-required`，不得修改共享测试站。未显式启用的 Worker 记录 `deferred_tracks=["test-execution"]`，不得写容器或 Worker 已更新。
