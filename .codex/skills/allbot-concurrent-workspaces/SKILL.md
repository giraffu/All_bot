...
- 同一 candidate 的 control-plane 与 test-execution journal/staged state 必须按 track 分目录；Worker 首次切换不得覆盖已提交的控制面 journal，也不得重跑 Postgres/Redis 控制面 cutover。发现升级前无 track 的失败 journal 时，只能通过带精确 `--track` 的 `recover` 兼容收口，禁止手工删 journal 或维护标记。
