...
- LTX 高级图生视频 FSM 支持三种模式：单首帧、首尾帧、视频配音。单首帧沿用 `ltx_video`；首尾帧收集 `image_path` 与 `end_image_path` 并提交 `ltx_mode=flf2v`；视频配音收集 `video_path` 与文本提示并提交 `ltx_mode=v2v_audio`，视频上限 40MB。结果消息存在 `extra_outputs.last_frame` 预期时展示“扩展生成”按钮，callback 前缀为 `ltx_extend`；点击后必须校验历史归属、下载尾帧到 FSM 临时目录，并作为下一段 LTX 起始帧进入设置确认。退出、超时或提交结束必须清理 start/end/video 临时文件。
