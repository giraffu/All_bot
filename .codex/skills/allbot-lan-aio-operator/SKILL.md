...
- GPU/Comfy 已失联、标准 queue-idle 门禁无法执行时，只允许对 Central 明确为 `quarantined|error` 且 `current_task_id/current_task_type` 均为空的单 slot 使用 `isolate-quarantined`。该动作写无 TTL 的 disabled control、只停止目标容器并验证 stopped，再把物理槽记为 intentionally-empty；状态不满足时拒绝，不能借此强杀运行中任务或跳过普通 canary stop。
