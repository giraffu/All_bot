...
- GPU/Comfy 已失联、标准 queue-idle 门禁无法执行时，只允许对 Central 明确为 `quarantined|error` 且 `current_task_id/current_task_type` 均为空的单 slot 使用 `isolate-quarantined`。该动作写无 TTL 的 disabled control，先把目标容器 restart policy 固定为 `no` 并复核，再停止目标容器、验证 stopped 并把物理槽记为 intentionally-empty；即使 GPU reset-required 导致 stop 失败，已验证的 `restart=no` 仍保证下次主机启动不会自动拉起该容器。状态不满足时拒绝，不能借此强杀运行中任务或跳过普通 canary stop。
