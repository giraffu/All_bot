...
- `i2i_pro` profile 同时承接 `i2i_pro`、执行面 `t2i-pornmaster-turbo` 和 `face_swap_v2`，通过 workflow override 绑定对应 API workflow。旧 `face_swap` V1 不进入该 profile，继续使用旧 workflow/worker；候选 profile 是否已部署必须以 Central 实时心跳复核。
