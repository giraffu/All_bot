...
- QQCC `AI视频` 配置保存 `{path,strength}`，提交边界转换为 LTX 既有 `{name,strength}`；强度限制 `0.1..2.0`、步长 `0.05`。空白 `negative_prompt` 必须在 Web/Bot/API 边界省略，不能覆盖工作流节点的内置默认文本；非空值只有在独立发布并验收对应 Worker mapping 后才可宣称生效，单独发布 QQCC 控制面不得为此触碰正式 LTX GPU runtime。主 Bot Telegram 高级 LTX 设置页仍不暴露负面提示词。
