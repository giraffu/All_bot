...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - LAN AIO 生产支持矩阵以 `ops/gpu_pool_controller/config/lan_aio_prod_slots.yml` 为事实源：2026-06-18 `gpu-177` 已整机 `prod_enabled`，GPU0 `image_to_video`（兼容 `video_insert` / `video_edit` alias）和 GPU1 `ltx_video` 均由 AIO 承载；`gpu-002` slot0 AIO 正式承载 SCAIL-2 的 `scail2_action_transfer` / `scail2_video_replacement` / `scail2_face_swap_v2`；`gpu-252-gpu1-wan22_video_v2` 已替换 `cloud_prod_worker_05`，AIO 只声明 `SUPPORTED_TASK_TYPES=wan22_video_v2`，不承接普通 `image_to_video` 或 `video_edit`，旧 `comfy1` / `cloud-prod-comfy-agent-5` stopped rollback；`gpu-252-gpu0-img2img_lora` 仍是 canary-ready 候选，启用前必须逐 slot 验收 workflow、模型 manifest、桶和 task type 路由。
