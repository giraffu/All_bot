...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - SCAIL-2 业务 task type 为 `scail2_action_transfer`（动作迁移，绑定 `SCAIL-2_Animation_multi-char.api.json`，`replacement_mode=false`）和 `scail2_video_replacement`（视频换人，绑定 `SCAIL-2_Replacement.api.json`，`replacement_mode=true`）。Bot 入口位于“视频生视频”二级菜单，按“视频换人 / 动作迁移 / 视频换脸”展示；SCAIL-2 两个入口只收参考图、驱动视频和正向提示词，负面词使用 `SCAIL2_DEFAULT_NEGATIVE_PROMPT`，驱动视频上限 40MB。四个 Nomadoor UI workflow 仍保存在 `workers/comfy_agent/workflows/` 与 `remote_workers/comfy_agent/workflows/` 供人工编辑/加载，业务执行只提交派生 API workflow；新增/重导后必须同步 `.api.json`、`mappings.json`、`workflow_task_patchers.py`、`src/workflow_mapping_validation.py` 与 `remote_workers/`。SCAIL-2 固定 512x896，只开放 5s/8s，成本分别为 40/80 灵石；worker patcher 强制 `VHS_LoadVideo.force_rate=16`、`skip_first_frames=0`，并把 `frame_load_cap` / `WanSCAILToVideo.length` 写为 `81/129`。manifest 前缀为 `scail2/2026-06-17-test`，LoRA 必须落在 `loras/Wan2.1/Wan21_I2V_14B_lightx2v_cfg_step_distill_lora_rank64.safetensors`，因为 workflow 的 `LoraLoaderModelOnly` 枚举引用带 `Wan2.1/` 子目录；`SCAIL2ColoredMask` 是 ComfyUI core 节点，镜像入口 `remote_workers/docker/runpod_profiles/scail2/Dockerfile` 必须使用包含 ComfyUI PR `Comfy-Org/ComfyUI#14373` 后的版本，并 baked VideoHelperSuite、KJNodes、rgthree、Frame-Interpolation、Fill-Nodes、ffmpeg、sshd/bootstrap 和 `remote_workers/requirements.txt` 运行依赖，但不得 baked 任何 SCAIL-2 模型权重。
  - `SCAIL-2_FaceSwap_v2.api.json` 是从 `SCAIL-2_Replacement.api.json` 派生的实验视频换脸 v2 workflow 资产，已同步 `workers/` 与 `remote_workers/`，但尚未绑定任何业务 task type。它复用 SCAIL-2 主模型、LoRA、采样、VAE、`WanSCAILToVideo` 与输出节点，只把 `SAM3_VideoTrack` 条件词从 `human` 收窄为 `face`、`max_objects=1`，并保留 `replacement_mode=true`。若要上线为正式入口，不能只覆盖旧 `face_video`，必须同步新增/复用 task type、`mappings.json`、`workflow_task_patchers.py`、`src/workflow_mapping_validation.py`、`remote_workers/src/workflow_mapping_validation.py` 与 Bot/Web 入口文案和计费。
  - `SCAIL-2_FaceSwap_v2_audio.api.json`、`SCAIL-2_Replacement_audio.api.json`、`SCAIL-2_Animation_multi-char_audio.api.json` 是未绑定入口的候选音频版本，仅在输出 `VHS_VideoCombine` 节点接入驱动视频 `VHS_LoadVideo` 的 audio 输出 `["113", 2]`，并保持 `trim_to_audio=false`。不要覆盖当前线上使用的 `SCAIL-2_Replacement.api.json` 或 `SCAIL-2_Animation_multi-char.api.json`。云测试 `cloud_worker_test_08` 可通过 `CLOUD_TEST_WORKER_08_TASK_TYPE_WORKFLOW_OVERRIDES` 把 replacement/action 指向 `_audio` 候选；`SCAIL-2_FaceSwap_v2_audio.api.json` 只能临时覆盖 `scail2_video_replacement` 做专项 smoke，不能直接绑定 legacy `face_video`。
  - SCAIL-2 运行环境分四类：云测试 LAN runtime 是 gpu-002 GPU0/`8190` 的 `allbot-lan-aio-gpu-002-gpu0-scail2-test`，由 `scripts/lan_scail2_aio_test.sh` 启停，容器不设置 `AGENT_ID` / `CENTRAL_API_URL` / `SUPPORTED_TASK_TYPES`，接单层是 `cloud_worker_test_08`；云测试 RunPod profile `scail2` 使用 `ghcr.io/giraffu/allbot-comfy-runpod-scail2:<tag>`，模型从 R2 `allbot-model-cache/scail2/2026-06-17-test/manifest.json` 同步，canary 临时停用同环境非 RunPod SCAIL-2 worker 后串行跑两个 5s Web 任务；云正式 LAN runtime 使用 `scripts/lan_scail2_aio_prod.sh` 在 gpu-002 slot0/`8190` 启动 `allbot-lan-aio-gpu-002-gpu0-scail2-prod` 与 agent `lan_aio_prod_gpu002_gpu0_scail2_01`，这是当前正式主接单路径；云正式手动 RunPod profile `scail2` 使用 agent `runpod_prod_scail2_manual_NN` 与 Pod `allbot-runpod-prod-scail2-manual-NN`，声明 `SUPPORTED_TASK_TYPES=scail2_action_transfer,scail2_video_replacement`，模型同样来自 `allbot-model-cache/scail2/2026-06-17-test/manifest.json`，用户输入/结果必须写 `user-data-prod`。正式 RunPod SCAIL-2 是手动备用/临时扩容，不是默认常驻容量；没有 heartbeat 或已删除的 `manual_NN` 不算可用 worker，正式 LAN 与正式 RunPod 可以在验收后并行 enabled 接单，但不得混用 `cloud_worker_test_08`、测试桶或 RunPod cloud-test profile。
  - LAN AIO 生产支持矩阵以 `ops/gpu_pool_controller/config/lan_aio_prod_slots.yml` 为事实源：2026-06-18 `gpu-177` 已整机 `prod_enabled`，GPU0 `image_to_video`（兼容 `video_insert` / `video_edit` alias）和 GPU1 `ltx_video` 均由 AIO 承载；`gpu-002` slot0 AIO 正式承载 SCAIL-2 的 `scail2_action_transfer` / `scail2_video_replacement`；`gpu-252-gpu1-wan22_video_v2` 已替换 `cloud_prod_worker_05`，AIO 只声明 `SUPPORTED_TASK_TYPES=wan22_video_v2`，不承接普通 `image_to_video` 或 `video_edit`，旧 `comfy1` / `cloud-prod-comfy-agent-5` stopped rollback；`gpu-252-gpu0-img2img_lora` 仍是 canary-ready 候选，启用前必须逐 slot 验收 workflow、模型 manifest、桶和 task type 路由。
