...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - SCAIL-2 运行环境分三类：云测试 LAN runtime 是 gpu-002 GPU0/`8190` 的 `allbot-lan-aio-gpu-002-gpu0-scail2-test`，由 `scripts/lan_scail2_aio_test.sh` 启停，容器不设置 `AGENT_ID` / `CENTRAL_API_URL` / `SUPPORTED_TASK_TYPES`，接单层是 `cloud_worker_test_08`；云测试 RunPod profile `scail2` 使用 `ghcr.io/giraffu/allbot-comfy-runpod-scail2:<tag>`，模型从 R2 `allbot-model-cache/scail2/2026-06-17-test/manifest.json` 同步，canary 临时停用同环境非 RunPod SCAIL-2 worker 后串行跑两个 5s Web 任务；云正式低影响发布使用 `scripts/lan_scail2_aio_prod.sh` 在 gpu-002 slot0/`8190` 启动 `allbot-lan-aio-gpu-002-gpu0-scail2-prod` 与 agent `lan_aio_prod_gpu002_gpu0_scail2_01`，必须写正式 Central 与 `user-data-prod`，不得混用 `cloud_worker_test_08`、测试桶或 RunPod cloud-test profile。
