...
- `ltx_t2v` / `ltx_t2v_ic` 使用独立 `ltx_t2v` profile，不得覆盖 `ltx_video` / 10Eros I2V。固定栈只能是官方 dev FP8 -> distilled 0.5 -> Sulphur 1.0，IC 再追加 Ingredients 1.0；禁止用户 LoRA。`character_reference_build` 复用 PornMaster FP8 profile，六个临时视图由 worker 确定性拼为私有参考表且不可投稿。两层 feature flag 默认关闭，当前只允许 LAN disabled-candidate 验收；未完成完整 Sulphur T2V 与 Sulphur+Ingredients A/B 实机验证前，不得宣称运行时可用。
