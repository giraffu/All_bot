...
- 图片换脸的业务任务类型仍分 V1 `face_swap` 与 V2 `face_swap_v2`。通用 V1 Worker 继续使用 `face_swap.json`，`i2i_pro` 继续只声明 `face_swap_v2`；专属诊断 profile `face_swap` 是明确例外，可同时声明两种任务类型，但必须通过 per-task overrides 将二者都执行为 `face_swap_v2.json`。不得用该例外修改 remote02 的旧 workflow、模型或环境。
