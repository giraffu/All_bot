...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - LAN AIO 生产支持矩阵以 `ops/gpu_pool_controller/config/lan_aio_prod_slots.yml` 为事实源：`gpu-177` 已整机 `prod_enabled`，GPU0 保留历史 `lan_aio_prod_gpu177_gpu0_image_to_video_01` / `allbot-lan-aio-gpu-177-gpu0-image_to_video-prod` 命名但渲染为 `wan22_video_v2`，并通过 slot-level `target_task_types` 只声明 `wan22_video_v2`；GPU1 `ltx_video` 由 AIO 承载。2026-06-20 旧 `cloud_prod_worker_02/03`、`cloud-prod-comfy-agent-2/3`、`comfy0/comfy1` 与 `/data/comfy` 已退役删除，gpu-177 不再有本地旧链路回滚。`gpu-002` slot0 AIO 正式承载 SCAIL-2 的 `scail2_action_transfer` / `scail2_video_replacement` / `scail2_face_swap_v2`，slot1 仍可承接 legacy `image_to_video`；`gpu-252-gpu0-img2img_lora` 已 `prod_enabled` 承接 `img2img/img2img_lora`；`gpu-252-gpu1-wan22_video_v2` 当前无本地 GPU1 可用，AIO control 必须保持 `disabled`，旧 `comfy1` / `cloud-prod-comfy-agent-5` 仍是 stopped rollback。
...
- **LTX AIO 镜像**：
  - LAN/RunPod 化最小镜像入口为 `remote_workers/docker/runpod_profiles/ltx_video/Dockerfile`，当前 LAN registry tag 为 `192.168.1.115:5000/allbot/comfy-runpod-ltx-video:20260618-ltx-min-cu128-sageattn1`；正式 RunPod GHCR 构建入口为 `.github/workflows/runpod_ltx_video_profile_image.yml`，发布 ref 必须是 `ghcr.io/giraffu/allbot-comfy-runpod-ltx-video:<tag>`。
  - LTX Dockerfile 的 `NODE_SOURCE_IMAGE` 默认使用可公网拉取的 Wan22 GHCR 基础镜像，不依赖局域网 registry；需要覆写时通过 GitHub workflow input 或 `scripts/build_runpod_profile_image.sh --node-source-image ...` 显式传入。
  ...
  - 10Eros v1.2 canary 必须复制为新 workflow 文件（`LTX 2.3 10Eros v1.2 I2V 6.1.json`、`LTX 2.3 10Eros v1.2 FLF2V 6.1.json`、`LTX 2.3 10Eros v1.2 V2V Audio 6.1.json`），并通过目标测试 worker 的 `TASK_TYPE_WORKFLOW_OVERRIDES` 或正式 RunPod `RUNPOD_TASK_TYPE_WORKFLOW_OVERRIDES_LTX_VIDEO` 覆盖；不要覆盖默认 `LTX 2.3 *.json`。主模型文件名为 `LTX 2.3/10Eros_v1.2_fp8mixed_learned.safetensors`；云端 R2 `allbot-model-cache/ltx_video/2026-06-10/manifest.json` 当前只保留 10Eros v1.2，旧 v1 不再作为正式 RunPod 回退。
  - 正式 RunPod `ltx_video` profile 只通过镜像 + R2 model manifest + workflow override 使用 v1.2，不把模型打入正在运行的 LAN LTX AIO 容器，也不改变 LAN AIO 的旧 workflow 默认入口。
- **红线**：
