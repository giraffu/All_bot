...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - SCAIL-2 当前只作为 LAN 手工测试 runtime，不新增业务 task type，不写 `mappings.json`，不接 Central worker。四个 Nomadoor UI workflow 保存在 `workers/comfy_agent/workflows/` 与 `remote_workers/comfy_agent/workflows/`：`SCAIL-2_Animation.json`、`SCAIL-2_Replacement.json`、`SCAIL-2_Animation_multi-char.json`、`SCAIL-2_Animation_WAN-Context-Windows.json`；自动 smoke 由 `scripts/scail2_submit_smoke.py` 在测试容器上临时把 UI workflow 转 API prompt。LAN manifest 为 `allbot-model-cache/scail2/2026-06-17-test/manifest.json`，LoRA 必须落在 `loras/Wan2.1/Wan21_I2V_14B_lightx2v_cfg_step_distill_lora_rank64.safetensors`，因为 workflow 的 `LoraLoaderModelOnly` 枚举引用带 `Wan2.1/` 子目录；`SCAIL2ColoredMask` 是 ComfyUI core 节点，镜像入口 `remote_workers/docker/runpod_profiles/scail2/Dockerfile` 必须使用包含 ComfyUI PR `Comfy-Org/ComfyUI#14373` 后的版本，并 baked VideoHelperSuite、KJNodes、rgthree、Frame-Interpolation、Fill-Nodes，但不得 baked 任何 SCAIL-2 模型权重。
