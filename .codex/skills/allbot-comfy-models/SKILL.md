...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - SCAIL-2 当前只接云测试 Web 与测试 Bot，不进云正式。业务 task type 为 `scail2_action_transfer`（动作迁移，绑定 `SCAIL-2_Animation_multi-char.api.json`，`replacement_mode=false`）和 `scail2_video_replacement`（视频换人，绑定 `SCAIL-2_Replacement.api.json`，`replacement_mode=true`）。测试 Bot 入口位于“视频生视频”二级菜单，按“视频换人 / 动作迁移 / 视频换脸”展示；SCAIL-2 两个入口只收参考图、驱动视频和正向提示词，负面词使用 `SCAIL2_DEFAULT_NEGATIVE_PROMPT`，驱动视频上限 40MB。对应 LAN AIO runtime 是 gpu-002 GPU0/`8190` 的 `allbot-lan-aio-gpu-002-gpu0-scail2-test`，由 `scripts/lan_scail2_aio_test.sh` 启停；该容器只提供 ComfyUI runtime、model sync、Nomadoor UI workflow、派生 API workflow 和样例素材，不设置 `AGENT_ID` / `CENTRAL_API_URL` / `SUPPORTED_TASK_TYPES`。接单层是 `cloud_worker_test_08`，指向 `http://192.168.1.2:8190` 并声明 `SUPPORTED_TASK_TYPES=scail2_action_transfer,scail2_video_replacement`。RunPod 云测试另有 `scail2` profile：镜像 `ghcr.io/giraffu/allbot-comfy-runpod-scail2:<tag>` 由 `.github/workflows/runpod_scail2_profile_image.yml` 构建，启动时从 R2 `allbot-model-cache/scail2/2026-06-17-test/manifest.json` 同步模型，canary 会临时停用 `cloud_worker_test_08` 等非 RunPod SCAIL-2 worker 后串行跑两个 5s Web 任务。四个 Nomadoor UI workflow 仍保存在 `workers/comfy_agent/workflows/` 与 `remote_workers/comfy_agent/workflows/` 供人工编辑/加载，业务执行只提交派生 API workflow；新增/重导后必须同步 `.api.json`、`mappings.json`、`workflow_task_patchers.py`、`src/workflow_mapping_validation.py` 与 `remote_workers/`。SCAIL-2 固定 512x896，只开放 5s/8s，成本分别为 40/80 灵石；worker patcher 强制 `VHS_LoadVideo.force_rate=16`、`skip_first_frames=0`，并把 `frame_load_cap` / `WanSCAILToVideo.length` 写为 `81/129`。manifest 前缀为 `scail2/2026-06-17-test`，LoRA 必须落在 `loras/Wan2.1/Wan21_I2V_14B_lightx2v_cfg_step_distill_lora_rank64.safetensors`，因为 workflow 的 `LoraLoaderModelOnly` 枚举引用带 `Wan2.1/` 子目录；`SCAIL2ColoredMask` 是 ComfyUI core 节点，镜像入口 `remote_workers/docker/runpod_profiles/scail2/Dockerfile` 必须使用包含 ComfyUI PR `Comfy-Org/ComfyUI#14373` 后的版本，并 baked VideoHelperSuite、KJNodes、rgthree、Frame-Interpolation、Fill-Nodes、ffmpeg、sshd/bootstrap，但不得 baked 任何 SCAIL-2 模型权重。
