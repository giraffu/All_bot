...
- **实施状态**：**通常无需修改**，但需注意**硬编码防爆红线**。
  ...
  - RunPod `i2i_pro` 只是现有 `i2i_pro` 任务的 cloud-test runtime profile，不新增业务 task type，正式 `prod-worker --profile i2i_pro` 仍未开放。镜像入口为 `remote_workers/docker/runpod_profiles/i2i_pro/Dockerfile`，默认 `yanwk/comfyui-boot:cu130-slim`，ComfyUI pin 到 `16cd8d8a8f5f16ce7e5f929fdba9f783990254ea`，当前 workflow `workers/comfy_agent/workflows/i2i_pro.json` 只依赖 ComfyUI/core `nodes` 与 `comfy_extras` 的 Flux2/edit 节点（`UNETLoader`、`CLIPLoader`、`VAELoader`、`ReferenceLatent`、`EmptyFlux2LatentImage`、`Flux2Scheduler`、`SamplerCustomAdvanced`）。模型不 baked 入镜像，专用 manifest 为 `allbot-model-cache/i2i_pro/2026-06-14-test/manifest.json`，六个模型文件总计约 `36.11 GiB`；首次 cloud-test canary 使用 `RUNPOD_CONTAINER_DISK_GB=120`，模型同步只能写 ComfyUI `models/`。
