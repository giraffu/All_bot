...
- **云测试/R2 直连 CORS**：`.env.cloud.test` 可将 `MINIO_*` 兼容变量全部指向 R2 S3 endpoint 与 `user-data-test` 桶；当前云测试应保持 `MINIO_PUBLIC_URL=`，并设置 `R2_PUBLIC_DOMAIN=https://r2-test.aivison.it.com`。Web owner 视频 `/result` 依赖 R2 公网 URL，`R2_PUBLIC_DOMAIN` 缺失会停在 99% / `pending_result`；若公开域名临时返回 403，图片可走短签 fallback，视频必须优先修复公开域名或实现受控 fallback。旧测试 MinIO 对象镜像到 R2 桶根路径，不能额外加旧桶名前缀，否则历史 `output_file` 无法命中。Web 参考图/视频上传会由浏览器直传 R2，R2 桶 CORS 必须允许当前 Web Origin：`https://web-test.aivison.it.com`、`https://web.aivison.it.com`、Cloudflare Pages canary `https://web-cf-test.aivison.it.com`/`https://allbot-web-cf-test.pages.dev`，以及正式 Pages 默认域 `https://allbot-web-prod.pages.dev`；方法 `GET/PUT/HEAD`、`AllowedHeaders=["*"]`、`ExposeHeaders=["ETag"]`。否则前端会报 `Network error during upload`。
