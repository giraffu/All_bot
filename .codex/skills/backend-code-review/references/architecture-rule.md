...
- **Core Isolation Principle**: Code under `src/core/` must strictly avoid importing platform-specific objects like Telegram `Update` or FastAPI `Request`. Core logic must operate on primitive types or internal models (e.g., `internal_user_id`).
- Suggested fix: Extract shared contracts into core/domain or service-level modules and make upper layers depend on lower, not the reverse. Parse platform objects at the Handler/Router layer before calling the Core layer.

### Avoid manual transaction rollbacks and compensation in routers
- Category: best practices
- Severity: high
- Description: In FastAPI routers or Telegram handlers, avoid manually calling compensation methods (like `refund_credits`) along with `session.rollback()` inside a `try-except` block. If the session is injected and managed by an outer Unit of Work (UoW) or middleware, manual rollbacks can prevent compensation records (like refund logs) from being persisted, leading to duplicate refund vulnerabilities or data inconsistency.
- Suggested fix: Rely on the outer UoW to handle rollbacks. If business compensation is needed, ensure it is executed in a separate transaction or use Saga patterns, and let the main transaction fail cleanly.
- Example:
  - Bad:
    ```python
...
### Decouple i18n from Backend DTOs (Presentation Layer Isolation)
- Category: architecture
- Severity: high
- Description: Backend DTOs (Data Transfer Objects) and facade layers (like `user_facade.py`) must never format strings with hardcoded languages or emojis for the Web UI. The backend should only return raw numerical progress, states, and boolean flags (e.g., `{"type": "invite", "done": true}`).
- Suggested fix: Shift all text formatting, localized strings, and presentation logic to the frontend (e.g., Vue frontend using `vue-i18n`). The backend API remains purely data-driven and language-agnostic.
- Example:
  - Bad:
    ```python
