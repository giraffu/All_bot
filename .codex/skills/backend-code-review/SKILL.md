...
description: Review backend code for quality, security, maintainability, and best practices based on established checklist rules. Use when the user requests a review, analysis, or improvement of backend files (e.g., `.py`) under the `src/` or `backend/` directory. Do NOT use for frontend files (e.g., `.tsx`, `.ts`, `.js`). Supports pending-change review, code snippets review, and file-focused review.
---
...
Use this skill whenever the user asks to **review, analyze, or improve** backend code (e.g., `.py`) under the `src/` or `backend/` directory. Supports the following review modes:
...
- **File-focused review**: when the user points to specific files and asks for a review of those files (one file or a small, explicit set of files, e.g., `src/...`, `backend/app/...`).
...
- The scope is not under `src/` or `backend/` (unless the user explicitly asks to review backend-related changes outside these directories).
