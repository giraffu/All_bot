...
主菜单业务入口只能有 `快速换脸`、`AI绘图V1`、`AI绘图V2`、`AI滤镜`、`AI动图V1` 和 `AI动图V2`。V1 两个入口默认关闭，只有 `main_buttons.ai_draw_v1` / `video_edit_v1` 被显式开启后才可见；旧 `ai_draw` / `video_edit` 开关只兼容 V2，不得隐式打开 V1。`快速换脸` 是 QQCC 对现有单图随机换脸流程的专用主菜单入口，显示文案走 `qqcc.menu.quick_faceswap`，不接入主 Bot 双图 `faceswap_fsm`。旧 `AI绘图` / `AI动图` 文本和 callback 继续路由 V2。不要直接改共享 `menu.video_edit` 文案来实现 QQCC 菜单改名，以免破坏旧按钮兼容和 QQCC 动图路由。旧 `快速脱衣` 主菜单和 `懒人P图` 主菜单已退出；用户点击旧 reply keyboard / 旧 P 图子按钮时必须回复 `功能暂未开放` 并拒绝提交。主菜单可额外有 `修仙市集`、`前往主bot` 和仅官方 QQCC 展示的 `私有bot` 非生成入口；私有 Application 必须隐藏 `私有bot`。官方入口还受 `main_buttons.private_bot` 独立开关控制，该开关不停止 private worker、不禁用既有私有 Bot，也不取代 `PRIVATE_QQCC_BOT_ENABLED` 总 gate；关闭后旧 reply keyboard 必须回复 `功能暂未开放`，正在等待 token 的申请也必须先删除 token 消息再拒绝创建。Telegram reply keyboard 不能直接承载 URL，因此点击 `前往主bot` 后由 QQCC Bot 回复主 Bot 的 inline URL 跳转按钮。主业务 Bot 的旧 `修仙市集` 底部入口已改为 `懒人bot` 跳转，正式跳转目标由 main-bot 专属 `MAIN_BOT_LAZY_BOT_USERNAME`（或可选 URL）配置；旧 `QQCC_LAZY_BOT_*` 只作整组兼容回退。QQCC 自己的 `修仙市集` 仍是专用轻量 Gallery。

场景配置 v2 持久化 `video_scenes_v1`、`video_scenes_v2`、`draw_scenes_v1`、`draw_scenes_v2`，旧 `video_scenes` / `draw_scenes` 只是 V2 兼容投影。正式迁移完整复制旧场景至 V1 并重写同版本引用与媒体对象：动图 V1 固定 `image_to_video`、动图 V2 固定 `wan22_video_v2`、绘图 V1 固定 `free_edit`、绘图 V2 固定 `free_edit_v2_5`。菜单、callback、素材接收、确认与最终提交的准入必须始终读取对应版本的 `ai_draw_v1|v2` / `video_edit_v1|v2`，不得直接读取旧 `ai_draw` / `video_edit`；旧键只负责配置归一化时兼容回填 V2。尾帧、跳转和后处理引用也必须携带并保持版本，不得跨版本解析场景。未来私有 Bot 只复制创建当时官方四组配置与租户隔离媒体快照。

QQCC `AI动图` 场景由独立 QQCC 配置 Web 的 `video_scenes` 动态配置。默认配置会一次性种子化旧五个懒人动图预设（传教士、后入、口交、脱衣吐舌、近景口交），种子化后它们和自定义场景结构一致，可编辑、删除、调整按钮名、提示词、负面提示词、固定时长、底层模型和尾帧来源。`image_to_video` 与 `wan22_video_v2` 都可按顺序配置最多 5 个附加模型和独立强度；切换 engine 不得清空选择。`end_frame_draw_scene_id` 只能引用归一化后的 `draw_scenes[].id`，空字符串保持单首帧旧行为；运行时若该绘图场景配置了后处理链，视频尾帧必须使用完整绘图链的最终图。`video_scenes` 与 `ai_video_scenes` 还可选填 `jump_draw_scene_id`；它只能引用有效绘图场景，Bot 会在该动图/视频场景的示范与上传提示下显示跳转按钮，点击直接进入目标 `qdraw_scene:` 单图上传流程。目标失效、删除或 AI绘图主入口关闭时必须隐藏按钮，不能影响原视频上传流程。绘图场景删除或引用非法时必须清空。二级场景菜单必须挂在 Bot 回复消息下方，用 inline button 展示，每行最多 3 个；新场景按钮 callback 前缀为 `qvid_scene:`，由 `get_quick_video_fsm_handler()` 直接承接并进入发送图片步骤。旧 `qvid_mode:<menu.video_edit_*>` callback 仅作已发消息兼容，若对应场景已删除必须回复 `功能暂未开放` 并拒绝提交。
...
三类场景都支持 `demo_input_media` / `demo_output_media`。AI绘图、AI滤镜的输入和输出都只能是 JPEG/PNG，点击场景后先以双图片 media group 展示；AI动图输入只能是 JPEG/PNG、输出只能是 MP4，点击后先以图片+视频 media group 展示。示范媒体必须先于上传素材的文字提示发送，单边缺失时允许只发送已有示范，不得阻断后续 FSM。媒体存入 R2 确定性 key `qqcc/demo/<scene_kind>/<scene_id>/<input|output>`，替换时覆盖写；配置保存 `content_sha256` 作为缓存版本。Telegram 发送成功后按 Bot ID 把 `file_id` 写入 `telegram_file_ids`，后续优先秒发缓存；缓存失效时先回退 R2 短签并刷新，若 Telegram 拒绝该 URL，Bot 必须在同一大小与文件签名校验内从 R2 读取并直接上传媒体，然后缓存新的 `file_id`，内容哈希变化时不得继承旧 file_id。

QQCC Config Web 每个场景操作区支持从已上传的 input 示范直接生成 output 示范。提交使用 `POST /api/qqcc/demo-generation/{scene_kind}`，前端再轮询 `GET /api/qqcc/demo-generation/{scene_kind}/{scene_id}/{generation_id}`；提交响应后 Config Backend 必须独立启动完成监视，即使浏览器关闭或停止轮询也继续检查终态。提交入口读取场景快照时必须使用独立短会话并在注册长时监视前关闭，禁止把 FastAPI request-scoped DB session 留到 BackgroundTasks 完成后才释放；完成监视只有在终态写回时才短暂打开自己的数据库会话，否则三个并发视频监视即可耗尽 Config Backend 的小连接池并让配置 GET 返回 500、生成 POST 返回 503。生成服务直接复用场景 engine、prompt、negative prompt、LoRA 和动图 duration 向 Central 提交，不进入用户 task facade，因此不扣灵石、不占用户并发、不写 History。`qqcc-config-backend` 的最小服务投影必须包含与 Central 一致的 `API_TOKEN`，缺失时配置阶段 fail closed，不能等到生成请求以 401 失败；该服务不需要 Worker 控制使用的 `AGENT_SECRET_TOKEN`。终态产物写入场景目录下带 generation ID 的唯一 output key，并在行锁内只更新目标场景的 `demo_output_media`；完成监视负责主自动回写，浏览器轮询终态负责幂等兜底，前端只有收到 `config_saved=true` 才能提示自动保存成功，不再要求管理员手动点击“保存”。监视对 Central/数据库瞬时异常必须在 24 小时窗口内重试，非法 generation namespace 或已删除场景 fail closed。配置 Web 的图片示范沿用 Ant Image 点击放大；视频示范缩略图点击或键盘 Enter/Space 打开带播放、全屏控制的大弹窗。私有 Bot 使用 owner 对应的 `qqcc/private/<id>/demo` namespace，禁止跨租户输入对象。
...
- `global_enabled`
- `main_buttons`: 除旧兼容 key 外包含 `ai_draw_v1`, `ai_draw_v2`, `video_edit_v1`, `video_edit_v2`；V1 默认 `false`、V2 默认 `true`。旧 `ai_draw` / `video_edit` 只回填缺失的 V2 开关；显式版本开关优先。`private_bot` 只控制官方 QQCC 的申请/管理入口，默认开启且不跟随 `global_enabled`
- `main_menu_layout`: `{ buttons_per_row, button_order }`；`buttons_per_row=null` 时保留旧固定分行的行数与每行按钮数量，但仍按 `button_order` 排序；只接受整数 `1..4` 启用统一分行。版本化绘图/动图 key 独立排序；旧 `ai_draw` / `video_edit` 排序 key 只归一为 V2。运行时必须先按开关/gate/场景/官方与私有上下文过滤，再按顺序分行；隐藏项不占位但保留配置位置。官方 checkpoint 与每个私有 Bot JSON 独立保存，不新增表
- `photo_buttons`: `masturbation`, `random_faceswap`；仅保留旧配置兼容
- `undress_methods`: `legacy`, `i2i_draw`；仅保留旧配置兼容
- `video_scenes`: `[{ id, name, prompt, negative_prompt, duration, engine, aspect_ratio, lora_items, lora_name, lora_strength, end_frame_draw_scene_id, jump_draw_scene_id, next_scene_id }]`；`ai_video_scenes` 同样支持 `jump_draw_scene_id` 与 `next_scene_id`。`jump_draw_scene_id` 只允许引用有效 `draw_scenes[].id`，无效值归一移除。`next_scene_id` 只能指向同数组场景，空值终止；配置 PUT 拒绝缺失目标、自环和任意间接循环，旧失效引用读取时清空。运行时迭代快照完整链，无业务深度上限；每段用上一段 `extra_outputs.last_frame` 作为首帧，最终按第一段画布居中裁剪并拼接。`aspect_ratio` 只允许 `source / 9:16 / 16:9 / 1:1`，缺失、空值或非法值归一为 `source`，无需提高 preset version；`lora_items` 是最多 5 个有序 `{name,strength}`。候选只展示 `wan22_explicit_lora_library/2026-07-18` 已下载并成对核验的 49 项，稳定键、中文标签、High/Low 路径和单滑杆保守推荐值来自 `src/wan22_explicit_lora_catalog.py`；强度按 `0.1..2.0`、步长 `0.05` 归一。旧七模型键自动迁移到对应新条目，保存继续镜像首项用于滚动兼容。
...
官方 QQCC 服务用对应 token polling；private worker 只用同一个环境对应 token 做统一频道会员检查，绝不能由此启动第二个 polling。用户私有 token 仍逐租户从数据库解密。官方入口使用 `qqcc_bot.polling_liveness` 中独立于 asyncio 事件循环的线程 watchdog：成功完成的 `getUpdates` 轮询负责 heartbeat，拉取到的 update 只有在全部 handler group 完成后才确认；任一信号连续 180 秒停滞时进程以专用退出码终止，由目标容器的 `restart: always` 只重启 QQCC Bot。官方 QQCC callback 共享路由必须有 45 秒总预算；场景示范媒体与后续上传提示、视频提交状态等非关键 Telegram I/O 必须分别有 15 秒总预算，超时后继续推进 FSM 或创建后台提交，不能等待到 watchdog 才恢复。quick image/video 与修仙市集一键应用的用户图片接收，其 `get_file`、下载与 FSM 临时文件落盘都必须纳入同一 15 秒 QQCC 交互预算；取消或异常时删除可能已写入的半文件并保持当前上传状态，让用户可以立即重试。市集一键应用下载成功后，可能运行数分钟的生成/监视链必须通过 `create_background_task(...)` 脱离当前 Telegram update，后台失败继续清理临时文件并提示用户；不得在 media handler 内同步等待任务终态。该预算只作用 `bot:qqcc` / `bot:qqcc-private:*` 上下文，主 Bot 保持原交互语义。空闲时成功返回的空轮询会刷新 heartbeat，不能因没有用户消息误重启；私有 webhook Application 不启用 polling watchdog。
...
主业务 Bot 的 `懒人bot` 菜单入口缺省显示；`MAIN_BOT_LAZY_BOT_ENABLED=false` 时隐藏新菜单并让旧文本/旧 callback fail closed。入口可用时优先读取 `MAIN_BOT_LAZY_BOT_URL`，未配置时可用 `MAIN_BOT_LAZY_BOT_USERNAME` 自动生成 `https://t.me/<username>`；正式配置固定使用 `MAIN_BOT_LAZY_BOT_ENABLED=true` 与 `MAIN_BOT_LAZY_BOT_USERNAME=@QQCC666_bot`，解析结果必须是 `https://t.me/QQCC666_bot`。新命名空间任一键存在即整组优先；只有完全不存在时才回退旧 `QQCC_LAZY_BOT_*`。两组都没有目标时保留菜单但只提示“懒人bot入口暂未配置”，不得在代码中硬编码地址。主 Bot 的 `图片换脸` 二级菜单只保留 `快速换脸` 与 `随机换脸`；旧 `快速脱衣`、`快速自慰`、旧 `menu.video_edit_*`、旧 `AI绘图` / `AI滤镜` / `AI动图` / `快速换脸` 文本和主 Bot 上的 `qvid_*` callback 必须回复 QQCC 懒人 Bot inline URL 跳转或入口未配置提示，且不得提交任务。QQCC 的 `qdraw_scene:*`、`qfilter_scene:*`、`qvid_scene:*` 和旧 `qvid_mode:*` 兼容不受影响。

正式启动或重建前必须有用户明确要求进入 QQCC 正式发布。日常先运行 `release.py promote --modules qqcc-bot` 查看无 mutation 预览，确认后只增加一次 `--confirm-prod`；用户同时明确决定跳过 Bot 默认生成维护时，operator 使用 `--no-maintenance` 记录该决定并直接 rolling，不再追加确认。该选项不能豁免未审阅 migration、首次切换、共享 Compose/env、blocker、未知契约或异常 polling 门禁，失败补偿仍可启用维护。若 QQCC artifact 的 Git 基线跨过仅属于客服/Web 等其它模块的 migration，只有 `deploy/release-policy.yml` 在 `qqcc-bot`/`qqcc-config` 下逐文件固定当前内容 SHA256 时，才可由高级 `deploy --policy <clean-main-policy> --no-maintenance` 记为 `reviewed-non-target-migration`；此路径不备份数据库、不执行 Alembic，只替换 QQCC 目标容器，任何新增文件或字节漂移重新阻断。发布器从 Bot 自己的正式 digest/source/config revision 计算变化与回滚，只选择目标服务，并在启动窗口内核对唯一目标容器、已知 legacy 实例停止和 Telegram polling conflict。目标配置缺失或漂移会在 pull/up 前阻断，发布器不得自动暂存；只有用户另行要求配置收敛时才用 `config-plan/config-apply --module qqcc-bot`。
