...
- QQCC 自己生成的快速换脸、AI绘图和 AI动图结果不得投稿或公开；新结果不展示 `submit_gallery_*` / `public_share_request` 按钮，旧结果按钮也必须在 QQCC callback 入口拒绝。结果完成文案必须显示 `快速换脸` 或选中的 QQCC 绘图/动图场景名，结果按钮必须展示 `重新生成` 并能从本人历史重建同一功能提交。
