...
- `video_scenes`: `[{ id, name, prompt, negative_prompt, duration, engine, lora_items, lora_name, lora_strength, end_frame_draw_scene_id }]`；`lora_items` 是最多 5 个有序 `{name,strength}`，名称来自 `QQCC_VIDEO_LORA_MODELS`，强度按 `0.1..2.0`、步长 `0.05` 归一；推荐默认值来自 `src/qqcc_video_lora_catalog.py`。旧 `lora_name/lora_strength` 自动迁移为单元素列表，保存继续镜像首项用于滚动兼容。两个 engine 均保留并透传该列表；其余 prompt、duration、id 与尾帧引用规则不变。
