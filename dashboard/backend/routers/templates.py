        ...
        for row in result:
            ...
            if contribution.is_reviewed:
                ...
                preview_url = storage.get_presigned_url(
                    obj_name, bucket=MINIO_TEMPLATE_BUCKET
                )
            else:
...
async def approve_contribution(
    ...
    try:
        stmt = select(TemplateContribution).where(
            TemplateContribution.id == contribution_id
        )
        result = await db.execute(stmt)
...
async def delete_contribution(contribution_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if contribution.is_reviewed:
            obj_name = (
                f"video_nice/{filename}"
                if contribution.file_type == "video"
                else f"quick_face/{filename}"
            )
        else:
            obj_name = f"temps/{filename}"
        ...
        await db.execute(
            delete(TemplateContribution).where(
                TemplateContribution.id == contribution_id
            )
        )
        await db.commit()
