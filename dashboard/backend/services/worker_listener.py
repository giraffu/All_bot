...
async def start_worker_listener():
    ...
    try:
        ...
        logger.error(f"!!! [WORKER_LISTENER] Worker listener crashed: {e}", exc_info=True)
...
async def process_message(message, r_worker, r_bot):
    try:
        ...
        if status in ["done", "error"]:
            ...
            if not task_info:
                logger.warning(f"Task {task_id} completed/failed but no details found in Redis.")
            ...
            logger.info(f"Saving log: worker_id={worker_id}, task_id={task_id}, type={task_type}")
            
            # Save to database
            try:
                async with AsyncSessionLocal() as session:
                    # Check if already exists to prevent duplicate logs from multiple gunicorn workers
                    existing = await session.execute(select(WorkerLog).where(WorkerLog.task_id == task_id))
                    if not existing.scalars().first():
                        ...
                        try:
                            ...
                            logger.info(f"Recorded worker log for task {task_id} by {worker_id} ({final_status})")
                        except Exception as e:
                            # Might be IntegrityError if multiple workers try to insert simultaneously
                            pass
            except Exception as inner_e:
                logger.error(f"!!! [WORKER_LISTENER] Inner error: {inner_e}", exc_info=True)
                
    except json.JSONDecodeError:
        print(f"!!! [WORKER_LISTENER] Failed to parse event data: {message.get('data')}")
    except Exception as e:
