...
class ImageService:
    async def submit_task(self, prompt: str, image_paths: list[str], negative_prompt: str) -> str:
        """Submit image generation task"""
        return await submit_img2img(prompt, image_paths, negative_prompt)

    async def submit_face_swap_task(self, face_image_path: str, body_image_path: str) -> str:
        """Submit face swap task"""
        return await submit_face_swap(face_image_path, body_image_path)

    async def submit_perfect_video_edit(self, prompt: str, image_path: str, width: int = 512, height: int = 512, length: int = 81) -> str:
        """Submit perfect video edit task"""
        return await submit_perfect_video_edit(prompt, image_path, width=width, height=height, length=length)

    async def submit_perfect_video_insert_task(self, prompt: str, image_path: str, width: int = 512, height: int = 512, length: int = 81) -> str:
        """Submit perfect video insert task"""
        return await submit_perfect_video_insert(prompt, image_path, width=width, height=height, length=length)
