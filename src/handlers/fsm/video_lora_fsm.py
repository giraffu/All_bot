...
async def receive_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not image_path:
        logger.warning(f"user={user_id} image_path missing before submit in video_lora")
    ...
    if not image_path:
        logger.warning(
            f"user={user_id} image_path consumed by another request before submit in video_lora"
        )
    ...
    await robust_reply_text(
        message, f"🚀 正在提交附加模型视频任务，预计消耗 {cost} 灵石，请耐心等待..."
    )
