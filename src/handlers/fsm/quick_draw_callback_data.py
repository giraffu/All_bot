...
QUICK_DRAW_V1_SCENE_CALLBACK_PREFIX = "qdrawv1_scene:"
QUICK_DRAW_SCENE_CALLBACK_PATTERN = r"^(?:qdraw_scene|qdrawv1_scene):[A-Za-z0-9_-]{1,32}$"
...
def build_quick_draw_v1_scene_callback_data(scene_id: str) -> str:
    return f"{QUICK_DRAW_V1_SCENE_CALLBACK_PREFIX}{scene_id}"
...
def parse_quick_draw_v1_scene_callback_data(data: str | None) -> str | None:
    if not data or not data.startswith(QUICK_DRAW_V1_SCENE_CALLBACK_PREFIX):
        return None
    scene_id = data[len(QUICK_DRAW_V1_SCENE_CALLBACK_PREFIX) :]
