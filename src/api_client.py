...
class APIClient:
    ...
    @async_retry(max_retries=3)
    async def submit_txt2img(self, prompt: str, priority: int = 0) -> str:
        """
        Submit txt2img via the legacy central-api t2i endpoint.
        The backend task id is generated by the central API itself.
        """
        data = {"prompt": prompt}
        ...
        r = await self._request(
            ...
            TXT2IMG_ENDPOINT,
            json=data,
            params={"async": "true", "priority": priority},
        )
        return r.json()["task_id"]
