...
def test_api_client_no_normalization():
    ...
    async def run_test():
        ...
        mock_resp_done.json.return_value = {"status": "done", "progress": 100}
        mock_resp_done.raise_for_status = MagicMock()

        with patch(
            "src.api_client.api_client._request", new_callable=AsyncMock
        ) as mock_request:
...
def test_task_service_queue_logic_new():
    ...
    async def run_test():
        ...
        with patch("src.services.task_service.image_service"):
            with patch(
                "src.services.task_service.robust_edit_text", new_callable=AsyncMock
            ) as mock_edit:
                ...
                assert any("第 1 位" in c for c in call_args), (
                    f"Should display '第 1 位' for queue_pos=0. Got: {call_args}"
                )

                # Check 2: queue_remaining=5 -> display "第 5 位"
                assert any("第 5 位" in c for c in call_args), (
                    f"Should display '第 5 位' for queue_remaining=5. Got: {call_args}"
                )
