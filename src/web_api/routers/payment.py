...
async def create_order(
    ...
    plan_res = await db.execute(
        select(MembershipPlan).where(MembershipPlan.id == req.plan_id)
    )
    ...
    if pay_result and pay_result.get("code") == 1:
        ...
        return {
            "code": 0,
            "message": "success",
            "data": {"order_id": out_trade_no, "pay_url": pay_url},
        }
    else:
        raise HTTPException(
            status_code=500,
...
async def get_order_status(
    ...
    order_res = await db.execute(
        select(Order).where(
            Order.order_id == order_id, Order.telegram_id == current_user.id
        )
    )
    ...
    return {"code": 0, "message": "success", "data": {"status": order.status}}
