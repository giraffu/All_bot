...
async def get_presigned_upload_url(
    ...
    if not filename or not content_type:
        raise HTTPException(
            status_code=400, detail="Filename and content_type are required"
        )
    ...
    object_key = (
        f"web_uploads/{current_user.id}/{date_str}_{unique_id}.{ext}"
        if ext
        else f"web_uploads/{current_user.id}/{date_str}_{unique_id}"
    )
    ...
    try:
        ...
        logger.error(
            f"Error generating presigned URL for user {current_user.id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail="Internal server error generating URL"
        )
