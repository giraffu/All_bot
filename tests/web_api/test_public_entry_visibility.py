...
async def test_public_entry_visibility_returns_only_safe_flags_without_cache():
    ...
    assert payload["task_price_overrides"] == {}
    assert payload["task_pricing"]["schema_version"] == 2
    assert payload["task_pricing"]["prices"] == {}
    assert "blowjob" not in str(payload["task_pricing"]["variants"])
