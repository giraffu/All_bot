...
- SCAIL-2 真实 task type 为 `scail2_action_transfer`（动作迁移）与 `scail2_video_replacement`（视频换人）。Web payload 使用 `inputs.images=[参考图, 驱动视频]`、`prompt`、`negative_prompt`、`duration`；Bot 入口在“视频生视频”二级菜单下，按“视频换人 / 动作迁移”收集参考图、驱动视频和正向提示词，负面词使用默认值，驱动视频上限 40MB。dispatcher 会转换为 Central simple route 的 `image`、`video`、`prompt`、`negative_prompt`、`length`，并把 SCAIL-2 duration / negative prompt 写入提交元数据，Web 成功落库时会进入 `extra_outputs.scail2_context` 供模板应用回填。SCAIL-2 Web/Bot 成功结果可投稿；Web 一键应用只复用原 motion/driving video，复用者重新上传 reference image，衍生任务提交 `is_template=true` 与 `source_post_id` 后必须保持 `allow_contribute=false`。业务 workflow 必须是 API format：`scail2_action_transfer -> SCAIL-2_Animation_multi-char.api.json`，`scail2_video_replacement -> SCAIL-2_Replacement.api.json`；Nomadoor UI JSON 仅供人工编辑/加载。worker patcher 固定 512x896、`force_rate=16`、`skip_first_frames=0`，5s/8s 分别写 `frame_load_cap` 与 `WanSCAILToVideo.length` 为 `81/129`，并分别强制 `replacement_mode=false/true`。云测试执行 runtime 可以是 gpu-002 LAN AIO SCAIL-2 容器 `http://192.168.1.2:8190` + 本地主 `cloud_worker_test_08`，也可以是 cloud-test RunPod `scail2` profile，测试输入/结果写 `user-data-test`。云正式执行 runtime 可以是 gpu-002 slot0 的 `lan_aio_prod_gpu002_gpu0_scail2_01`，也可以是手动正式 RunPod `runpod_prod_scail2_manual_NN`；正式用户输入/结果只能写 `user-data-prod`，模型权重只从 `allbot-model-cache/scail2/2026-06-17-test/manifest.json` 同步。正式发布前用 `scripts/cloud_prod_generation_release_gate.py` 冻结新生成并退款剩余 pending；LAN slot0 用 `scripts/lan_scail2_aio_prod.sh` 启动/验收/回滚，RunPod 用 `scripts/runpod_prod_ops.sh add|canary|enable --profile scail2` 创建、验收和放开。
