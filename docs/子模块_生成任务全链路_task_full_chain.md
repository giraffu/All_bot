...
- LTX 高级图生视频 worker 必须同时声明 `SUPPORTED_TASK_TYPES=ltx_video,ltx_video_flf2v,ltx_video_v2v_audio`，否则 Web/Bot 选择首尾帧或视频配音模式后会只在 Central pending 队列中等待匹配 worker。
