...
尾帧提取与最终拼接依赖控制面运行镜像内的 `ffmpeg`、`ffprobe`，不依赖 Worker workflow。`qqcc-bot`、`private-bot-worker`、`qqcc-config-backend`、`dashboard-backend` 四个真实消费者继承 `python-media-runtime-base`，发布 full-validation 对各最终 digest 执行双工具 smoke。runner 还必须区分 `generation` 与 `tail_frame` 失败阶段：已成功扣费并产出视频、但尾帧提取失败时，不得把该段误报为“生成失败”；成功前缀、失败任务退款和已成功任务计费规则保持不变。
...
- LAN `pornmaster_flux2_edit_bf16` 是首个有界重叠灰度 profile：单图与多图 BF16 共用最多 3 个 claimed、最多 2 个 Comfy inflight（一个真实执行、一个 pending）和 1 个交付槽。该特例通过版本化 `PIPELINE_PROFILE_POLICY=bf16_lan_claim3_comfy2_delivery1` 激活；兼容 worker 将默认 `1/2/1` 解析为 `2/3/1`，旧 worker 忽略策略并保持 `1/2/1`，所以回滚旧镜像也会恢复旧执行深度。其它 LAN profile 默认仍为 1 个 Comfy inflight、1 个 reserved prefetch；RunPod 不启用本次 BF16 LAN 特例。
