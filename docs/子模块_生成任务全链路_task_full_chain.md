...
- SCAIL-2 真实 task type 包括 `scail2_action_transfer`（动作迁移）、`scail2_video_replacement`（视频换人）以及云测试已接入的 `scail2_face_swap_v2`（视频换脸 v2）。Web payload 使用 `inputs.images=[参考图, 驱动视频]`、可选 `prompt`、`negative_prompt`、`duration`；Bot 入口在“视频生视频”二级菜单下，三类 SCAIL-2 任务都收集参考图、驱动视频和可选正向提示词，Bot 可跳过、Web 可留空，空值由 `normalize_scail2_positive_prompt(...)` 按 task type 补默认提示词。负面词使用默认值，驱动视频上限 40MB。dispatcher 会转换为 Central simple route 的 `image`、`video`、`prompt`、`negative_prompt`、`length`，并把 SCAIL-2 duration / negative prompt 写入提交元数据，Web 成功落库时进入 `extra_outputs.scail2_context`。SCAIL-2 Web/Bot 成功结果可投稿；Web 一键应用只复用原 motion/driving video，复用者重新上传 reference image，衍生任务必须保持 `allow_contribute=false`。业务 workflow 必须是 API format；测试 worker8 当前把三类任务覆盖到 audio workflow，其中 `scail2_face_swap_v2 -> SCAIL-2_FaceSwap_v10_firstframe_faceswap_replacement_audio.api.json`。v10 是两阶段测试候选：worker8 先抽驱动视频第一帧，调用 `192.168.1.226:8188` 的 `face_swap_v2.json` 把用户参考脸换到首帧，再把换脸后的首帧作为 SCAIL-2 reference image，并走视频换人式 `human` track / replacement workflow。worker patcher 固定 512x896、`force_rate=16`、`skip_first_frames=0`，5s/8s 分别写 `81/129`；动作迁移强制 `replacement_mode=false`，视频换人与视频换脸 v2 强制 `replacement_mode=true`。云测试执行 runtime 可以是 gpu-002 LAN AIO SCAIL-2 容器 `http://192.168.1.2:8190` + `cloud_worker_test_08`；云正式 SCAIL-2 LAN/RunPod 当前仍只声明动作迁移与视频换人，正式用户输入/结果只能写 `user-data-prod`，FaceSwap v2 正式发布需单独确认。
