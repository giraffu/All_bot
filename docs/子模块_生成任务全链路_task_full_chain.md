...
`reference_audio` 与 `reference_video`；原片最长 40 秒。用户选择开头
3/5/10/15 秒，原片不得短于所选片段；价格随长度递增，旧请求默认 5 秒。直接扩展
仍只把父段尾帧作为 I2V 首帧。
...
私有 active registry 额外保存 `_bot_task_recovery` presentation contract，恢复时还原 `send_result`、用户可见 task type/prompt、输入索引、结果 metadata、完成文案和语言。私有旧记录缺 contract 时 fail closed，隐藏中间输出不得被恢复器当最终结果发送。QQCC 私有多阶段 continuation 使用 Redis checkpoint 持久化原始输入、stage plan、确定性 submission sequence/registry ID、当前输出与状态；active registry 内 `_private_qqcc_continuation` 关联 chain/stage/executor fence。中间阶段先 CAS 推进 checkpoint 再 cleanup；最终阶段先记录 `delivery_pending`，发送成功后再 CAS delivered。checkpoint 不可用时保留 paid registry/用户锁；worker 在 TaskRegistry 为空时仍扫描 ready/delivery checkpoint，租约丢失取消旧 owner，running orphan 在旧锁失效后 rewind。continuation policy 不解析具体 task executor；Quick 与 recovery composition root 注入 generation/video/LTX executor 和 downloader，缺失时在副作用前失败。私有多步链仍使用 exact tenant `client_type`。
...
- Pub/Sub 终态经 backend index 设 due-now；ZSET兜底，旧 Hash 以 bounded
  `HSCAN` + `ZADD NX` 补索引
- Web 成功写 History 以 `user_id + task_id + source` 幂等；仅首次创建记录后刷新用户修为并执行 R2 warmup，使生成次数晋升和权益立即生效。幂等重放跳过两者，`record_history=false` 的内部阶段也跳过。
- 新协议 Web 结果具备 `task-results/{backend_task_id}/...` 一致 object key、完整
  SHA/size/content type 和实际维度时，History 直接引用 durable key，只生成相邻缩略图并写
  `canonical_r2_media_materialization_completed`。旧结果保留下载/探测与 History warmup，
  写 `history_r2_compatibility_warmup_completed` 和 `compat.r2.history_media_prefix`；fallback
  在 Probe→Copy→Switch 和零命中后退出。
