...
调度，但 `freenoise=false`，避免后续窗口复用前段噪声导致动作循环。
...
- SCAIL-2 长动作迁移的 Context Windows 节点必须保持 `freenoise=false`；`workflow_task_patchers.py` 会对 `scail2_action_transfer_long` 再次写入该值，避免 workflow 重导出后把 FreeNoise 打开。
