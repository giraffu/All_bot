...
- `prompt_optimize` 可在终态前通过 Central `text_delta` 与 Web SSE
  `text_snapshot/text_delta` 展示只读预览。该路径使用 backend task ID 保存快照、用
  registry task ID 做用户 owner fence；断线通过快照恢复，重复 sequence 不重复追加。
  只有最终完整 JSON 校验和 `/complete` 能写成功结果，部分输出后的 failure 仍按根
  registry ID 幂等退款，并以 `partial_unvalidated` 返回只读失败片段。
