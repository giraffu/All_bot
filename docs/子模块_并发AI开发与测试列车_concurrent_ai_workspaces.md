...
→ 用户需要时以 streamlined/strict 自动分类部署云测试
...
`streamlined` 只适用于已知 schema-v2 main control-plane 影响且目标服务配置投影无漂移；测试和正式均只替换 planner 选中的服务，失败使用目标主机已有旧 ref 回切。任一 migration、Compose/env、首次切换、未知影响或专用执行轨会使整个混合发布进入 `strict`，继续使用完整门禁。该分类不改变 A-H handoff、release batch、main CI 构建或生产确认边界，也不授权槽位直接部署共享环境。
