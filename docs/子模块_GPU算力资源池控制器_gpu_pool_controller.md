...
拆分 manifest 入口为 `python scripts/gpu_pool_controller.py runpod split-video-manifests --env-file .env.cloud.test`；真实上传两个小 manifest 时追加 `--execute`，脚本会读取 `wan22_aio_video/2026-06-12-test/manifest.json`、按 profile 选择文件、对所有复用的原始 R2 `key` 做 HEAD，只上传 manifest JSON，不复制 safetensors 大对象。split video cloud-test 真机 canary 入口为 `runpod split-video-canary`：执行前必须临时打开 `RUNPOD_DRY_RUN=false`、`RUNPOD_AUTOSCALER_ENABLED=true`、`RUNPOD_MAX_PODS_TOTAL=2`、`RUNPOD_MAX_PODS_PER_TYPE=1` 并传 `--execute`；命令会同时创建 2 个视频 Pod，GPU 调度按 `RTX 5090` 优先、`RTX 4090` 回退，临时禁用支持视频任务的非 RunPod 测试 worker，通过 Web `/api/tasks/generate` 提交 `image_to_video` 无 LoRA、`image_to_video + Insertion`、`wan22_video_v2` 三条 preview/5s 任务，下载 MP4 与 `last_frame` PNG 到 `runpod_video_test_results/`，最后恢复 worker control 并删除两个 Pod。若第二个 Pod 因 RunPod 库存不足创建失败，canary 必须删除已创建的第一个 Pod，再返回结构化失败 summary。
