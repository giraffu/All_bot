...
2026-06-12 已新增 `wan22_aio_video` RunPod cloud-test dry-run profile。该 profile 只服务 `render-create` 和 `runpod canary` 预检，不创建正式 Pod，不接 `prod-worker`。固定渲染口径为 `SUPPORTED_TASK_TYPES=image_to_video,wan22_video_v2`、`POOL_RUNTIME_PROFILE=wan22_aio_video`、`AGENT_ID_PREFIX=runpod_test_wan22_aio_video`、`gpuTypeIds=NVIDIA GeForce RTX 5090,NVIDIA GeForce RTX 4090`（5090 优先，库存不足时允许 4090 回退）、`MINIO_*_BUCKET=user-data-test`、`RUNPOD_MODEL_BUCKET=allbot-model-cache`、`RUNPOD_MODEL_PREFIX=wan22_aio_video/2026-06-12-test`、`RUNPOD_MODEL_MANIFEST_KEY=wan22_aio_video/2026-06-12-test/manifest.json`、`RUNPOD_COMFY_CUSTOM_NODES_ENABLED=false` 和 `RUNPOD_COMFY_KJNODES_ENABLED=false`。2026-06-13 已为 Wan22 cloud-test 配置 RunPod template `77gi0wqo8x`；`RUNPOD_USE_TEMPLATE_WAN22_AIO_VIDEO=true` 时 create 请求使用 `templateId`，`RUNPOD_IMAGE_NAME_WAN22_AIO_VIDEO` 仍保留为镜像来源记录和 direct image 回退参考。dry-run 示例：

```bash
RUNPOD_IMAGE_NAME_WAN22_AIO_VIDEO=ghcr.io/giraffu/allbot-comfy-runpod-wan22-aio-video:20260612-wan22aio-test \
RUNPOD_USE_TEMPLATE_WAN22_AIO_VIDEO=true \
RUNPOD_TEMPLATE_ID_WAN22_AIO_VIDEO=77gi0wqo8x \
...
`wan22_aio_video` canary dry-run 已支持 profile-aware template 校验：direct image 路径要求 public GHCR Wan22 baked image 前缀；template 路径允许 `templateId=77gi0wqo8x` 且不要求 create JSON 同时携带 `imageName`。两条路径都继续校验视频 GPU 列表为 `RTX 5090,RTX 4090`、测试 Central、测试 bucket、模型 manifest 和 RunPod secret reference。未来真实 `--execute` 时，若未显式传 `--worker-id`，只会临时禁用云测试中支持 `image_to_video` 或 `wan22_video_v2` 的非 RunPod worker；任务 case 固定为 `image_to_video` preview/5s 单起始帧和 `wan22_video_v2` preview/5s 单起始帧，均要求 `extract_last_frame=true`。真实 canary 会校验主 MP4 `result_url` 可下载，并强制校验 Web result 的 `extra_outputs.last_frame` 存在且可读；中断 canary 时会返回结构化 cleanup summary，并删除已创建 Pod。首尾帧、扩展生成、旧视频 LoRA、transfer/build Pod 和正式视频 worker 仍是后续阶段，不能从本 dry-run profile 推导为已验证。

2026-06-13 RunPod cloud-test 已从临时 AIO 视频 profile 拆出两个专属视频 profile：`image_to_video` 与 `wan22_video_v2`。两者暂时复用 Wan22 template `77gi0wqo8x` 与同一个 GHCR image，但 create env 分别渲染 `SUPPORTED_TASK_TYPES=image_to_video` / `wan22_video_v2`、`POOL_RUNTIME_PROFILE=image_to_video` / `wan22_video_v2`、`AGENT_ID_PREFIX=runpod_test_image_to_video` / `runpod_test_wan22_video_v2`，并使用独立模型 manifest：`image_to_video/2026-06-13-test/manifest.json` 与 `wan22_video_v2/2026-06-13-test/manifest.json`。`wan22_aio_video` 暂时保留为兼容/回滚 canary profile，不作为新测试主路径。通用 dry-run scale 入口为：
...
| `RUNPOD_IMAGE_NAME_*` / `RUNPOD_USE_TEMPLATE_*` / `RUNPOD_TEMPLATE_ID_*` / `RUNPOD_GPU_TYPE_IDS_*` | 每个 task profile 的镜像、template 开关、template id 和 GPU 限定；不要把 img2img、AIO Wan22 和 split Wan22 混用。 | `img2img_lora` 使用已验证 public GHCR 4090/5090/L40S 口径；`wan22_aio_video`、`image_to_video`、`wan22_video_v2` 为视频口径，GPU 列表是 `NVIDIA GeForce RTX 5090,NVIDIA GeForce RTX 4090`，当前均复用 template `77gi0wqo8x` 和 Wan22 GHCR image | 当前手动正式 worker 固定 public `img2img_lora` GHCR image 和 `NVIDIA GeForce RTX 4090` |
| `RUNPOD_DOCKER_START_SCRIPT_FILE_*` | Controller 读取本地脚本文件内容并渲染为 RunPod `dockerStartCmd`，用于启动前 bootstrap、模型同步入口和 relay/agent 拉起；已创建 Pod 不会热更新。 | `IMG2IMG_LORA`、`WAN22_AIO_VIDEO`、`IMAGE_TO_VIDEO`、`WAN22_VIDEO_V2` 都可指向 `remote_workers/scripts/runpod_bootstrap_from_git.sh`；split 视频 profile 当前复用 Wan22 bootstrap | 正式手动 worker 如需使用必须单独评估，不从 cloud-test 自动继承 |
