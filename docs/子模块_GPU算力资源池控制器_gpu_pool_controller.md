...
| 层级 | 已覆盖/候选能力 | 当前口径 |
| :--- | :--- | :--- |
| LAN AIO 正式接单 | `img2img`、`img2img_lora`、`image_to_video`（兼容 `video_insert` / `video_edit` alias）、`ltx_video`、`scail2_action_transfer`、`scail2_action_transfer_long`、`scail2_video_replacement`、`scail2_face_swap_v2`、`pornmaster_flux2_single_edit`、`pornmaster_flux2_multi_edit` | `gpu-177` GPU0 真实运行态为 `image_to_video`、GPU1 为 LTX；`gpu-177-gpu1-wan22_video_v2` 已因 32GB OOM 进入 `blocked_oom_32gb`，不计入候选容量；`gpu-252` GPU0 正式切给 PornMaster Flux2 edit；SCAIL-2 由 `gpu-002` slot0 正式 AIO 承载；`gpu-002` slot1 当前由 PornMaster Flux2 edit AIO 承载，image_to_video 是同卡回切候选 |
| LAN AIO disabled 候选 | `img2img_lora`、`image_to_video` 回切口径，以及未 blocked 的新增候选 | 候选 slot 不自动接单；Dashboard/CLI takeover 必须指定或推断同服务器当前运行目标，且按 live runtime profile 拒绝同 profile 替换；`maintenance_disabled` / `blocked_*` slot 不允许 takeover |
...
scripts/lan_aio_fleet_prod_ops.py candidate-plan --node-id gpu-252 --profile img2img_lora --replace-slot gpu-252-gpu0-pornmaster_flux2_edit
...
候选配置遵循“Git/YAML 事实源 + 生成/校验”的边界。新增候选时先用 `candidate-plan --node-id ... --profile ... --replace-slot ...` 输出标准 YAML patch、渲染摘要、agent/container/remote_dir/host_port、模型 manifest 和预检命令；该命令不写文件、不执行远端操作。操作者审阅 patch 并通过 git 合入 `lan_aio_prod_slots.yml` 后，Dashboard 才能看到候选。Dashboard 不提供自由写生产 YAML、自由镜像或自由 manifest 的入口。
...
2026-06-18 `gpu-177` 进入整机 LAN AIO 接管：GPU0 由 `lan_aio_prod_gpu177_gpu0_image_to_video_01` 提供 `image_to_video`，GPU1 由 `lan_aio_prod_gpu177_gpu1_ltx_video_01` 提供 `ltx_video`。2026-06-20 已按用户确认退役本地旧链路：旧 `cloud_prod_worker_02/03` control 为 `disabled`，本地主 `cloud-prod-comfy-agent-2/3`、GPU 节点 `comfy0/comfy1`、旧 `/data/comfy` 和旧镜像已删除；gpu-177 不再提供本地旧链路回滚，恢复入口改为 AIO restart/recreate 或外部容量兜底。2026-06-30 已把 GPU0 配置目标重新收敛到 `image_to_video`，避免 Dashboard 把尚未真实运行的 `wan22_video_v2` 误显示为当前能力。2026-07-01 正确目标 `gpu-177-gpu1-ltx_video` 的 Wan22 takeover 曾成功完成，但第一笔真实 `wan22_video_v2` 任务在 RTX 5090 32GB 上 OOM kill ComfyUI（status 137），随后已恢复 LTX；`gpu-177-gpu1-wan22_video_v2` 因此标记为 `blocked_oom_32gb`、`retargetable=false`，不得通过 Dashboard/CLI takeover 启用，直到有更小 profile 或 48GB+ LAN 容量验证。
...
2026-06-28 `gpu-002-gpu1-pornmaster_flux2_edit` 曾通过 fleet 入口替换旧 slot1 `image_to_video` AIO。2026-06-29 曾按单 slot 回切到 image_to_video；同日 23:17 Asia/Shanghai 后又通过 Dashboard LAN AIO takeover 切回 `gpu-002-gpu1-pornmaster_flux2_edit`。当前 `lan_aio_prod_gpu002_gpu1_pornmaster_flux2_edit_01` 在 host `8191` 接 `pornmaster_flux2_single_edit` / `pornmaster_flux2_multi_edit`，`gpu-002-gpu1-image_to_video` 作为同卡回切候选，回切时应 drain/stop PornMaster agent/container 后再启动 `allbot-lan-aio-gpu-002-gpu1-image_to_video-canary` 并补齐 RIFE 热缓存。Dashboard 当前标签只认 live heartbeat / running container；无 live signal 的 `prod_enabled`、`maintenance_disabled`、`candidate`、`blocked_*`、`superseded_*` 都不得被标成 `runtime_current`。不得让两个 8191 容器或两个 GPU1 agent 同时 enabled。

后续优化方向：
- 配置阶段应区分 `prod_enabled`、`canary_ready`、`blocked_host_service_runtime`，避免已正式接管的 slot 仍被误读为 canary。
- `wan22_video_v2` 在 `gpu-252` GPU1 slot 和历史 `gpu-177` GPU1 blocked slot 都通过 slot-level `target_task_types` 收窄为只接 `wan22_video_v2`；后续新增共享镜像 slot 时也应优先显式声明目标 task type，避免 profile 默认 alias 误接单。`gpu-177` GPU0 当前配置和 live runtime 都应按 `image_to_video` 判断；`gpu-177-gpu1-wan22_video_v2` 的 32GB OOM blocked 状态解除前不得作为候选容量。
...
| `LAN AIO 管理` slot 列表 | `GET /api/runpod/lan-aio/slots?include_disabled=true` | 按节点/物理 GPU 汇总 enabled 当前 slot 与 disabled 候选 slot，展示 Central control、remote container、image/manifest 和 `model-cache-marker.json` 摘要；云正式 `DASHBOARD_LAN_AIO_EXECUTION_MODE=ssh` 时状态采集也必须走本地主 runner 读取远端 Docker/model-cache，不在云容器内直连 `allbot-gpu-*`；slot 会同时返回 `configured_profile_id/configured_task_types` 与 `live_runtime_profile/live_task_types/live_image_ref/runtime_drift`，并补充 `live_state`、`target_container_state`、`switch_readiness`、`switch_blockers`、`recover_readiness`、`recover_blockers`、`recover_prefer`、`last_failed_operation_id`、`recovery_status`；UI 的 AIO 类型列以 live runtime 为准；当前态只认 live heartbeat / running container，无 live signal 时不再用配置态兜底为 `runtime_current` |
| `LAN AIO 管理` slot 操作 | `POST /api/runpod/lan-aio/slots/{slot_id}/{action}` | 页面暴露受控串联动作 `takeover` 与空物理 GPU 的 `recover`；候选 slot 可带 `replacement_target_slot_id` 指定同服务器当前运行目标，未显式指定时后端优先推断同物理 GPU 的 live 目标，存在多个 live 目标且无法唯一推断时返回 422；后端按目标 live runtime profile 校验，不同 profile 才允许替换，并拒绝 `maintenance_disabled` / `blocked_*` slot takeover；`failure_policy` 默认 `auto_rollback`，底层 takeover 命令传给 `lan_aio_fleet_prod_ops.py takeover --failure-policy auto_rollback ... --execute`；recover 先按实时状态确认同物理 GPU 无 active runtime，再执行 `recover --slot <slot-id> --physical-slot <node:gpuN> --prefer old|candidate --execute`；后端/API/CLI 仍支持 `preflight`、`pull-image`、`warm-cache`、`drain-legacy`、`wait-idle`、`stop-old`、`start-disabled`、`enable-aio` 单步排障动作 |
...
Dashboard 入口不重写 RunPod provider 逻辑，只异步调用 `scripts/runpod_prod_ops.sh` 或 LAN AIO fleet helper。LAN AIO 管理不提供自由镜像、自由 manifest 或测试环境切换；页面 slot 操作显示 `一键切换` 和在无 active runtime 时才可用的 `恢复此 AIO`。普通候选默认替换同一物理 slot；`retargetable` 候选可以在弹窗里选择同服务器任一当前运行目标，默认选同一 `physical_slot_key` 的 live 当前 slot，目标列表展示节点、GPU、live runtime 与 configured profile，按 live runtime profile 置灰同类型目标，并用目标物理 slot 级 lock 拒绝并发切换/缓存操作，返回 409 让操作者先看正在运行的 operation。`巡检本地服务` 会刷新所有 LAN AIO slot 的 live/container/cache/control 状态；云正式 runner 模式下这一路径也必须由本地主 runner 代查远端状态，避免云容器对 LAN SSH 返回 255 后误显示 `status_unavailable`。当某物理 GPU 无 active slot 且配置内目标 slot 可恢复时，Dashboard 才开放 `恢复此 AIO`，提交前后端会再次校验 `recover_readiness`，并用同一物理 slot lock 防并发。单步 operation 仍保留在后端/API/CLI 排障入口，不作为 Dashboard 日常按钮。Worker 卡片看到 `control_state=disabled|draining` 时显示 `暂停中`，接单控制按钮显示 `开启`；其它状态显示 `暂停`。RunPod Worker 卡片提供 `锁定/解锁`，后端 API 为 `POST /api/runpod/workers/{agent_id}/lock|unlock`、`GET /api/runpod/workers/locks`；锁记录持久化在 operation store 的 `dashboard:runpod:locked_workers`，`/api/system/workers` 会标注 `runpod_locked` 并显示 `已锁定`。锁定后 Dashboard 手动 `DELETE /api/runpod/workers/{agent_id}` 返回 409，autoscaler 缩容与 add 失败/终止 cleanup 的 `down` 都会跳过该 worker，直到再次解锁。数量字段是新增数量；旧前端若仍发送 `desired_count`，后端也按新增数量解释，不会触发 `scale --desired` 或删除既有 slot。当前 Dashboard 手动 RunPod profile 列表包含 `img2img`、`image_to_video`、`wan22_video_v2`、`i2i_pro`、`scail2 / 视频生视频`、`ltx_video / 高级图生视频` 与 `pornmaster_flux2 / 自由P图 v2`；`scail2` 对应 `scail2_action_transfer,scail2_video_replacement` 两类正式任务，`ltx_video` 对应 `ltx_video,ltx_video_flf2v,ltx_video_v2v_audio`，`pornmaster_flux2_edit` 对应 `pornmaster_flux2_single_edit,pornmaster_flux2_multi_edit`。系统监控页的活跃 Worker 详情来自 Dashboard `/api/system/status.runpod_profile_queue_details`，按正式手动 RunPod 7 个 profile 聚合 active/pending、最长 pending 等待和非低信任最长 pending 等待；`pornmaster_flux2_edit` 返回 `autoscaler_enabled=false`，可手动新增/暂停/删除，但不进入 Dashboard RunPod autoscaler 自动 add/down；`i2i_pro` 汇总 `i2i_pro,t2i-pornmaster-turbo,face_swap`，正式 RunPod `scail2` 不统计 `scail2_face_swap_v2`。同一请求里同一 profile 只能出现一次；若同 profile 已有未结束的 `add` operation，Dashboard 后端会返回 409，禁止再次提交，避免并发新增抢到同一个 `manual_NN` slot。后台 operation 默认使用 30 秒间隔、100 次无库存重试，真实执行只打开 `RUNPOD_DRY_RUN=false` 与 `RUNPOD_AUTOSCALER_ENABLED=true`，并把 `RUNPOD_PROD_MAX_MANUAL_SLOTS` 设为 `100` 或请求指定值。运行中的新增 operation 可从最近操作点 `终止`，后端会先向该 operation 的进程组发送 SIGTERM；如果该次 operation 已记录 `runpod_create_pod_NN`，会继续提交对应 slot 的 `down` 清理；若对应 RunPod worker 已锁定则跳过 cleanup 并记录 `skipped_locked`/`partial_locked`。未记录到创建 slot 的终止只停止等待/重试进程，不推测删除其它 Pod。云正式 Dashboard 后端默认优先把容器内 `/app/.env` 同时作为 `--runpod-env-file` 与 `--prod-env-file`；该文件由云正式 `.env.cloud.prod` 挂载，必须包含完整、shell-compatible 的 `RUNPOD_*` 手动池配置和可用 `RUNPOD_API_KEY`。LAN AIO 管理默认同进程执行 helper；云正式若要从 Dashboard 发起 LAN AIO mutation 或状态巡检，必须设置 `DASHBOARD_LAN_AIO_EXECUTION_MODE=ssh`、`DASHBOARD_LAN_AIO_RUNNER_HOST` 和 `DASHBOARD_LAN_AIO_RUNNER_PROJECT_ROOT`，让真实 SSH/Docker 操作落在本地主服务器 runner 上，避免把 LAN GPU SSH key 或 `192.168.1.0/24` 私网路由放进云控制面。正式 compose 给 Dashboard 后端只读挂载 `/app/runtime/lan-aio-runner`，`DASHBOARD_LAN_AIO_RUNNER_SSH_COMMAND` 默认使用该目录的 `id_ed25519`；更换 key 或 runner 用户时只改云正式 env 和本地主 `authorized_keys`，不要改代码。远端 runner 默认读取 `<runner-root>/.env.cloud.prod`、`<runner-root>/.env.lan-aio-prod`、`<runner-root>/.env.lan.model-cache`，可用 `DASHBOARD_LAN_AIO_RUNNER_PROD_ENV_FILE`、`DASHBOARD_LAN_AIO_RUNNER_AIO_ENV_FILE`、`DASHBOARD_LAN_AIO_RUNNER_MODEL_ENV_FILE` 覆盖。若本地主交互环境依赖本地代理访问正式公网域名，云端 runner 还应配置 `DASHBOARD_LAN_AIO_RUNNER_HTTP_PROXY`、`DASHBOARD_LAN_AIO_RUNNER_HTTPS_PROXY`、`DASHBOARD_LAN_AIO_RUNNER_ALL_PROXY` 和 `DASHBOARD_LAN_AIO_RUNNER_NO_PROXY`，后端会在远端命令中同时导出大小写 proxy 变量；`NO_PROXY` 至少覆盖 LAN registry/model-cache 和目标 GPU IP。未配置 runner host 时，后端应返回可读 503，而不是在云机上直接超时。LAN AIO `takeover` 的 preflight 失败会把脱敏 JSON 摘要写入 operation log，方便在 Dashboard 面板直接看到具体失败检查项。不要把本机测试专用 `RUNPOD_PUBLIC_KEY_FILE` 路径带入云正式容器；生产路径默认不依赖 RunPod SSH。必要时仍可通过 `DASHBOARD_RUNPOD_ENV_FILE` / `DASHBOARD_RUNPOD_PROD_ENV_FILE` 覆盖 env 路径；不得在 API 响应、operation 日志或文档中输出任何 env 内容或密钥。
