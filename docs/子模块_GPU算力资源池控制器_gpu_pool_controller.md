...
| `gpu-002-gpu1-image_to_video` | `cloud_prod_worker_07` | `lan_aio_prod_gpu002_gpu1_image_to_video_01` | `image_to_video` | 8191 | `prod_enabled` |
...
scripts/lan_aio_fleet_prod_ops.py takeover --slot gpu-002-gpu1-pornmaster_flux2_edit --include-disabled
...
| `LAN AIO 管理` slot 操作 | `POST /api/runpod/lan-aio/slots/{slot_id}/{action}` | 固定动作 `preflight`、`pull-image`、`warm-cache`、`drain-legacy`、`wait-idle`、`stop-old`、`start-disabled`、`enable-aio` 与受控串联动作 `takeover`，底层命令总是 `lan_aio_fleet_prod_ops.py ... --include-disabled --execute` |
