...
2026-06-12 已新增 `wan22_aio_video` RunPod cloud-test dry-run profile。该 profile 只服务 `render-create` 和 `runpod canary` 预检，不创建正式 Pod，不接 `prod-worker`。固定渲染口径为 `SUPPORTED_TASK_TYPES=image_to_video,wan22_video_v2`、`POOL_RUNTIME_PROFILE=wan22_aio_video`、`AGENT_ID_PREFIX=runpod_test_wan22_aio_video`、`gpuTypeIds=NVIDIA GeForce RTX 5090`、`MINIO_*_BUCKET=user-data-test`、`RUNPOD_MODEL_BUCKET=allbot-model-cache`、`RUNPOD_MODEL_PREFIX=wan22_aio_video/2026-06-12-test`、`RUNPOD_MODEL_MANIFEST_KEY=wan22_aio_video/2026-06-12-test/manifest.json`、`RUNPOD_COMFY_CUSTOM_NODES_ENABLED=false` 和 `RUNPOD_COMFY_KJNODES_ENABLED=false`。2026-06-13 已为 Wan22 cloud-test 配置 RunPod template `77gi0wqo8x`；`RUNPOD_USE_TEMPLATE_WAN22_AIO_VIDEO=true` 时 create 请求使用 `templateId`，`RUNPOD_IMAGE_NAME_WAN22_AIO_VIDEO` 仍保留为镜像来源记录和 direct image 回退参考。dry-run 示例：
...
拆分 manifest 入口为 `python scripts/gpu_pool_controller.py runpod split-video-manifests --env-file .env.cloud.test`；真实上传两个小 manifest 时追加 `--execute`，脚本会读取 `wan22_aio_video/2026-06-12-test/manifest.json`、按 profile 选择文件、对所有复用的原始 R2 `key` 做 HEAD，只上传 manifest JSON，不复制 safetensors 大对象。split video cloud-test 真机 canary 入口为 `runpod split-video-canary`：执行前必须临时打开 `RUNPOD_DRY_RUN=false`、`RUNPOD_AUTOSCALER_ENABLED=true`、`RUNPOD_MAX_PODS_TOTAL=2`、`RUNPOD_MAX_PODS_PER_TYPE=1` 并传 `--execute`；命令会同时创建 2 个 5090 Pod，临时禁用支持视频任务的非 RunPod 测试 worker，通过 Web `/api/tasks/generate` 提交 `image_to_video` 无 LoRA、`image_to_video + Insertion`、`wan22_video_v2` 三条 preview/5s 任务，下载 MP4 与 `last_frame` PNG 到 `runpod_video_test_results/`，最后恢复 worker control 并删除两个 Pod。
