...
SCAIL-2 LAN AIO runtime 已用于 Web/Bot 的视频生视频能力：正式侧包含 `scail2_action_transfer`（动作迁移）和 `scail2_video_replacement`（视频换人），云测试 worker8 额外接入 `scail2_face_swap_v2`（视频换脸 v2）。它有测试 runtime、云测试 RunPod profile、云正式 slot0 runtime 与云正式手动 RunPod profile 四条边界，不能混用测试/正式桶或 worker。
