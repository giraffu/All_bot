...
`pornmaster_flux2_edit` 是 2026-06-27 新增的测试 profile，面向 `pornmaster_flux2_single_edit` 与 `pornmaster_flux2_multi_edit` 两个执行面 task type。它复用 i2i_pro 的 ComfyUI Flux2 core 节点能力，新增 workflow/API 映射和专用 LAN AIO 镜像；默认不接正式流量，cloud-test 可用 `scripts/lan_pornmaster_flux2_edit_aio_test.sh` 在 `gpu-252` GPU0 临时暂停 `gpu-252-gpu0-img2img_lora` 后渲染 canary AIO 到 host `8192`。模型 bundle `pornmaster_flux2_edit_baseline/2026-06-27` 已缓存到本地 registry，需要三个文件：`diffusion_models/flux2/PornMaster_flux2_klein_9b_turbo_fp8_V4.safetensors`、`text_encoders/flux2/qwen_3_8b_fp8mixed.safetensors`、`vae/flux2/full_encoder_small_decoder.safetensors`。其中 Qwen 复用 `i2i_pro_baseline`，VAE 与 PornMaster 9B fp8 UNET 已按 sha256 写入 manifest；真实 token 不得写入文档或 git。`20260628-pornmaster-flux2-edit-cu128-smallvae1` 镜像包含 FLUX.2 small-decoder VAE 兼容补丁，已在 RTX 4090 / 24GB 上完成 fp8 1 图与 2 图 smoke；bf16 只作为 48GB 级显存 canary 候选。
...
| 层级 | 已覆盖/候选能力 | 当前口径 |
| :--- | :--- | :--- |
| LAN AIO 正式接单 | `img2img`、`img2img_lora`、`image_to_video`（兼容 `video_insert` / `video_edit` alias）、`ltx_video`、`scail2_action_transfer`、`scail2_action_transfer_long`、`scail2_video_replacement`、`scail2_face_swap_v2`、`pornmaster_flux2_single_edit`、`pornmaster_flux2_multi_edit` | `gpu-177` GPU0 真实运行态为 `image_to_video`、GPU1 为 LTX，且 GPU1 另有 `wan22_video_v2` 同卡候选；`gpu-252` GPU0 正式切给 PornMaster Flux2 edit；SCAIL-2 由 `gpu-002` slot0 正式 AIO 承载；`gpu-002` slot1 当前由 PornMaster Flux2 edit AIO 承载，image_to_video 是同卡回切候选 |
| LAN AIO disabled 候选 | `wan22_video_v2`、`img2img_lora`、`image_to_video` 回切口径 | 候选 slot 不自动接单；Dashboard/CLI takeover 必须指定同服务器当前运行目标，且按 live runtime profile 拒绝同 profile 替换 |
...
scripts/lan_aio_fleet_prod_ops.py takeover --slot gpu-177-gpu1-wan22_video_v2 --replace-slot gpu-177-gpu0-image_to_video --include-disabled
...
| `LAN AIO 管理` slot 列表 | `GET /api/runpod/lan-aio/slots?include_disabled=true` | 按节点/物理 GPU 汇总 enabled 当前 slot 与 disabled 候选 slot，展示 Central control、remote container、image/manifest 和 `model-cache-marker.json` 摘要；slot 会同时返回 `configured_profile_id/configured_task_types` 与 `live_runtime_profile/live_task_types/live_image_ref/runtime_drift`，并补充 `live_state`、`target_container_state`、`switch_readiness`、`switch_blockers`、`recover_readiness`、`recover_blockers`、`recover_prefer`、`last_failed_operation_id`、`recovery_status`；UI 的 AIO 类型列以 live runtime 为准；当前态只认 live heartbeat / running container，无 live signal 时不再用配置态兜底为 `runtime_current` |
| `LAN AIO 管理` slot 操作 | `POST /api/runpod/lan-aio/slots/{slot_id}/{action}` | 页面暴露受控串联动作 `takeover` 与空物理 GPU 的 `recover`；候选 slot 可带 `replacement_target_slot_id` 指定同服务器当前运行目标，后端按目标 live runtime profile 校验，不同 profile 才允许替换；`failure_policy` 默认 `auto_rollback`，底层 takeover 命令传给 `lan_aio_fleet_prod_ops.py takeover --failure-policy auto_rollback ... --execute`；recover 先按实时状态确认同物理 GPU 无 active runtime，再执行 `recover --slot <slot-id> --physical-slot <node:gpuN> --prefer old|candidate --execute`；后端/API/CLI 仍支持 `preflight`、`pull-image`、`warm-cache`、`drain-legacy`、`wait-idle`、`stop-old`、`start-disabled`、`enable-aio` 单步排障动作 |
