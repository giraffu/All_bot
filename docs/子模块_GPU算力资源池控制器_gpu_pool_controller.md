...
2026-07-12 为 gpu-226 GPU0 新增 cache-only `pornmaster_flux2_edit_bf16` 候选：LAN 镜像仍复用 `20260628-pornmaster-flux2-edit-cu128-smallvae1`，独立 manifest 为 `pornmaster_flux2_edit_bf16/2026-07-12/manifest.json`，主权重是 Civitai V4 turbo BF16（SHA-256 `5085c05fa34b2455245a75f393885780b41e80a7517265b4b53da2e5044b004e`），并复用现有 Qwen fp8 text encoder 与 small-decoder VAE。该候选只声明独立执行任务类型 `pornmaster_flux2_edit_bf16`，复用单图 workflow 参数映射并在 worker patcher 中把 UNet 节点 100 强制切到 BF16 文件；不会承接现有 fp8 的 `pornmaster_flux2_single_edit` / `pornmaster_flux2_multi_edit` 队列。该类型目前仅作为 LAN 执行候选事实，不开放用户入口、RunPod autoscaler、计费或 Gallery；这些能力必须在后续明确启用时另行补齐。镜像和三文件缓存已通过 fleet `pull-image` / `warm-cache` 准备完成，但容器未启动；该 32GB RTX 5090 尚未做显存 canary 或生产启用，不计入当前容量。
...
同日晚间又增加了与 LAN 独立的手动 RunPod profile `pornmaster_flux2_edit_bf16`：仅允许 `NVIDIA GeForce RTX 4090`，只声明同名任务类型，使用 120GB container disk、`--lowvram`、独立 agent/pod 命名空间与 `pornmaster_flux2_edit_bf16/2026-07-12/manifest.json`，且不进入 autoscaler。BF16 UNet 从 Civitai 下载链接在云控制机直接流式写入 R2，Qwen/VAE 从已有 R2 对象做 server-side multipart copy，未经本地上传；三文件总计 `27,071,581,434` bytes，manifest HEAD/size/SHA metadata 校验通过。2026-07-13 实际创建 RunPod `0owo38uayvyjcu`（RTX 4090 24GB，`$0.69/h`），运行分支 `codex/runpod-bf16-4090`；单图 Central 直接 canary 成功，端到端 `23.88s`，结果 PNG `1,005,766` bytes，无 OOM。该 Pod 保持 RUNNING，worker 为 idle/enabled，供 QQCC 自由P图 v3 手动验收；现有 fp8 v2 队列与 LAN `image_to_video` 不受影响。本次 Pod 按正式安全基线未开 SSH，因此只能确认 24GB + `--lowvram` 可完整出图，未采集精确峰值显存曲线。
...
LAN AIO 当前态不再在本文维护静态大表。先读 `lan_aio_fleet_state.yml` 判断每张物理 GPU 的 current/cached/blocked 摘要，再用 `scripts/lan_aio_fleet_prod_ops.py list --include-disabled` 和 `status --include-disabled` 做 live 仲裁；若 state 与 live 冲突，报告 drift 并停止生产 mutation。2026-07-04 operator 校准后，state 已记录 `gpu-226` GPU0 当前为 `image_to_video` LAN AIO，旧 `cloud_prod_worker_01` / `gpu-226:8188` 只保留为手工回滚元数据；`gpu-177` GPU0 当前为 `wan22_video_v2`、GPU1 当前为 `ltx_video`，SCAIL-2 是同卡回切候选；`gpu-252` GPU0 当前为 `i2i_pro` 且通过 `gpu_device_id` 固定健康 UUID，`img2img_lora`、`image_to_video`、PornMaster Flux2 edit 与 SCAIL-2 是同卡回切候选；返修 GPU1 当前为 `gpu-252-gpu1-pornmaster_flux2_edit` 低负载接单，`gpu-252-gpu1-scail2` 虽已完成 cache、disabled heartbeat、direct canary 和短 CUDA smoke，但真实 SCAIL-2 face-swap workload 复现 Xid 119/154，SCAIL-2/Wan22 仍保持 maintenance-disabled，不计入本地视频容量。

2026-06-18 阶段能力口径：

| 层级 | 已覆盖/候选能力 | 当前口径 |
| :--- | :--- | :--- |
| LAN AIO 正式接单 | `img2img`、`img2img_lora`、`image_to_video`（兼容 `video_insert` / `video_edit` alias）、`i2i_pro`、`t2i-pornmaster-turbo`、`face_swap`、`ltx_video`、`scail2_action_transfer`、`scail2_action_transfer_long`、`scail2_video_replacement`、`scail2_face_swap_v2`、`pornmaster_flux2_single_edit`、`pornmaster_flux2_multi_edit` | `gpu-226` GPU0 当前承载 `image_to_video` AIO；`gpu-177` GPU0 真实运行态为 `wan22_video_v2`、GPU1 为 `ltx_video`，SCAIL-2 是同卡回切候选；`gpu-177-gpu1-image_to_video` 与 `gpu-177-gpu1-wan22_video_v2` 已因 32GB OOM 进入 `blocked_oom_32gb`，不计入候选容量；`gpu-252` GPU0 当前承载 `i2i_pro`，通过 UUID 绑定健康卡，`img2img_lora`、`image_to_video`、PornMaster Flux2 edit 与 SCAIL-2 是同卡回切候选；`gpu-252` GPU1 返修卡当前承载 `pornmaster_flux2_edit` 低负载图片编辑，`gpu-252-gpu1-scail2` 为 Xid 隔离中的 maintenance-disabled slot，不计入 SCAIL-2 产能；`gpu-002` slot0 承载 SCAIL-2，slot1 当前由 PornMaster Flux2 edit AIO 承载，image_to_video 是同卡回切候选 |
...
Dashboard 后端的 RunPod autoscaler 只复用上述安全入口，不直接调用 RunPod API。`pornmaster_flux2_edit_bf16 / 自由P图 v3` 只出现在手动 profile 选项，`autoscaler_enabled=false`，不得因 v3 pending 自动 add/down。正式启用时
