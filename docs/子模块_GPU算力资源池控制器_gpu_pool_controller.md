...
8. `start-disabled --slot ... --execute` 启动 AIO 容器，只等待 disabled heartbeat，不允许接单。
...
Dashboard 的 `一键切换` / CLI `takeover --slot ... --include-disabled --execute` 按 `preflight -> pull-image -> warm-cache -> drain-legacy -> wait-idle -> stop-old -> start-disabled -> enable-aio` 串联上述步骤；任何一步失败即停止。若需要首次配置 Docker insecure registry，仍要在维护窗口先单独执行 `configure-registry` 或用离线 `docker load` 预置镜像。
...
| `LAN AIO 管理` slot 操作 | `POST /api/runpod/lan-aio/slots/{slot_id}/{action}` | 页面只暴露受控串联动作 `takeover`；后端/API/CLI 仍支持 `preflight`、`pull-image`、`warm-cache`、`drain-legacy`、`wait-idle`、`stop-old`、`start-disabled`、`enable-aio` 单步排障动作，底层命令总是 `lan_aio_fleet_prod_ops.py ... --include-disabled --execute` |
