...
2026-06-13 随后用同一 workflow 覆盖 `base_image=yanwk/comfyui-boot:cu128-slim` 构建 `ghcr.io/giraffu/allbot-comfy-runpod-wan22-aio-video:20260613-wan22aio-yanwkclean-108c7ea`，匿名 manifest 检查通过，manifest 为 28 层、压缩约 7.01 GiB、最大层约 838 MiB。该镜像 dry-run 通过，但真实 canary Pod `bzjh2cmq6ibfa1` 约 8 分钟仍停留 `runtime=false` / `pod_readiness`，未进入 worker heartbeat 或模型同步，已中断并清理到 `list-pods count=0`。当前 `.env.cloud.test` 中 `RUNPOD_TEMPLATE_ID_IMG2IMG_LORA=x750yt0uln` 但 `RUNPOD_USE_TEMPLATE_IMG2IMG_LORA=false`；已验证的图生图 baked-image canary 实际也是 direct public `imageName` + Controller 渲染 env 路径，非 templateId 路径。RunPod UI 中保留的图生图 template 可作为人工复现/配置固化参考；Wan22 后续也应创建同等 template，但 Controller / canary 需要先支持 template-aware validation，避免把 `templateId` 路径误判为缺少 public `imageName`。
