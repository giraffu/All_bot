...
- Web 的 H3 Pro 能力还必须受 `enable_minimax_h3` 约束；Dashboard“入口控制”的
  ...
  动作迁移、视频换人和视频换脸。别名和历史执行类型先映射到类型族，再应用开关；
  H3 的 Gallery `minimax_h3` 开关仍独立于 Web 练功房 Pro 入口。
...
- Wan22 首尾帧按顺序显示为“起始帧 / 终止帧”。LTX 与高级图生视频pro 单首帧显示为“起始帧”，首尾帧显示为“起始帧 / 终止帧”；H3 REF2V 第 1 张显示为“主图（套用时由用户自选）”，后续按顺序显示为“模板参考图 N”。历史兼容的视频配音记录可显示为“输入视频”，但当前 Web/Bot 不再提供该入口。SCAIL-2 按顺序显示为“参考图 / 驱动视频”。
