...
- v2.5 与 v3 普通结果均允许投稿。一键应用必须锁定原 prompt、要求用户重新上传恰好 1 张原图且隐藏 LoRA；v2.5 模板按 `free_edit_v2_5` 单阶段扣 3 灵石，v3 模板按既有 BF16→原脸恢复链路扣 5 灵石。两类模板派生任务均写 `allow_contribute=false`，禁止递归投稿；QQCC 市集遇到 v2.5 时只显示正确名称并交给 Web 应用，不新增 QQCC 原生生成入口。
