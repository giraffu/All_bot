...
无效尾帧并要求重传。文生视频与角色参考模式仍展示固定画面比例。
高级图生视频pro 的设置摘要与进入提示词状态的消息会按已选附加模型动态展示强度
和提示词指南，并说明训练触发词由 Worker 自动添加；Bot 仍只提交模型 ID，由
`advanced_video_pro_submission_service.py` 和 domain catalog 补默认强度，FSM 不
复制模型路径或触发词注入逻辑。
