...
| `docs/子模块_LTX文生视频与人物一致性_ltx_t2v_characters.md` | LTX T2V/人物一致性 | domain config、人物草稿/四槽位子图表、workflow、profile、`shared/character_reference_sheet.py`、结果物化、RunPod canary | current | LTX 专项；练功房按正脸、全身正面、全身侧面、全身背面四个槽位独立生成私有子图，四图齐全后合成 v3 纯白底单一 character ingredient 面板，旧布局必须重新保存；人物图库位于修仙笔记且支持单图重生；IC 固定官方 ComfyUI 快速模板的 distilled FP8 checkpoint + FP4 Gemma + Ingredients 1.0，禁止复用 Eros/dev+distilled LoRA/Sulphur 底模；按官方契约把人物面板复制成等长静态参考视频，通过 `GetICLoRAParameters` 在 `frame_idx=0` 注入 guide，关闭可见首帧 I2V 条件并由 `LTXVCropGuides` 删除 guide latent；采样使用标准 KSampler 8 steps/CFG 1/euler ancestral/linear quadratic；Worker 将人物描述规范化为 face/turnaround 两个训练子标题并过滤布局句，避免目标 latent 把参考表布局当作场景；test Web/后端和 cloud-test 人工 RunPod 已支持 |
