...
| `docs/子模块_社区与存储_gallery_storage.md` | `src/core/gallery*`、`src/web_api/services/*gallery*`、R2 scripts | 已修正 | R2/legacy MinIO 退出口径有效；补充 LTX 高级图生视频首尾帧 Gallery 标签、apply-context 回填与 `ltx_video_flf2v` alias 口径；补充 `i2i_draw` Web 一键应用禁用字段与 apply-context 400 口径 |
...
| `.codex/skills/allbot-gallery-storage/SKILL.md` | Gallery/R2 code、Skill 体积审计 | 已修正 | 存储与社区边界有效；已补充自由P图 v2 独立 `free_edit_v2_group`、Web 一键应用无 LoRA/重传参考图和 single/multi 提交口径；补充 LTX 首尾帧 tag、apply-context 回填与 `ltx_video_flf2v` alias 口径；补充 `i2i_draw` Web 一键应用关闭、列表禁用字段与 apply-context 400 口径 |
