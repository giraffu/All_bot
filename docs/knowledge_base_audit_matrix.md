...
| `docs/子模块_中控API与节点通信_central_api.md` | `backend/app/main.py`、`backend/app/queue_manager.py`、worker relay | 已修正 | Central / worker protocol 口径有效；同步 SCAIL-2 测试与正式 LAN worker 四任务声明，正式 RunPod scail2 仍两任务 |
| `docs/子模块_任务黄金路径回归清单_task_golden_path.md` | `tests/backend`、`tests/core`、`tests/web_api`、worker tests | 已核对 | 回归分组仍可用 |
| `docs/子模块_GPU算力资源池控制器_gpu_pool_controller.md` | `ops/gpu_pool_controller`、Dashboard RunPod 服务、RunPod scripts、PornMaster Flux2 edit profile/scripts | 已修正 | 已包含 Dashboard autoscaler 预计清空时间模型、profile 级自动管理暂停、RunPod 故障/暂停自愈、bootstrap timeout 换机清理、RunPod/LAN AIO 当前边界，并记录 `pornmaster_flux2_edit` 镜像、cached 模型 manifest、gpu252:8192 正式 AIO slot、gpu002:8191 已于 2026-06-29 回切 `image_to_video`、旧 img2img_lora / gpu002 PornMaster rollback、ignored token env 与 fp8/bf16 运行口径；补充 Dashboard `LAN AIO 管理` profiles/slots/action API、`warm-cache` cache marker、物理 slot operation lock、无自由镜像/manifest/一键切换边界；补充正式 LAN SCAIL-2 可承接 `scail2_action_transfer_long` 且正式 RunPod 不承接 |
...
| `docs/子模块_局域网GPU节点资源与运维_lan_gpu_resource_ops.md` | LAN AIO scripts、GPU pool config、worker compose、PornMaster Flux2 edit profile/image | 已修正 | 长运行态文档保留，容量需按实时探测复核；PornMaster Flux2 edit 当前正式 slot 为 `gpu-252-gpu0-pornmaster_flux2_edit` 使用 gpu252:8192；gpu002:8191 已于 2026-06-29 回切 `image_to_video`，gpu002 PornMaster AIO 停止并固定 disabled；补充 Dashboard LAN AIO 管理仅展示配置内当前/候选 slot、`warm-cache` marker 和分步 operation，不做自由导入或一键切换；SCAIL-2 正式 LAN slot0 改为四任务含 `scail2_action_transfer_long` |
