...
| `docs/子模块_局域网GPU节点资源与运维_lan_gpu_resource_ops.md` | LAN AIO scripts、GPU pool config、worker compose、PornMaster Flux2 edit profile/image | 已修正 | 长运行态文档保留，容量需按实时探测复核；PornMaster Flux2 edit 当前正式 slot 为 `gpu-252-gpu0-pornmaster_flux2_edit` 使用 gpu252:8192，且 gpu002:8191 在 2026-06-29 23:17 Asia/Shanghai 后再次由 `gpu-002-gpu1-pornmaster_flux2_edit` 接管；`gpu-002-gpu1-image_to_video` 为同卡回切候选；补充 Dashboard LAN AIO 管理以 runtime heartbeat / running container 优先判定当前 slot、当前 slot `一键切换` 禁用、页面只暴露受控 `takeover`，单步 operation 保留为后端/API/CLI 排障入口，不做自由导入或跨 slot 批量切换；SCAIL-2 正式 LAN slot0 改为四任务含 `scail2_action_transfer_long` |
