...
| `docs/子模块_LTX文生视频与人物一致性_ltx_t2v_characters.md` | LTX T2V/人物一致性 | domain config、人物草稿/子图表、workflow、profile、`shared/character_reference_sheet.py`、结果物化、RunPod canary | current | LTX 专项；练功房按槽位独立生成最多六个私有子图，每张 3 灵石，至少两张后合成单一 character ingredient 面板，旧 3×2 等权表按已有子图惰性迁移；人物图库位于修仙笔记且支持单图重生；IC 按官方 Ingredients 契约把人物面板复制成等长静态参考视频，在 `frame_idx=0` 注入 guide，关闭可见首帧 I2V 条件并按目标输出帧数精确裁除追加 guide latent，不添加或裁除保护尾段；test Web/后端和 LAN all Worker 已支持 |
