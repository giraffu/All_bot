...
| `docs/子模块_MiniMaxH3视频服务_minimax_h3.md` | MiniMax H3 四个公开模式、10Eros v3 TURBO 与官方 INT8 模式专属执行 profile、Web 有序上传/私人人物 typed refs、十八个候选 LoRA（单次最多十三个，含 REF2V-only 与 T2V/I2V-only 变体）、分阶段入口、test/prod Worker 共享 ComfyUI 并存边界、26 文件模型包和 canary | task registry、API workflow、profile、model manifest、人物引用解析器 | runtime-verification-required | MiniMax H3 开发/发布 |
