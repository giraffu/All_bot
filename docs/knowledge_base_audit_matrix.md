...
| `docs/子模块_MiniMaxH3视频服务_minimax_h3.md` | MiniMax H3 四个公开模式、10Eros/官方主模型受控选择、TURBO REF2V/ManualSigmas、Web 有序上传/私人人物 typed refs、十七个候选 LoRA（单次最多十三个，含 REF2V-only 变体）、分阶段入口、test/prod Worker 共享 ComfyUI 并存边界、25 文件模型包和 canary | task registry、API workflow、profile、model manifest、人物引用解析器 | runtime-verification-required | MiniMax H3 开发/发布 |
