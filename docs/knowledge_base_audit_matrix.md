...
| `docs/子模块_QQCC懒人Bot_qqcc_lazy_bot.md` | `qqcc_bot/main.py`、`qqcc_bot/gallery_market.py`、`qqcc_bot/keyboards.py`、`qqcc_bot/prompt_handlers.py`、`src/services/qqcc_config_service.py`、quick image/video FSM、Gallery/apply-context 服务、cloud compose QQCC profile、`scripts/update_cloud_prod_qqcc_bot.sh` | 已修正 | 补充 AI绘图 `draw_scenes` 动态配置、`qdraw_scene:` callback、默认无场景隐藏、提交 `pornmaster_flux2_single_edit` 自由P图 v2 单图任务；保留 AI 动图 `video_scenes`、`qvid_scene:`、旧 `qvid_mode:` 兼容、场景固定时长、Bot 只选画质、动图动态场景统一提交 `custom_video` 且底层复用 `image_to_video` / `Wan22AioV82.json`；保留 QQCC 市集、快速入口、`bot:qqcc` 来源和双 polling 红线 |
...
| `.codex/skills/allbot-qqcc-lazy-bot/SKILL.md` | `qqcc_bot`、`qqcc_bot/gallery_market.py`、cloud compose、`scripts/update_cloud_prod_qqcc_bot.sh`、`src/services/qqcc_config_service.py`、quick image/video FSM | 已修正 | 补充 QQCC AI绘图动态 `draw_scenes` 配置、`qdraw_scene:` callback、默认隐藏与 `pornmaster_flux2_single_edit` 提交口径；保留 AI 动图动态 `video_scenes`、`qvid_scene:` callback、旧 `qvid_mode:` 兼容、后台固定时长、Bot 只选画质与 `custom_video` 提交口径；QQCC 独立 Bot、快速入口、市集入口和单 polling 红线有效 |
