...
| `docs/子模块_QQCC懒人Bot_qqcc_lazy_bot.md` | `qqcc_bot/main.py`、`qqcc_bot/gallery_market.py`、`qqcc_bot/keyboards.py`、`qqcc_bot/prompt_handlers.py`、`src/services/qqcc_config_service.py`、quick image/video FSM、Gallery/apply-context 服务、cloud compose QQCC profile、`scripts/update_cloud_prod_qqcc_bot.sh` | 已修正 | 补充 AI绘图 `draw_scenes` 动态配置、`qdraw_scene:` callback、默认无场景隐藏、场景 engine/LoRA 配置与 `free_edit_v2`/`free_edit` 提交路由；保留 AI 动图 `video_scenes`、`qvid_scene:`、旧 `qvid_mode:` 兼容、场景固定时长、Bot 只选画质，并同步旧 `image_to_video` 可提交 `custom_video`/`video_lora`、`wan22_video_v2` 不支持 LoRA；新增 `end_frame_draw_scene_id` 引用有效 AI绘图场景，隐藏生成尾帧后首尾帧提交视频，额度预检按绘图加视频合计；保留 QQCC 市集、快速入口、`bot:qqcc` 来源和双 polling 红线 |
