...
本矩阵记录 2026-06-27 对 AllBot 实时知识库的逐项核对结果，并承接后续 2026-07-03 QQCC 绘图后处理链、2026-07-05 Cloudflare 公网入口拆分等局部知识库同步。事实源优先级为当前代码、compose、脚本、Alembic、`pytest --collect-only`、`ruff check`、文档结构检查与明确标注的只读运行态/API 探测；未标注的远端 SSH、线上 curl 或 Docker 运行态不作为本轮事实源。
...
| 文档 | 事实源 | 本轮状态 | 处理结果 |
| :--- | :--- | :--- | :--- |
| `README.md` | `docs/` 清单、`.github/workflows/docs_ci.yml`、部署脚本 | 已修正 | 按 `docs/*.md` 当前清单补齐活跃索引，恢复 GPU Pool、QQCC、付费群审核、本地数据分析平台与 Cloudflare 公网入口等入口；同步云测试快速单模块更新为日常默认 |
| `AGENTS.md` | `.codex/skills/*/SKILL.md`、运维脚本 | 已修正 | 测试优先部署改为云测试快速单模块更新优先，维护式脚本退为整栈/迁移/排空/明确维护窗口入口；新增 `allbot-cloudflare-ops` 作为 Cloudflare token/DNS/Tunnel/Access/公网管理域名路由 |
...
| `docs/子模块_Cloudflare公网入口与账号管理_cloudflare_ops.md` | Cloudflare DNS/Access/Tunnel 只读 API 探测、`/home/hfy/.cloudflare/allbot-cloudflare-admin.token` 文件元数据、本地分析 compose、用户级 `cloudflared-local-analytics.service`、网络/本地分析文档 | 新增 | 新增 Cloudflare 专项 SOP：记录账号/zone/team domain 非敏感事实、自动化 token 主路径与兼容链接、当前 `analytics.aivison.it.com` 与 `qqcc-admin.aivison.it.com` Access 保护入口、DNS/Tunnel/Access 验证命令、受保护公网入口新增流程、token 轮换流程、回滚与文档维护红线；明确不记录 token 明文，Account-owned token 以目标 API 探测为准 |
