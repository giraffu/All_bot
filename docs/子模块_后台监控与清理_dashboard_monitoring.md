...
- Dashboard `/api/system/status` 同时返回 `runpod_profile_queue_details` 兼容字段：后端字段按正式 RunPod profile 事实源聚合；前端隐藏旧 `pornmaster_flux2_edit` 监控行，保留 `pornmaster_flux2_edit_bf16 / 自由P图 v3`。BF16 返回 `autoscaler_enabled=true`，与其它正式 profile 一样自动 add/down/restart/enable，默认单任务 30 秒、清空阈值 30 分钟，并保留暂停、锁定、重启和删除人工兜底。前端仍用 `queue_by_type_details` 重新计算展示用活跃数和排队数；`scail2` 展示额外折入 LAN 任务，但 RunPod autoscaler 只统计 RunPod 可承接类型。该字段的 `active_count` 是未终态任务数而非 worker 数；真实 RunPod/本地 worker 数由 `/api/system/workers` 心跳另算。
