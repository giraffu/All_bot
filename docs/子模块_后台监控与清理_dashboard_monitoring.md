...
- Dashboard `/api/system/status` 同时返回 `runpod_profile_queue_details` 兼容字段：后端字段仍按正式手动 RunPod profile 事实源聚合 `img2img`、`image_to_video`、`wan22_video_v2`、`i2i_pro`、`scail2`、`ltx_video`、`pornmaster_flux2_edit`（UI label `pornmaster_flux2 / 自由P图 v2`，统计 `pornmaster_flux2_single_edit,pornmaster_flux2_multi_edit`）。前端“活跃 Worker 详情”会用 `queue_by_type_details` 重新计算展示用的活跃数和排队数，让同页的任务类型明细与 worker profile 行保持一致；其中 `scail2` 展示会额外折入 LAN 正式可承接的 `scail2_action_transfer_long` 与 `scail2_face_swap_v2`，但 RunPod autoscaler 决策、清空阈值和单任务耗时设置仍只按正式 RunPod `scail2_action_transfer,scail2_video_replacement` 口径生效，避免把 RunPod 不能接的任务纳入自动扩容。该字段的 `active_count` 是 profile 覆盖任务类型中的未终态任务数，不是 worker 数；真实 RunPod/本地 worker 数由前端基于 `/api/system/workers` 的 worker 心跳另行计算。该字段的最长等待和非低信任最长等待均取 profile 覆盖任务类型中的最大 pending 等待，不包含执行中任务运行时长；同时返回 `non_low_trust_clear_pending_count`、`non_low_trust_clear_pending_count_by_task_type` 与 `last_non_low_trust_pending_queue_index`，表示该 profile 内按 Central pending 队列顺序清到最后一个已知非低信任 pending 任务所需处理的任务前缀。低信任或未知用户任务若排在该锚点前会计入前缀，排在锚点后不计入；没有已知非低信任 pending 或低信任查询失败时前缀为 0。`pornmaster_flux2_edit` 返回 `autoscaler_enabled=true`，进入 Dashboard RunPod autoscaler 自动扩缩容，默认单任务 30 秒、清空阈值 30 分钟，仍可在 Dashboard RunPod 管理中手动新增/暂停/删除。
