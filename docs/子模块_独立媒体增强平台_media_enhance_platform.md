...
cookie；管理员通过本地环境变量首次初始化，密码不得进入 Git。新普通账号只能以
中国大陆手机号接收短信验证码，核验通过后才创建账号并发放欢迎点数，登录使用
“手机号 + 密码”。系统不再开放邮箱注册，也不发送验证或找回邮件；历史邮箱账号
和环境变量初始化的管理员仍可使用“邮箱 + 密码”登录，避免升级后锁号。
...
override 注入 fake，不发送真实短信。注册验证码与历史账号补绑验证码使用不同
purpose，不能交叉消费；发送间隔、手机号/账号 24 小时次数、请求来源 24 小时次数、
全站 24 小时费用上限、验证码有效期和失败核验次数均由服务端限制。手机号明文只在当次请求和 PNVS 调用期间使用，数据库长期
