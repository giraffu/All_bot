...
- 高级图生视频pro 接收 Web/主 Bot 新生成的 I2V、FLF2V 与 REF2V 投稿，统一在
  ...
  参考图作为模板素材默认带入、可预览并可逐张替换。投稿若包含一个主角参考语音，
  一键应用也默认带入该语音并允许使用者删除或换成自己的上传；客户端只持有投稿级
  受控引用和短期试听地址，不获得作者私有上传路径。缺少完整 Pro 上下文的旧投稿仍
  可点赞、收藏、评论和举报，但不能一键应用；模板衍生结果不能再次投稿。
- H3 普通续段不能单独投稿，但仍可按该段 I2V/FLF2V 设置一键应用，新用户必须重新
  提供自己的首帧或首尾帧。整链免费拼接作品按顺序合并各段提示词，并继承首段投稿
  权限：原创首段的完整拼接作品可投稿，模板派生首段不能借拼接恢复投稿权限；拼接
  作品本身禁止一键应用，也不能继续作为扩展父段。
