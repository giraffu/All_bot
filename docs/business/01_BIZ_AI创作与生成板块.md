...
- **人物参考图**：在练功房把一张人物源图拆成最多六个可编辑、可独立重生的视角，
  至少完成两个视角后保存到修仙笔记的人物图库
...
人物参考图模式使用独立工作台：上传源图并创建免费草稿，在正脸、侧脸、3/4
侧脸、全身正面、全身侧面、全身背面六个 tab 间编辑默认提示词；每个子图独立
提交并消耗 3 灵石。至少两个子图完成后可保存固定六宫格参考图。人物资产在
