...
- **人物身份素材**：可从任意一张人物、身体局部或物件图开始；十个可选槽包含正脸、
  裸体/穿衣正面全身、三个身体局部和四个自定义上传位。前三个基础槽可生成，局部槽
  可选择管理员多图片模板或上传替换，至少一张 ready 后合成一张 H3 REF2V 人物素材图
...
人物身份素材模式使用独立工作台，页面不重复展示站点上游已经处理的确认。用户先选择
任意槽位并上传图片或选择管理员局部模板即可创建，名称是唯一必填元数据；人物描述、
子图名称和子图描述在创建后补充。正脸、裸体正面全身、穿衣正面全身可按自由 P 图流程
生成并计费；胸部、正面私处、背面私处只允许选择模板或上传，四个扩展槽只允许上传。
每个槽位均可缺失，至少一张即可保存。

服务端把两张可能存在的正面全身图作为最左侧窄长列，其余 ready 图片按上下两张一列
依次排列，缺图自动回流，最后单张占满其列。完整合成图用于人物库资产与预览；H3
REF2V 从人物库逐张选择 ready 子图，可选择同一人物的不同视图，并与临时上传混排、
拖动排序，总数最多四张。人物描述、子图描述和存储路径均由服务端注入，Prompt
Optimizer 与最终生成保持相同 `<Picture N>` 顺序。
服务端会把同一人物的多张子图绑定为目标视频中的同一个人物实例，并自动禁止输出参考图
本身、接触表、分屏和重复身体；用户不需要手写 `<Picture N>`。
