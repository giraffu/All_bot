...
429/SlowDown 或明确 5xx 立即降一档，单个 timeout/reset 不降档，只在滑动窗口
错误率达到系统性门槛时降档。即使 SDK 重试最终成功，R2 操作延迟 p95 连续
两个批次不低于 8 秒，或单批最大值不低于 120 秒，也降一档。只有连续两个
窗口同时满足低错误率、p95 不高于 5 秒且最大值不高于 30 秒时才逐档回升，
最高 128。这些阈值属于 artifact 代码身份，不提供生产 CLI 热调参入口。非瞬态错误
