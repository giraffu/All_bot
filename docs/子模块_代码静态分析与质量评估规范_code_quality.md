...
- `logs/code_analysis_report_*.md` 是一次性报告，不作为当前架构事实源。
- 行数、复杂度、重复率、测试数量和告警数量必须在本轮重新计算，不复制历史
  数字到活跃文档。
- 稳定结论只同步到对应 Skill、架构文档、热点门禁或 compat 退出表。
- 已完成清理和 dated 扫描结果进入 `docs/archive/`；默认研发上下文不加载。
