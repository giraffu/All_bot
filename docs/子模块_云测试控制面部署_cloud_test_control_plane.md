...
- 验收结束后必须恢复临时禁用的非 RunPod cloud-test `i2i_pro` worker，删除本次新建的 RunPod Pod，并确认 `list-pods` / `reconcile-managed-pods` 的非忽略 managed count 为 0；既有 prod 手动备用 Pod 必须保持运行。
