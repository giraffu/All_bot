...
- Test-train：A-H 只提交 PR；集成 AI 执行 `scripts/test_train_release.py plan|deploy|accept|block`。最终组合候选再执行 `freeze|approve-release`，批准记录绑定测试站真实运行 digest；未批准冻结可 `abort-freeze`。main promotion 复用原字节，不再次部署本测试站。包装器使用独立 candidate cache 和本地排他锁，GPU 变化只报告并转交 profile canary/operator。默认只部署需要测试的 control-plane/公共 Web；QQCC Config 候选按 standard 部署专属测试前后端，Dashboard-only 候选写 `test-not-required`。Worker 专项诊断显式加 `--with-test-execution`。
