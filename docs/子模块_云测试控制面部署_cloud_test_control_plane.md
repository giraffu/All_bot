...
- Web 自由P图 v2.5/v3 发布验收共用运行时开关 `enable_free_edit_v3` 和 BF16 执行池。先验收两张模式卡、单图限制、3/5 灵石、无 LoRA、预签名上传、投稿与一键应用入口；目标 Worker 可用时分别完成“v2.5 单阶段生成→投稿→应用”和“v3 BF16→换脸生成→投稿→应用”黄金路径。Worker 不可用时记录独立待办，不得把未执行写成通过；未完成 24 小时观察时只记录 smoke，不执行 `verify-test`。
