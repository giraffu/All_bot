...
- Web 自由P图 v2.5/v3 的专项业务验收共用运行时开关 `enable_free_edit_v3` 和 BF16 执行池。普通容器 smoke 成功已自动形成 verified evidence；若本次明确要求业务黄金路径，再补充两张模式卡、计费、预签名上传、投稿/应用及“v2.5 单图/双图、v3 BF16→换脸”等人工验收，并可用 `verify-test` 附加证据。未执行的专项检查不得写成通过。
