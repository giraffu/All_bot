...
| `allbot-ops-deployment` | Docker Compose、Alembic、safe_deploy_test / safe_deploy、云正式控制面、Worker workflow 事实源、部署排障与恢复 | 调整部署、迁移、容器、环境变量、云正式热修、workflow 资产或恢复脚本时 |
| `allbot-comfy-models` | LoRA / ControlNet / ComfyUI 工作流参数透传、动态注入、Worker workflow 映射校验 | 修改附加模型、工作流映射、Bot 菜单参数时 |
