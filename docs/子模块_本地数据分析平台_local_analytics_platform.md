...
网关通过只读 SSH 路径打开 NAS 普通文件并支持 Range。表格输入/输出单元格直接显示
首个可用媒体预览，并作为“查看输入/输出”按钮；独立“归档媒体/查看全部”列已移除。
详情图片点击后在页内灯箱放大原图，不使用新窗口原件链接。公网读取必须同时通过
Cloudflare Access 和应用登录，响应使用 `private, no-store`；opaque ref ID、loopback
网关和 NAS 凭据边界保持不变。
