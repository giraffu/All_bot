...
- 用户画像: 用户增长、每日新增入宗门、每日新增生成用户、生成活跃、签到趋势、身份/修为/灵石/生成/活跃分布和用户排行；新增入宗门按 `users.created_at` 分桶并筛选当前 `is_channel_member=true`，新增生成用户按 `history` 中每个用户的首次生成日期分桶。
...
- 核心可视化: 用户画像、灵石收支、充值情况和生成分析四个核心 Tab 使用本地静态 `ECharts 6.0.0`，替代旧 `div` 高度柱状块；主图具备坐标轴、图例、tooltip、donut、漏斗、堆叠柱、累计折线、分时对比和风险散点，仍保持独立静态页面，不引入 Dashboard Vue 构建链。
...
- Playwright 桌面与窄屏检查应确认 `body[data-loaded="true"]`、核心四个 Tab 的 ECharts 容器非空、有坐标轴或图例、无前端 console error、无整页水平溢出；静态测试应确认核心 Tab 不再出现 `spark-bars` / `hourly-bars` 主图容器。
