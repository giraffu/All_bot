...
测试状态按 `track + artifact + digest` 保存在 history。standard 写 tested，低风险直发写 waived，GPU attestation-only 写 attested。测试列车默认不部署 test-execution，Dashboard/QQCC 管理服务从测试 Compose 移除。GPU artifact 强制 attestation；业务 canary 只对 standard 必需。RunPod/LAN operator 每次只滚动一个 slot，disabled 验证失败即恢复旧 digest 并停止。
