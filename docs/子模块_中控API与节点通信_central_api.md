...
- 若分发逻辑异常导致任务堆积，应先检查：
  ...
  - SCAIL-2 Web 测试只应由 `cloud_worker_test_08` 声明 `SUPPORTED_TASK_TYPES=scail2_action_transfer,scail2_video_replacement`，并通过 agent control 在验证前保持 `disabled`；ComfyUI runtime 是 gpu-002 LAN AIO SCAIL-2 容器 `http://192.168.1.2:8190`，用于动作迁移/视频换人测试，不是云正式 worker 能力
