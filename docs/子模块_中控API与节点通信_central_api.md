...
- 若分发逻辑异常导致任务堆积，应先检查：
  ...
  - LTX 高级图生视频正式可由 LAN AIO `lan_aio_prod_gpu177_gpu1_ltx_video_01` 或手动正式 RunPod `runpod_prod_ltx_video_manual_NN` 接单；RunPod profile 必须声明 `ltx_video,ltx_video_flf2v,ltx_video_v2v_audio`，写 `user-data-prod`，并通过 profile override 使用 10Eros v1.2 workflow。canary 结束后 RunPod worker 保持 disabled，手动 enable 后才参与调度。
