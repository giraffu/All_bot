...
- `/system/status.queue_by_type` 仍表示 Central pending 队列按执行类型的计数；`queue_by_type_details` 是同一 Central 视图的轻量补充，只读扫描 `comfy:queue:pending` 与 `comfy:task:{backend_task_id}` 中的 `type` / `created_at` / `priority`，按任务类型返回 `pending_count`、`max_free_pending_wait_seconds` 和 `max_paid_pending_wait_seconds`。这里的免费/付费最长等待只按 Central 入队 priority 分桶：`priority <= 0` 归为免费，`priority > 0` 归为付费或加速；它不查询用户订单、低信任免费层或 active registry，也不改变调度顺序。Dashboard `/api/system/status.queue_by_type_details` 仍是更高层聚合口径，包含 active count、低信任免费层与非低信任最长等待等字段。
...
- 任务类型事实表位于 `src/domain_config/task_type_registry.py`，当前提供查询 helper 并驱动 Gallery/apply 等低风险读路径，同时作为一致性门禁；它不改变 Central 按 `SIMPLE_TASK_TYPE_MAP`、队列 task type 与 worker `SUPPORTED_TASK_TYPES` 分发的运行时语义。新增 Central simple route 或 workflow 映射时，必须同步 registry 并跑 `tests/config/test_task_type_registry.py`。
