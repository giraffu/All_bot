...
- 任务类型事实表位于 `src/domain_config/task_type_registry.py`，当前只作为一致性门禁，不改变 Central 按 `SIMPLE_TASK_TYPE_MAP`、队列 task type 与 worker `SUPPORTED_TASK_TYPES` 分发的运行时语义。新增 Central simple route 或 workflow 映射时，必须同步 registry 并跑 `tests/config/test_task_type_registry.py`。
