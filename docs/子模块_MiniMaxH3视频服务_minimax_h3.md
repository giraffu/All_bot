...
主 Bot 的 REF2V 使用统一参考模式，不再把图片、音频拆成前后两个步骤。用户可在同一
状态直接发送上述三类媒体，Bot 自动识别类型并在每次接收后显示当前有效价格、图片/
音频/视频剩余名额，以及 `<Picture N>`、`<Audio 1>`、`<Video 1>` 的简短提示词指导。
音频不能单独提交；视频上传后只展示不超过原片长度的片段按钮。当前价通过任务定价
matcher 解析后台覆盖配置，并作为提交快照传入任务 facade，保证提示价与最终扣费一致。
