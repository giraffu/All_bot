...
- 四类场景的 `credit_cost` 只允许 `null` 或大于等于 1 的整数，缺失字段统一按
  `null` 归一。固定价只读取用户直接点击的根场景，代表完整链总价；清空后恢复
  动态/分段计费。新增 AI动图、AI绘图和 AI滤镜场景仍从 options 读取
  `6/2/2`，新增 AI视频保持 `null`。Config Backend 通过
  `ai_video_credit_costs` 下发 H3 完整矩阵，配置弹窗根据模式、时长和清晰度展示
  当前默认价，但保存时仍保持 `null`，因此后续默认矩阵更新会自动生效；管理员显式
  填写的固定价保持不变。
...
- 配置 Web 四类场景行均展示灵石消耗与输入/输出示范操作；AI视频固定价留空时，输入框提示当前模式、时长和清晰度对应的 H3 默认价。上传后双栏预览。REF2V 参考图卡片额外提供模板显示名输入，排序/替换/删除时名称与 file_id 缓存同步移动或失效。绘图/滤镜点击场景先发双图片，动图/视频先发图片+视频，随后才发文字提示；REF2V 再发送带名称的模板图库，第一张自动成为默认模板并立即进入主体图接收状态。按钮标为“替换：名称”；点击后进入独立模板上传状态，下一张图只替换该名称对应的模板且不得生成。Bot 按槽位保留当轮所有用户替换，通过用户上传的 Telegram `file_id` 与未替换的管理员缓存重新发送完整模板组，用户替换图不得写回管理员缓存；随后再发送场景主提示和按钮并等待女性人物正面清晰图。首次发送管理员模板时从 R2 获取并缓存 file_id，重复发送不再上传；缓存失效自动刷新，替换内容后旧 file_id 不复用。
