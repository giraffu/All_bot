...
图库，第一张立即作为默认模板，用户可直接上传女性人物正面清晰图，也可通过名称按钮
...
- 配置 Web 四类场景行均展示灵石消耗与输入/输出示范操作；上传后双栏预览。REF2V 参考图卡片额外提供模板显示名输入，排序/替换/删除时名称与 file_id 缓存同步移动或失效。绘图/滤镜点击场景先发双图片，动图/视频先发图片+视频，随后才发文字提示；REF2V 再发送带名称的模板图库，第一张自动成为默认模板并立即进入主体图接收状态。按钮标为“替换：名称”；点击后进入独立模板上传状态，下一张图只替换该名称对应的模板且不得生成。Bot 按槽位保留当轮所有用户替换，通过用户上传的 Telegram `file_id` 与未替换的管理员缓存重新发送完整模板组，用户替换图不得写回管理员缓存；随后再发送场景主提示和按钮并等待女性人物正面清晰图。首次发送管理员模板时从 R2 获取并缓存 file_id，重复发送不再上传；缓存失效自动刷新，替换内容后旧 file_id 不复用。
- REF2V 提交计划只接受当前场景仍存在的 `selected_reference_image` 槽位，生成输入固定为 `[用户主体图, 槽位 1 最终图, ..., 槽位 N 最终图]`；每个槽位独立选择当轮用户上传图或管理员对象，替换任意一个或多个槽位都不得删除其余默认模板。History 保存这组完整有序输入；重生成下载并恢复全部 History 输入槽位，确保沿用当时的混合模板组。配置删除当前选中槽位或历史输入数量不完整时 fail closed。
