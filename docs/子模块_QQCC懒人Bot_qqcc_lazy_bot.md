...
主体图。上传的主体图作为 `<Picture 1>`，最终管理员模板或用户替换图固定为
...
- REF2V 提交计划只接受当前场景仍存在的 `selected_reference_image` 槽位，生成输入固定为 `[用户主体图, 最终模板]`；最终模板可为管理员对象或当轮用户上传的临时图。结果 metadata 记录槽位 key、显示名和 `user_upload` 来源；重生成从 History 第二个输入恢复用户替换图，管理员模板则继续按 object key 下载，配置删除该槽位后 fail closed。
