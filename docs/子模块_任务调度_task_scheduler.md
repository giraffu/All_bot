...
- SCAIL-2 Web 测试 task type 为 `scail2_action_transfer` 与 `scail2_video_replacement`，只允许云测试 worker `cloud_worker_test_08` 指向 `http://192.168.1.2:8190` 接单，初始必须通过 agent control 保持 `disabled`。第一版只开放 5s/8s，成本 40/80 灵石，不承诺长视频；非法时长应在 Web task core strategy 阶段以 400 拒绝。
