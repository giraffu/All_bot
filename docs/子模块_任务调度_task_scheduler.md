...
- SCAIL-2 Web 测试 task type 为 `scail2_action_transfer` 与 `scail2_video_replacement`，只允许云测试 worker `cloud_worker_test_08` 接单；该 worker 指向 gpu-002 LAN AIO SCAIL-2 runtime `http://192.168.1.2:8190`。runtime 容器本身不设置 `AGENT_ID` / `CENTRAL_API_URL` / `SUPPORTED_TASK_TYPES`，Central 接单边界仍在 worker agent。第一版只开放 5s/8s，成本 40/80 灵石，不承诺长视频；非法时长应在 Web task core strategy 阶段以 400 拒绝。
