...
新增装配能力时应优先落在对应 builder，保持
`build_default_task_core_process_dependencies(...)` 作为聚合入口，而不是重新把
基础设施解析堆回 facade。
...
- 图片换脸业务类型仍分流：`face_swap` 默认扣 2 灵石，快速/随机换脸和旧 Gallery/历史重生成继续提交该类型；`face_swap_v2` 独立调用也默认扣 2 灵石。Public Web 的图片换脸和随机换脸模式价格标签也必须显示 2，与计费注册表保持一致。当前 i2i_pro/专属 face-swap profile 对两个类型都 override 到 `face_swap_v2.json`，业务类型差异不再对应价格或 V1/V2 执行池。自由P图 v3、QQCC 原脸恢复和 SCAIL-2 首帧预处理只把 V2 作为组合任务内部阶段，不得重复扣费或改变上层 History 类型。
- SCAIL-2 task type 包括 `scail2_action_transfer`、`scail2_video_replacement` 与 `scail2_face_swap_v2`；正式视频换脸走 SCAIL-2 FaceSwap v10 two-stage audio workflow，但只由 gpu-002 LAN slot0 agent `lan_aio_prod_gpu002_gpu0_scail2_01` 承接，正式 RunPod `scail2` profile 仍只接动作迁移/视频换人。视频换脸先抽取驱动首帧，以固定优先级 100 提交标准 `face_swap_v2`；阶段成功后先持久化 continuation intent、切换 TaskRegistry，再用确定性 backend ID 按根任务正常身份/入口优先级提交 `reference_preprocessed=true` 的 SCAIL-2 replacement 阶段。第一阶段可取消，第二阶段不可取消；全链只扣 40/80 灵石一次，中间图不进入 History、Gallery 或投递。SCAIL-2 Worker 不再调用外部图片换脸 runtime。SCAIL-2 第一版只开放 5s/8s，不承诺长视频；非法时长应在 Web task core strategy 阶段以 400 拒绝，或在 Bot FSM 内拒绝。Bot 收参考图、驱动视频和可选正向提示词，Web prompt 也可留空；空 prompt 由 `normalize_scail2_positive_prompt(...)` 按 task type 补默认值，负面词使用默认值，驱动视频上限 40MB。正式发布或强制 RunPod canary 前用 `scripts/cloud_prod_generation_release_gate.py` 写共享 `/var/lib/allbot/prod/runtime/GENERATION_MAINTENANCE`，并在 Web、Main Bot、QQCC Bot、Private Bot Worker 四个 Compose service 内确认挂载可见。pending 退款只能通过最终 Web API 精确 digest 的 `src.ops.generation_release_refund` 运行时执行：先只输出无 PII 汇总，orphan/歧义映射直接阻断，再显式执行；ZREM 竞态变为 running 的任务不退款也不强杀。
