...
- SCAIL-2 task type 包括 `scail2_action_transfer`、`scail2_video_replacement` 与 `scail2_face_swap_v2`。视频换脸先抽驱动首帧，以固定优先级 100 提交标准 `face_swap_v2`；阶段成功后先持久化 continuation intent、切换 TaskRegistry，再用确定性 backend ID 按根任务正常身份/入口优先级提交 `reference_preprocessed=true` 的 SCAIL-2 replacement 阶段。第一阶段可取消，第二阶段不可取消；全链只扣 40/80 灵石一次，中间图不进入 History/Gallery/投递。SCAIL-2 Worker 不再调用外部图片换脸 runtime。SCAIL-2 第一版只开放 5s/8s，不承诺长视频；非法时长应在 Web task core strategy 阶段以 400 拒绝，或在 Bot FSM 内拒绝。正式发布或强制 RunPod canary 前用 `scripts/cloud_prod_generation_release_gate.py` 冻结 Web/Bot 新生成，等待 pending 低于阈值 10 后只取消退款仍在 pending 的任务；running 任务不强杀。
