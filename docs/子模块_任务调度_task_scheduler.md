...
- 测试环境：默认云测试控制面，日常维护式更新优先使用 `scripts/update_cloud_test_with_maintenance.sh --execute`；`scripts/safe_deploy_cloud_test.sh` 是远端控制面重建子步骤。旧本地测试栈不再作为受支持测试或回滚路径，仅作为历史取证材料保留。
