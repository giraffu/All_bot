...
96GB 节点 OOM，该结论不适用于本版受限中分辨率、动态时空预算和 VSR 收尾链路。
...
2 倍时直接逐帧 RTX VSR；超过 2 倍时才在 Div32 画布执行 IC 8 步，用 LTX-2.3
潜空间模型放大 2 倍并采样 3 步，裁掉适配任意比例产生的黑边，最后由 RTX VSR
精确收敛到目标尺寸。中间画布不是固定分辨率：按规范化后的实际时长计算
“IC guide 合并后的潜空间帧数 × 画布面积”统一预算，长边同时封顶 864；典型
768×448 输入在 5/10/15 秒时分别使用 640×384、448×256、384×224，源视频为
480p、512p、1080p 或其它比例时都按同一预算和原始宽高比动态适配。潜空间放大后
的 RTX VSR 收尾倍率最高 4 倍。

两个 `LTXVLoopingSampler` 都显式使用 `temporal_tile_size=1000` 和
`adain_factor=0.1`，确保 1 至 15 秒任务只进入一个时间窗口。当前固定 revision 的
IC-LoRA guide 不支持 LoopingSampler 的第二时间块：使用 208 帧分窗会令第二块的
guide token 与 grid mask 长度不一致，因此不能通过时间分窗换显存；显存必须由上述
时长相关的空间画布控制。无声视频会补静音轨；最终 `CreateVideo` 复用源视频音轨，
不使用采样器生成的音频作为结果。
...
Pod 在 Worker heartbeat 前进入 `EXITED`、`STOPPED`、`TERMINATED` 或 `DELETED`
终态时立即失败，不能仅凭短暂的 RUNNING/镜像端口信号继续空等 heartbeat timeout。
