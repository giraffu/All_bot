...
`face_swap_v2` 与 i2i_pro profile 接取的 legacy `face_swap` 都使用
`face_swap_v2.json`，`t2i-pornmaster-turbo` 使用
`txt2img_from_i2i_pro.json`，三者与 `i2i_pro.json` 共享
...
`scail2` LAN AIO 泛化 fleet 渲染必须内置正式四任务 workflow override：动作迁移 audio workflow、长动作迁移 Context Windows workflow、视频换人 replacement workflow、视频换脸 v10 replacement workflow。视频换脸首帧预处理已迁到 Central 标准 `face_swap_v2` 阶段；`runtime_profile=scail2` 不再渲染任何 `SCAIL2_FACE_SWAP_V10_*`、外部 Comfy API 地址或 `face_swap_v2.json`。Worker 只接受 Central 已标记 `reference_preprocessed=true` 的视频阶段。
...
| `i2i_pro` | `i2i_pro,t2i-pornmaster-turbo,face_swap_v2,face_swap` | `i2i_pro` | `runpod_test_i2i_pro` | `i2i_pro/2026-06-14-test/manifest.json` |
...
| `i2i_pro` | `runpod_prod_i2i_pro_manual_NN` | `i2i_pro,t2i-pornmaster-turbo,face_swap_v2,face_swap` | `i2i_pro/2026-06-14-test/manifest.json` | `NVIDIA GeForce RTX 4090` |
...
- Central 核心 canary 任务类型分别为 `i2i_pro`、`t2i-pornmaster-turbo`、`face_swap_v2`，每单 `pop_evidence.agent_id` 都匹配该 RunPod worker；heartbeat 必须额外声明 legacy `face_swap`，且 render 显示其 override 为 `face_swap_v2.json`。本次兼容发布不对 RunPod 提交 legacy smoke。
...
`i2i_pro/t2i-pornmaster-turbo/face_swap_v2/face_swap=12s`，四者都纳入 i2i_pro autoscaler。`scail2_action_transfer/scail2_video_replacement=300s`、
...
- `prod-worker canary --profile i2i_pro --execute` 的既有核心 canary 会串行提交 `i2i_pro`、Web `txt2img`、`face_swap_v2` 三单；兼容发布只做未来新 Pod 的四任务只读 render，不创建、重启或测试 RunPod。
