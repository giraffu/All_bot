...
公共云 Compose 的逐服务 `env_file` 使用长语法 `required: false`，只为允许 Compose 在局部模块发布时解析尚无投影的非目标服务；这不是容器运行时的缺配置豁免。发布器在任何 pull/up 之前仍通过目标模块 service closure 严格生成、校验并核对目标投影 revision，目标投影缺失或必填键缺失立即失败。非目标容器不执行 `up`，且事务会逐一核对其启动时间不变。该 Compose 内容以精确 checksum 纳入 owner-only snapshot，任一后续改动重新成为共享契约 blocker。
...
增量空集判定还必须与当前部署 state 的 exact digest 比较。目标 artifact 即使由较早 main 构建、本次 bundle 只复用，但当前运行态缺记录或 digest 不同时，planner 仍必须选入并替换；仅 source SHA 复用不能报告 no-change。测试环境静态不存在的 Dashboard、Payment 和 Paid Group 不进入运行态漂移替换集。
