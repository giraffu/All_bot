...
- RunPod Provider v0 的稳定边界是云测试 `img2img/img2img_lora` canary、云测试 split video profile (`image_to_video` / `wan22_video_v2`) canary、云测试 `i2i_pro` profile canary，以及手动云正式备用 worker；生产自动按队列扩容仍未开启，`i2i_pro` 正式 RunPod 尚未开放。
