...
LAN-only `all` 镜像不得由 GPU 节点现场构建，也不得让 GPU 节点从 GHCR 拉取。可信 main 构建节点使用 `scripts/build_runpod_profile_image.sh --profile all --reuse-base-custom-nodes`，以本地 registry 中 digest-pinned 的 LTX 基线和 Wan 节点源镜像为输入，直接向 `localhost:5000` 推送；该模式拒绝公网 registry、mutable tag 或缺少 `@sha256` 的 source ref，并把两个 source digest 写入 OCI labels。完成 smoke、revision 与目标 manifest digest 核验并把 LAN digest 固定到 catalog 后，GPU 节点才可从 `192.168.1.115:5000` 预拉。模型权重不进入镜像，只通过 LAN cache manifest 与内容寻址对象在内网同步。
