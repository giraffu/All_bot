...
最近一次 gpu-252/gpu0 LTX T2V/Ingredients disabled canary：2026-07-22，Asia/Shanghai；验收后停止并恢复 intentionally-empty。
...
SCAIL-2 动作迁移 / 视频换人 / 视频换脸 v10 有测试与正式链路，不能混用桶和 worker。云测试链路由 `cloud_worker_test_08` 指向 LAN AIO SCAIL-2 runtime `http://192.168.1.2:8190`，可声明 `scail2_action_transfer,scail2_action_transfer_long,scail2_video_replacement,scail2_face_swap_v2` 并通过测试 env 覆盖到 audio/context-window/v10 workflow；视频换脸会先调用 `192.168.1.226:8188` 的 `face_swap_v2.json` 对驱动视频第一帧做图片换脸。测试 runtime 容器 `allbot-lan-aio-gpu-002-gpu0-scail2-test` 本身只跑 ComfyUI、模型同步与 workflow 资产，不注册 Central worker。云正式 SCAIL-2 LAN profile 固定使用从 verified GPU release `b2587e560fe3f94c941e21777dad40547e3e0158` 保摘要复制到 LAN registry 的 baked worker digest `192.168.1.115:5000/allbot/comfy-runpod-scail2@sha256:858ac45522f33189e16e6ad41c0080b785c6bb87808d890d8f9899e0ed9b7607`；禁止恢复旧 `20260704-sm120-xformers-pr1262` tag 或宿主 `remote_workers` 挂载。`gpu-226-gpu0-scail2` 在 2026-07-05 回切到 image_to_video 后保留为同卡候选，但该 RTX 5090 32G 低于 profile catalog 的 48G 偏好容量，未来再启用仍需观察 OOM/status 137。`gpu-252-gpu1-scail2` 固定返修 UUID `GPU-33de1af6-ca27-7eeb-ae46-6a9f4f89523e` 与 host `8191`，2026-07-04 虽通过 preflight、warm-cache、disabled heartbeat、`/system_stats`、`/object_info` 模型枚举、direct canary 和短 CUDA smoke，但真实 SCAIL-2 face-swap workload 复现 Xid 119/154，当前保持 maintenance-disabled，不计入 SCAIL-2 正式容量。该返修卡同日改承接低负载 `pornmaster_flux2_edit`，不代表 SCAIL-2/Wan22 解封。`gpu-177-gpu1-scail2` 已验证可启动并保留为同卡候选，但 2026-07-02 又切回 LTX 当前态；`gpu-252-gpu0-scail2` 在 2026-07-04 切到 i2i_pro 后保留为同卡回滚候选。正式 LAN SCAIL-2 worker 必须写正式 Central 与 `user-data-prod`，不要把它误判为测试 `cloud_worker_test_08` 能力；正式 RunPod `scail2` 仍保持两任务声明。
...
`takeover` 默认 `--failure-policy auto_rollback`：`stop-old` 后若新候选启动、disabled heartbeat 或 enable gate 失败，helper 会 disable 新候选、停止新候选容器、启动旧 runtime 并恢复旧 agent control，operation 仍按失败记录并带 `recovery_status`。`pull-image` 会先尝试目标节点 `docker pull`；若节点未配置 HTTP registry 但 runner 本地已有目标镜像，会自动流式 `docker save | ssh docker load` 到目标节点，避免为了单个镜像重启 Docker daemon。ledger 明确为 `intentionally_empty` 时，可对指定候选单独执行只读 `preflight --execute` 获取逐项结果；mutation 仍必须走 managed action。`configure-registry` 会同时补齐 Docker daemon 的 `insecure-registries`、`proxies.no-proxy` 和 systemd `NO_PROXY/no_proxy`，保留既有代理端点，避免 LAN registry 被误送到本机代理；重启后只等待原先正在运行的候选恢复，空槽不会被误启动或误等健康。`warm-cache` 会先尝试用 SSH 用户创建 workspace；若 retarget 后的 `/srv/allbot/runpod-runtime/...` 目录为 root-owned 导致 `mkdir` 权限不足，会临时使用目标镜像挂载 workspace parent 创建目录，再继续模型同步和 marker 写入。profile 可用 `lan_model_workspace_key` 复用同一物理槽内已经通过 manifest 校验且文件字节一致的模型目录，同时保留独立代码 workspace、worker、workflow 与远端 cache marker；不得用它共享未核对 checksum 的近似模型。release 候选使用 `canary-start-disabled --profile ... --release-index ... --sha ...` 时，`--release-index` 可传完整三轨 index，或同 SHA、`completeness=complete` 的独立 GPU manifest；helper 会解析并拉取精确 digest，拒绝 incomplete manifest，校验实际 image、OCI revision 与 runtime contract 后保持 intake disabled。若故障 runtime 已被 Central 标为 `quarantined|error`、没有 current task 且 Comfy queue 已不可达，或 agent 已注销但 control 仍明确为 `disabled`，只能用 `isolate-quarantined --slot ... --execute` 做故障隔离：写入无 TTL disabled，先设置并验证目标容器 `restart=no`，再停止和验证容器、把账本收口为 intentionally-empty；GPU reset-required 导致 stop 失败时，下次主机启动仍不会自动拉起该容器。普通健康 runtime 或仍有任务时拒绝。`start-disabled` 会在 compose up 前清理同名 `exited/created/dead/removing` 候选容器，并复查 host port 此刻必须没有 Docker owner；若同名容器正在运行、inspect 结果不匹配，或端口仍被任何容器占用，直接阻断，不误删。失败现场手工入口为 `recover --physical-slot <node>:gpuN --slot <slot-id> --prefer old|candidate`，只恢复一个物理 GPU；live/container/cache/control 巡检与恢复提交都只从本地主 AI operator/CLI 发起。
...
2026-06-27 曾新增 PornMaster Flux2 FP8 single/multiple image-edit profile。2026-07-24 该 execution profile 已退役：GPU artifact、模型 bundle、RunPod 创建/管理入口和 LAN 候选均从当前 catalog 删除；`pornmaster_flux2_single_edit` / `pornmaster_flux2_multi_edit`、旧 workflow 与镜像记录只保留为历史兼容证据，不再映射 worker pool，也不得通过旧测试 helper 重新启用。当前自由P图执行能力只使用独立 BF16 profile。

2026-07-12 gpu-226 GPU0 已准备 `gpu-226-gpu0-pornmaster_flux2_edit_bf16` slot。fleet helper 已把 PornMaster LAN 镜像加载到 gpu-226，并把 `pornmaster_flux2_edit_bf16/2026-07-12/manifest.json` 的 V4 turbo BF16 UNet、Qwen fp8 text encoder与 small-decoder VAE 同步到独立 PornMaster workspace；marker 为 ready。slot 的 `SUPPORTED_TASK_TYPES` 为 `pornmaster_flux2_edit_bf16,pornmaster_flux2_multi_edit_bf16`，分别复用 single/multiple workflow 并切换 BF16 UNet；它与已退役的 FP8 execution profile 不共享任务队列。

同日 14:10 Asia/Shanghai 已执行获批 takeover：在途 `image_to_video` 任务自然结束后，BF16 容器通过 health 与 disabled heartbeat gate 并启用；当前容器为 `allbot-lan-aio-gpu-226-gpu0-pornmaster_flux2_edit_bf16-prod`，host `8190`，接 `pornmaster_flux2_edit_bf16,pornmaster_flux2_multi_edit_bf16`。第一次接管因 heartbeat gate 错把 slot profile id 当 runtime profile 而自动回滚，修复门禁并补回归测试后第二次成功。未运行生成 canary，`image_to_video` 保留为同卡 rollback candidate。

2026-07-23 gpu-226 BF16 有界流水线完成 20 单正式观察并保留运行，证明 `gpu_done` 后把下一单计算与上一单串行交付重叠可行。2026-07-24 将该机制推广为两档版本化策略：`img2img/img2img_lora`、`i2i_pro`、`pornmaster_flux2_edit_bf16` 使用 claimed/Comfy/delivery=`3/2/1`；`image_to_video`、`ltx_video`、`scail2`、`wan22_video_v2` 使用 `2/1/1`，保持单 Comfy/GPU 执行槽。LAN render 与后续新建 RunPod 注入相同策略，存量 RunPod 不改；显式数字环境仍固定为旧镜像安全回滚默认 `1/2/1`。结果始终要完成物化、上传并成功调用 Central `/complete` 后才终态。
...
- 历史 canary 镜像：`192.168.1.115:5000/allbot/comfy-runpod-scail2:20260704-sm120-xformers-pr1262`；该 tag 仅解释 2026-07-04 的 RTX 5090 验证记录，后续恢复必须使用 profile catalog 的 baked digest
...
- `gpu-252-gpu0-i2i_pro` 和 `gpu-252-gpu1-i2i_pro` 的镜像、manifest 与旧生产验收只是历史事实；2026-07-22 实时 live/ledger 已是两槽 intentionally-empty，GPU1 还有硬件隔离门禁。
- `gpu-252-gpu0-ltx_t2v` 固定镜像 `192.168.1.115:5000/allbot/comfy-runpod-ltx-t2v@sha256:9ed3de73923fc8f021716f7fc19d8d3e5f6ed552a8ee11cb849b3dfb293db043`，使用 `ltx_t2v/2026-07-22` 10 文件 bundle；四组 A/B、成人 NSFW、三场景 IC 与最终 digest 回归均通过，但未开启 production intake。
- GPU0 所有 profile 当前只作为 catalog 候选；再启动必须由 AI operator/CLI 按 live/ledger 门禁选择精确 slot，不需要 drain 一个并不存在的“当前 i2i_pro”。
...
- `gpu-252-gpu1-pornmaster_flux2_edit` 历史正式 AIO 使用 LAN registry 镜像 `192.168.1.115:5000/allbot/comfy-runpod-pornmaster-flux2-edit:20260628-pornmaster-flux2-edit-cu128-smallvae1`，模型从 `allbot-model-cache/pornmaster_flux2_edit/2026-06-27/manifest.json` 同步；2026-07-04 曾通过短任务。2026-07-22 人物六视图真实 workload 完成后，同一 replacement UUID `GPU-8153a439-e3f6-8922-039d-dc13e97da6d7` 复现 Xid 119 并升级为 Xid 154 / GPU Reset Required，candidate 无法正常 stop。该实体卡现不得继续运行 PornMaster、LTX、SCAIL-2 或 Wan22；必须保持 agent disabled，待同机 GPU0 空闲和明确维护窗口后通过 operator 恢复，不能用此前短任务结论解除隔离。
...
- 只处理 `pornmaster_flux2_single_edit/pornmaster_flux2_multi_edit` 正式接单时，当前定位 gpu-002 GPU1 的 PornMaster AIO；gpu-252 两张卡均不接 production intake，历史 PornMaster slot 不应直接启用。
...
- 排查 `gpu-252` GPU1 任一 profile 时，先核对 replacement UUID 的最新 Xid 119/154 与 reset-required 状态；只定位该物理卡对应 slot，GPU0 也必须保持当次 live/ledger 所声明的状态（当前为 intentionally-empty）。解除隔离前必须完成维护恢复、workload-specific burn-in 或明确厂商/驱动处理结论，并重新完成 disabled heartbeat、目标 profile canary 和真实任务观察。
...
- gpu-002 GPU0 当前由 fleet 配置 `gpu-002-gpu0-scail2` 纳入 fleet/operator 管理，`scripts/lan_scail2_aio_prod.sh` 只保留 SCAIL-2 低层启动/重建/回滚入口；GPU1 当前由 `gpu-002-gpu1-i2i_pro` 接单，`gpu-002-gpu1-image_to_video` 是同卡回切候选。旧 FP8 PornMaster slot 已从 catalog 删除。
