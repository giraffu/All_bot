...
2026-07-23 已准备 gpu-226 BF16 有界流水线代码候选，尚需 main 同 SHA GPU artifact、单槽 `release-rollout` 与 20 单正式灰度后才能视为运行事实。候选同时覆盖 `pornmaster_flux2_edit_bf16` 与 `pornmaster_flux2_multi_edit_bf16`：兼容 worker 识别 `PIPELINE_PROFILE_POLICY=bf16_lan_claim3_comfy2_delivery1` 后使用最多 3 个 claimed task、最多 2 个 Comfy inflight（一个执行、一个 pending）、1 个交付槽；旧 worker 忽略版本化策略并继续使用 compose 的默认 `1/2/1`，确保精确旧 digest 回滚也恢复单计算槽。`gpu_done` 只释放计算阶段，不代表任务完成；结果仍须串行物化/上传并成功调用 Central `/complete`。灰度通过后保留新版本运行；出现超占单、并发交付、重复/缺失结果、僵尸任务、OOM 或容器重启时，先 drain 再按精确旧 digest 回滚。
