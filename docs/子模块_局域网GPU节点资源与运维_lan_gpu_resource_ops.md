...
SCAIL-2 动作迁移 / 视频换人 / 视频换脸有测试与正式链路，不能混用桶和 worker。云测试 `cloud_worker_test_08` 可指向 LAN AIO SCAIL-2 runtime `http://192.168.1.2:8190` 并声明四类 SCAIL-2 task type。视频换脸的驱动首帧由控制面抽取并提交标准 `face_swap_v2`，SCAIL-2 Worker 只执行带 `reference_preprocessed=true` 的 replacement 阶段，不访问任何外部 8188。测试 runtime 容器本身只跑 ComfyUI、模型同步与 workflow 资产，不注册 Central worker；正式 LAN Worker 必须写正式 Central 与 `user-data-prod`。
...
SCAIL-2 正式 slot0 接管会牺牲原 slot0 `img2img_lora` AIO 产能；slot1 当前用于 i2i_pro AIO。fleet/operator 的事实源是 fleet slot `gpu-002-gpu0-scail2`；低层维护入口仍是默认 dry-run 的 `scripts/lan_scail2_aio_prod.sh`。验收必须确认必需节点、模型枚举、正式 Central/桶、四任务 workflow override，并确认 compose/env 中不存在 `SCAIL2_FACE_SWAP_V10_*` 或固定图片换脸地址。回滚只恢复旧 slot0 img2img_lora AIO，不删除其它 workspace、缓存或 GPU 节点。
