...
- 节点存储 GC：`node-storage-gc --node-id <exact-node>` 只处理一个明确 LAN
  ...
  开关，BuildKit 缓存必须逐项给出
  `--prune-build-cache-builder <exact-builder>`；默认 dry-run 并输出精确候选。
  execute 会拒绝 current/运行容器、跨节点路径、符号链接、被非目标容器挂载的
  目录和不存在的 builder，操作前后 current identity 必须一致。禁止使用
  `docker system prune` 扩大范围。
- 验收：空槽使用成对
...
- Docker 镜像和卷只能在 `node-storage-gc` 内按目标节点处理：镜像仅回收没有任何
  ...
  和卷不会进入候选。BuildKit 只清理操作者逐项声明的本地 builder 缓存。
...
LAN OCI registry、模型 MinIO 分发缓存和模型源仓库可落到 NAS，但不得把 GPU
runtime workspace、Docker data-root 或数据库卷一起迁移。稳定部署契约位于
`ops/lan_artifact_nas/`：NAS 上的 Registry/MinIO 只绑定 2.5GbE 专线，主服务器
以 socket proxy 保留既有 `192.168.1.115:5000/9010`，模型源仓库通过仅允许
`10.250.150.1` 的 hard NFS 挂回原路径。
