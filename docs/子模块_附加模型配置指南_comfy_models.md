...
- MiniMax H3 四个公开模式的固定基础链（普通模式为 10Eros Beta2 + LightX2V 8-step，
  ...
  十六个候选 LoRA（单请求最多十三个）、API workflow、模型 bundle、镜像与 LAN canary 契约见
...
- 唯一公开目录是 `src/domain_config/minimax_h3.py:MINIMAX_H3_ADDON_MODELS`。每个选项必须
  ...
  同一文件注册多个别名。
- 领域目录可以下发 16 个候选，Bot/Web 单次只提交最多 13 个有序 `{name,strength}`，强度 `0.1..2.0`。Web 逐项可编辑；
...
- `AI视频` 使用独立 `ai_video_scenes`，底层固定 `minimax_h3`。配置层与主 Bot/Web 共用 `src/domain_config/minimax_h3.py` 的 16 项目录，保存最多 13 个有序 `{name,strength}`，强度 `0.1..2.0`、步长 `0.05`；无尾帧引用走 H3 I2V，有引用时复用同一绘图链生成尾帧后走 H3 FLF2V。`build_qqcc_config_options()` 下发稳定 ID、中文标签、默认强度和 `supports_lora=true`，Vue 不硬编码目录。
