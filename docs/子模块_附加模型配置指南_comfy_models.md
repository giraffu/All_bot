...
- **当前约定**：
  ...
  - `scripts/prepare_ltx_unified_model_bundle.py` 生成 `ltx_unified_runtime/2026-08-01-comfy-fast`：合并现有 LTX Video/T2V bundle、按路径和 SHA 去重、排除两份完整 10Eros checkpoint，并验证 extracted LoRA 的固定 revision、大小和 SHA。LAN 发布使用 `ltx_unified/2026-08-01-comfy-fast/manifest.json`，模型不进入镜像。
