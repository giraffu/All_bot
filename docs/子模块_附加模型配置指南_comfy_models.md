...
- `AI动图` 的 `image_to_video` 与 `wan22_video_v2` 都接受最多 5 个有序 `{name,strength}`。QQCC Config options 从 `src/qqcc_video_lora_catalog.py` 返回名称、标签与推荐强度；旧 `lora_name/lora_strength` 自动迁移并镜像首项。Vue 复用 AI视频的多选/逐项强度交互，切换 engine 不清空。
- 本地与远端 `workflow_task_patchers.py` 提交前清空节点 `26`/`18` 的全部旧 LoRA 槽，再按顺序把 `lora_1..5` 写成 `{name}_high_noise.safetensors` / `{name}_low_noise.safetensors` 与各自强度；空列表保持所有槽位为空。v2 split manifest 从 `2026-07-18-lora5` 起包含现有视频目录的全部双文件，并在 manifest 发布前逐对象 HEAD，下载未完成时 fail closed。
