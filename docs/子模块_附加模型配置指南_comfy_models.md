...
- `wan22_video_v2` 自 2026-07-21 使用独立的 `2026-07-21-pruned-v11` manifest：High/Low 都是 DaSiWa SnatchKiss v11 的 FP8 pruned 配对权重（每个 `14,528,782,272` bytes，SHA-256 以 manifest 为准）。旧 `2026-07-18-lora5` full-FP8 manifest 保留为只读回滚；同名权重的不同量化/裁剪变体不得覆盖旧对象，必须先用 `ops/gpu_pool_controller/model_transfer_batches/wan22_video_v2_pruned_v11.json` 转存到新版本 prefix，再由 `runpod wan22-v2-pruned-manifest` HEAD 校验并发布。`image_to_video` 和兼容 `wan22_aio_video` 保持原 manifest，不随 v2 切换。
