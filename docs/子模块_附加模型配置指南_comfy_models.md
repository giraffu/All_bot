...
- `AI视频` 使用独立 `ai_video_scenes`，底层固定 `minimax_h3`。配置层与主 Bot/Web 共用 `src/domain_config/minimax_h3.py` 的 17 项目录，保存最多 13 个有序 `{name,strength}`，强度 `0.1..2.0`、步长 `0.05`；其中 `motion_booster_ref2va` 只支持 REF2V，I2V/FLF2V 保存与提交时都必须剔除。无尾帧引用走 H3 I2V，有引用时复用同一绘图链生成尾帧后走 H3 FLF2V。`build_qqcc_config_options()` 下发稳定 ID、中文标签、默认强度、`supported_modes` 和 `supports_lora=true`，Vue 不硬编码目录。
