...
- LAN workspace 的代码与模型寿命需要分离：`lan_workspace_key` 是只允许安全单段字符的版本 key，当 baked ComfyUI/custom-node revision 变化时必须换 key，以便新 workspace 从镜像完整种子化；旧 workspace 保留作回滚，不得临场覆盖 custom nodes。当前 SCAIL-2 使用 `scail2-b2587e56`。
