...
| `RUNPOD_MODEL_BUCKET` / `RUNPOD_MODEL_PREFIX` / `RUNPOD_MODEL_MANIFEST_KEY` | `allbot-model-cache` + profile-specific manifest | `image_to_video` 使用 `image_to_video/2026-06-13-test/manifest.json`；`wan22_video_v2` 下一不可变候选为 `wan22_video_v2/2026-07-18-lora5/manifest.json`，下载、对象 HEAD 与 canary 完成前不得正式切换。其它 profile 版本保持现有配置。 |
