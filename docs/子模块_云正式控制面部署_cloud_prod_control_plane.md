...
自由P图 v2 正式 LAN 接单当前主要使用 GPU002 GPU1 的 LAN AIO fleet slot `gpu-002-gpu1-pornmaster_flux2_edit`；GPU252 GPU0 的 `gpu-252-gpu0-pornmaster_flux2_edit` 已保留为同卡回切候选，当前 `8192` 由 `gpu-252-gpu0-img2img_lora` 接 `img2img/img2img_lora`。PornMaster Flux2 edit AIO 只声明 `pornmaster_flux2_single_edit,pornmaster_flux2_multi_edit`；正式入口需在 `.env.cloud.prod` 设置 `ENABLE_FREE_EDIT_V2=true`，前端 Pages 构建需设置 `VITE_ENABLE_FREE_EDIT_V2=true`。
