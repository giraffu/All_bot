...
- `LOCAL_ANALYTICS_PRESERVE_ON_SHADOW_SYNC=true` 默认开启：每日恢复 `_next` 后、切换当前 shadow 前，会把旧 `bot_db_prod_shadow` 内本地分析平台生成的 `analytics_prompt_*` 表复制到 `_next`，避免云端 dump 覆盖掉 Prompt Mart、提示词瘦身和向量相似表；无这些表时只跳过，不阻断 shadow 备份。manifest 会记录本轮保留的表名与数量。
