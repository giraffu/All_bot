...
- `ltx_t2v_ic`：恰好选择两个已就绪人物，并上传一张场景背景图，固定
...
```json
{
  ...
  "background_object_key": "web_uploads/{user_id}/scene.webp"
}
```

服务端在扣费前检查两个 ID 不同、均归属当前用户、状态为 `ready`、面板版本有效且
描述非空；背景必须属于当前用户，格式为 PNG/JPEG/WebP 且不超过 20 MB。随后才将
...
文生视频使用 `ltx_scene_script_cinematic@4`：
