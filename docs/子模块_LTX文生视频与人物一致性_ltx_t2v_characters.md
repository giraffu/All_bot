...
Web 人物工作台不重复展示成年人和素材权利勾选，入口会按站点上游确认契约提交这两项
服务端事实。初始页只选择性别，不展示身体标签；管理后台为每个子图配置实际生效的
标签组后，工作台只在该子图提示词输入框下展示对应标签。配置事实源是
`CharacterViewPromptConfig`：可修改子图展示名、neutral/female/male 提示词模板、
标签组合及各选项提示词片段，未保存数据库配置时使用内置 revision 0。

人物草稿创建后已经进入私人人物库。每个 `CharacterReferenceView` 完成或直传后立即
回填对应槽位；四个基础视图全部 ready 时，服务端自动合成面板并把人物提升为 ready，
不再要求用户停留在工作台手动保存。批量生成由全局 Pinia 人物 store 持有运行状态，
先读取 `/characters/batch-capacity` 的实时身份并发额度，再按可用槽位并行提交；每个
子任务都注册现有悬浮任务。路由切换不会取消尚待提交的子图，store 继续轮询人物槽位
事实源并收口 `record_history=false` 任务。
...
Dashboard 的“角色生成管理”只管理私人人物子图的名称、提示词模板和标签组合；它不
改变模型、价格、对象归属或官方素材生命周期。保存配置增加 revision，并立即影响后续
新建或刷新的人物工作台；已提交任务继续使用提交时已经持久化的具体 prompt。
