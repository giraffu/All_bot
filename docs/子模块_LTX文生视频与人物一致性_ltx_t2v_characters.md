...
人物参考图已升级为独立的私人人物身份素材库，能力由
`CHARACTER_ASSETS_ENABLED/enable_character_assets` 控制，不再跟随 LTX 开关。
Web 普通导航另由 `enable_character_assets_entry` 控制；入口关闭时人物 API、已有素材、
人物深链和 H3 引用能力保持不变，便于后续重新开放。
新人物有十个固定可选槽位：正脸、裸体正面全身、穿衣正面全身、胸部镜头、正面私处、
背面私处及四个自定义上传槽。任意一张即可创建和保存，描述、子图名称和子图描述均可
在创建后补充；页面不重复展示成年人和素材权利确认。

只有正脸、裸体正面全身和穿衣正面全身配置 neutral/female/male 默认提示词并允许走
自由 P 图生成。胸部、正面私处和背面私处只允许选择管理员图片模板或用户上传替换，
不提供提示词生成；同一局部槽可有多张 active 模板，Dashboard 可上传、命名、标注
适用性别、排序、启用或停用，并可为每个局部槽指定至多一张默认模板。创建新人物时，
服务端把各槽位仍处于 active 的默认模板复制到该用户的私人人物目录并直接创建 ready
子图；初始子图与默认模板同槽时以用户创建时选择的初始子图为准，已有人物不回填，
用户创建后仍可改选模板或上传替换。四个自定义槽只接受上传，用户自行命名，可用于脚部、
衣服、饰品和物件等证据。所有子图仍只写私人人物库，不写 History/Gallery。
...
Dashboard 的“角色生成管理”分为两类事实源：前三个可生成视图的名称、提示词模板和
标签组合，以及三个身体局部槽位的多图片模板库。两者都不改变模型、价格、对象归属或
官方素材生命周期；提示词配置保存增加 revision，模板状态立即影响用户下拉选项。
同一身体局部槽只有一个默认模板；设置新默认会自动取消旧默认，停用默认模板也会取消
其默认状态。默认设置只在创建新人物时生效。
