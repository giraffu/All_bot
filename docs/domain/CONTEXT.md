...
- **人物身份素材**：当前用户私有的单人物视图集合。四个基础视图是 LTX 面板必需项，
  可选器官特写是 H3 单图引用，不进入 LTX 面板或 History/Gallery。
- **角色子图生成配置**：Dashboard 按子图维护的展示名、男女/中性提示词模板、实际
  生效标签组与标签片段；数据库 revision 覆盖内置 revision 0，只影响后续提交。
