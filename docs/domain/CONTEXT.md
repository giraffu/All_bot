...
- **可售卖价格变体**：当前 Web/主 Bot 用户入口在一组规范化条件（如输入数量、模式、
  分辨率、时长和参考素材）下的一条定价单元。它可以匹配一个或多个执行 task type，
  但不等同于任务 registry 条目；退役功能、懒人 Bot 场景和内部执行步骤不是价格变体。
- **公司法定会计账**：以公司主体、银行、收入、成本、税款和财务报表为对象的账簿；
  不等同于产品订单、灵石或 affiliate 账本，二者通过业务单和支付结算流水勾稽。
- **公司运营保险库**：本机 XDG 受限目录中保存主体详情、真实凭据、税务状态、流水和
  证据原件的私密层；Git 只保存其字段契约和操作规则。
...
- **任务黄金路径**：真实用户验收的共享主干，至少贯通身份、提交、终态、结果、History 和账本。
- **系统用户验收**：从真实入口串行提交任务的低资源功能验收；可按 `FULL-LITE`、`CORE-SPINE` 或 `LOCAL-SLICE` 选择范围，不等同于自动化测试或压力测试。
...
- **REF2V**：MiniMax H3 的参考图生视频模式。有序 typed refs 是 Web 请求的顺序事实源，
  服务端解析出的图片数组保持同序，`<Picture N>` 从 1 开始对应数组位置；可选参考
  音频和参考视频属于独立 typed input，不占用图片编号，也不等同于首尾帧约束。
- **人物身份素材**：当前用户私有的可选子图集合，最多六个内置槽和四个自定义上传槽，
  至少一张即可形成人物库完整合成素材；不进入 History/Gallery，H3 逐张引用 ready 子图。
- **人物素材合成图**：`character-asset-mosaic-v1`。裸体/穿衣正面全身窄列置左，其余
  ready 子图按两行列流式排列，缺图不占位；仅作人物库完整资产与预览，不进入 H3。
- **角色子图生成配置**：Dashboard 只为正脸、裸体正面全身、穿衣正面全身维护提示词
  模板与标签；身体局部使用独立的多图片模板库，可排序和停用。
- **QQCC REF2V 主体图**：用户在官方 QQCC 场景中上传的单张主体图，固定为
  `<Picture 1>`；管理员参考图从 `<Picture 2>` 起按场景配置顺序追加。
## 媒体与社区
...
- **shadow 数据库**：从云正式数据同步到本地的只读分析/灾备数据库副本，不是正式写入主库；同步业务原始表时只保留既有本地分析派生表，不会自动重算派生指标。
- **用户画像每日快照**：基于当前 shadow 业务数据按自然日 upsert 的本地派生指标；每日刷新独立于 shadow 数据同步，缺日时展示层可能沿用最近快照，因此新鲜度必须单独核验 `snapshot_date/captured_at`。
