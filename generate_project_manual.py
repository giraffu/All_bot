...
def get_tree_structure():
    try:
        ...
        def build_tree_md(node, prefix="", is_last=True, name="root", depth=0):
            ...
            if isinstance(node, dict) and "lines" not in node and "type" not in node:
                ...
                for i, k in enumerate(keys):
                    tree_str += build_tree_md(node[k], prefix, i == len(keys)-1, k, depth + 1)
            return tree_str
            
        return build_tree_md(data)
    except Exception:
        return "目录结构暂不可用"

tree_str = get_tree_structure()

manual_content = f"""# 修仙主题 Telegram 图像与视频机器人 - 项目说明书
...
with open('PROJECT_MANUAL.md', 'w', encoding='utf-8') as f:
    f.write(manual_content)

print("PROJECT_MANUAL.md created successfully.")
