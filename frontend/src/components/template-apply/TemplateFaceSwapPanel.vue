...
const { currentTask, setSubmittedTaskId, downloadResult } = useTaskResult()
...
watch(
  [faceAsset, targetAsset],
  () => {
    const isDirty = faceAsset.value.key !== initialFaceKey.value || targetAsset.value.key !== initialTargetKey.value
...
const initializeFromContext = () => {
  isTemplateApplied.value = props.context.rawTaskType === 'face_swap'
...
const beforeUploadTarget = (rawFile: File | { originFileObj?: File }) => {
  if (isTargetLocked.value) {
    message.warning(t('template_apply.face_swap.target_locked'))
    return false
  }

  return handleUpload(rawFile, 'target_image', targetAsset)
}
  ...
  <div class="template-panel flex flex-col lg:flex-row gap-6 min-h-[70vh]">
    <section class="w-full lg:w-[52%] flex flex-col bg-slate-900/70 rounded-2xl border border-slate-700/70 overflow-hidden">
      <div class="p-6 overflow-y-auto flex-1">
        <h2 class="text-2xl font-bold text-slate-100 mb-2">{{ t('template_apply.face_swap.title') }}</h2>
        <p class="text-slate-400 text-sm mb-6">{{ t('template_apply.face_swap.current_page_desc') }}</p>
        ...
        >
          <div class="rounded-xl border border-slate-700 bg-slate-800/70 p-4">
            ...
            >
              ...
              <p class="text-slate-400 text-xs">{{ t('template_apply.face_swap.face_hint') }}</p>
            </a-upload-dragger>
          </div>
          ...
          <div v-if="showTargetSection" class="rounded-xl border border-slate-700 bg-slate-800/70 p-4">
            ...
            >
              ...
              <p class="text-slate-200">{{ t('template_apply.common.upload_target_image') }}</p>
              <p class="text-slate-400 text-xs">{{ t('template_apply.face_swap.template_target_hint') }}</p>
            </a-upload-dragger>
          </div>
        </div>
      ...
      <div class="border-t border-slate-700 px-6 py-4 flex items-center justify-between gap-4">
        <div class="text-sm text-slate-300">
          {{ t('template_apply.common.estimated_cost') }} <span class="text-cyan-300 font-semibold">1</span> {{ t('template_apply.common.credits_unit') }}
        </div>
        <a-button
          type="primary"
          size="large"
    ...
    <section class="w-full lg:w-[48%] flex flex-col bg-slate-900/70 rounded-2xl border border-slate-700/70 overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <div v-if="currentTask" class="space-y-4">
          ...
          >
            <img :src="currentTask.resultUrl" class="w-full rounded-xl object-contain" />
