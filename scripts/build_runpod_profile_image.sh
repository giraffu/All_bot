...
test -d "${comfyui_dir}/custom_nodes/ComfyUI-MiniMax-ContextIR"
test -d "${comfyui_dir}/custom_nodes/ComfyUI-ReservedVRAM"
...
COMFYUI_DIR="${comfyui_dir}" LTXVIDEO_NODE_DIR="${comfyui_dir}/custom_nodes/ComfyUI-LTXVideo" PYTHONPATH="${comfyui_dir}:${PYTHONPATH:-}" python3 -c '"'"'import importlib.util, os, sys; from pathlib import Path; import comfy.cli_args; comfy.cli_args.args.cpu=True; root=Path(os.environ["COMFYUI_DIR"]); node_dir=Path(os.environ["LTXVIDEO_NODE_DIR"]); spec=importlib.util.spec_from_file_location("allbot_ltxvideo_smoke", node_dir/"__init__.py", submodule_search_locations=[str(node_dir)]); module=importlib.util.module_from_spec(spec); assert spec.loader is not None; sys.modules[spec.name]=module; spec.loader.exec_module(module); expected={"LTXVSpatioTemporalTiledVAEDecode","LTXICLoRALoaderModelOnly","LTXAddVideoICLoRAGuide"}; assert expected <= set(module.NODE_CLASS_MAPPINGS); core=(root/"comfy_extras"/"nodes_lt.py").read_text(encoding="utf-8"); assert all(name in core for name in ("GetICLoRAParameters","LTXVAddGuide","LTXVImgToVideoInplace")); scail=(root/"comfy_extras"/"nodes_scail.py").read_text(encoding="utf-8"); assert "WanSCAILToVideo" in scail and "SCAIL2ColoredMask" in scail'"'"'
...
OBJECT_INFO="$object_info" python3 -c '"'"'import json, os; payload=json.load(open(os.environ["OBJECT_INFO"])); expected={"MiniMaxH3UnifiedToVideo","MiniMaxH3AudioLock","MiniMaxH3ReferenceToVideo","ModelAttentionBackend","ReservedVRAMSetter","MiniMaxH3SigmaShift","DaSiWa_ResolutionScaleCalculator","VAEDecodeAudio","VHS_VideoCombine"}; missing=expected-set(payload); assert not missing, sorted(missing); attention=payload["ModelAttentionBackend"]["input"]["required"]["attention"][0]; assert "comfy kitchen attention" in attention, attention'"'"'
