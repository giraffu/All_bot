...
try:
    from scripts.release_artifacts_v2 import load_catalog, plan_builds as plan_artifact_builds
...
def render_track_release_env(manifest: Mapping[str, Any], config_revision: str) -> str:
    """Render only the image variables owned by one schema-v2 track."""
    ...
    if track == "control-plane":
        for name, variable in CONTROL_ARTIFACT_ENV.items():
            ...
            if isinstance(artifact, Mapping) and artifact.get("kind") in {"image", "external-image"}:
...
def _resolve_previous_sha(args: argparse.Namespace, *, track_scoped: bool = False) -> str | None:
    if args.from_sha:
...
def build_plan(args: argparse.Namespace) -> tuple[ReleaseImpact, dict[str, Any], str]:
    ...
    if manifest_document.get("schema_version") == 2:
        ...
        service_to_artifact.update(
            {name: name for name in CONTROL_ARTIFACT_ENV if name not in {"imgproxy", "postgres", "redis"}}
        )
        ...
        if previous_sha and previous_sha != sha:
            ...
            planned_impact = plan_changed_paths(
                load_structured_file(Path(args.policy)),
                changed_paths,
            )
        ...
        manifest["_impact"] = planned_impact
        manifest["_previous_sha"] = previous_sha or ""
    else:
    ...
    if manifest.get("schema_version") == 2:
        ...
        if args.track == "control-plane":
            ...
            services = {"worker"} if artifact_names & {"worker-agent", "worker-relay"} else set()
        else:
            services = artifact_names
        planned = manifest.pop("_impact")
        planned.services = services
        if f"track:{args.track}" not in planned.matched_rules:
            planned.matched_rules.append(f"track:{args.track}")
        return planned, manifest, str(manifest.pop("_previous_sha"))
...
def _deploy_cloud(
    ...
    release_dir = (
        f"/var/lib/allbot/releases/{manifest['track']}/{sha}"
        if manifest.get("schema_version") == 2
        else f"/var/lib/allbot/releases/{sha}"
    )
    ...
    if manifest.get("schema_version") == 2:
        revision_refs = [
            (str(artifact["ref"]), str(artifact["oci_revision"]))
            for name, artifact in manifest["artifacts"].items()
            if name in manifest.get("selected_artifacts", [])
            and artifact.get("kind") == "image"
            and name not in {"imgproxy", "postgres", "redis"}
        ]
    else:
        revision_refs = [
            (str(manifest["images"][name]), sha)
            for name in ("app", "central", "dashboard_backend", "dashboard_frontend")
        ]
    revision_checks = "\n".join(
        (
        ...
        for ref, revision in revision_refs
    )
...
def _deploy_web(
    ...
    if args.skip_web:
        return
    if manifest.get("schema_version") == 2:
        web_metadata = manifest["artifacts"]["public-web"]
        expected_hash = str(web_metadata["sha256"])
        default_name = str(web_metadata["ref"])
    else:
        expected_hash = str(manifest["web_artifact_sha256"])
        default_name = "web-dist.tgz"
    ...
    if not artifact.is_file() and args.web_artifact == "web-dist.tgz":
        ...
        for candidate in (
            cache / default_name,
            cache / "release" / default_name,
            cache / "release-v2" / default_name,
        ):
            if candidate.is_file():
                artifact = candidate
                break
    _verify_web_artifact(artifact, expected_hash)
...
def _promotion_check(args: argparse.Namespace, manifest: Mapping[str, Any]) -> None:
    ...
    if manifest.get("schema_version") == 2:
        state_root = f"/var/lib/allbot/deployments/test/{manifest['track']}"
    else:
        state_root = "/var/lib/allbot/deployments/test"
    if args.command == "rollback":
        state_path = f"{state_root}/history/{manifest['git_sha']}.json"
    else:
...
def _test_rollback_check(args: argparse.Namespace, manifest: Mapping[str, Any]) -> None:
    ...
    if manifest.get("schema_version") == 2:
        if state.get("status") != "verified" or state.get("artifacts") != {
            name: {
            ...
            raise ReleaseError("rollback target is not the previously verified track digest set")
        return
    if (
...
def validate_test_acceptance(
    ...
    if manifest.get("schema_version") == 2:
        ...
        expected = {
            ...
            for name, artifact in manifest["artifacts"].items()
            if name in manifest.get("selected_artifacts", manifest["artifacts"])
        }
        if evidence.get("artifacts") != expected:
            raise ReleaseError("test acceptance artifact digests do not match the release")
    else:
...
def _mark_test_verified(args: argparse.Namespace) -> None:
    sha = validate_full_sha(args.sha)
    manifest_document = _read_json(Path(args.manifest))
    if manifest_document.get("schema_version") == 2:
        manifest = _load_v2_track(
            Path(args.manifest),
            sha=sha,
    ...
    host = args.remote_host or ENVIRONMENT["test"]["host"]
    state_path = (
        f"/var/lib/allbot/deployments/test/{manifest['track']}/current.json"
        if manifest.get("schema_version") == 2
        else "/var/lib/allbot/deployments/test/current.json"
    )
    result = _run(
        ["ssh", "-o", "BatchMode=yes", host, f"cat {state_path}"],
        check=False,
    ...
    if manifest.get("schema_version") == 2:
        expected = {
            name: artifact.get("digest") or artifact.get("sha256")
            for name, artifact in manifest["artifacts"].items()
            if name in manifest["selected_artifacts"]
        }
        if state.get("git_sha") != sha or any(
            state.get("artifacts", {}).get(name, {}).get("digest") != digest
            for name, digest in expected.items()
        ):
            raise ReleaseError("cloud-test runtime state does not match acceptance evidence")
        for name in expected:
            state["artifacts"][name]["status"] = "verified"
    elif (
    ...
    state_root = str(Path(state_path).parent)
...
def _write_state(
    ...
    payload = json.dumps(state, sort_keys=True, indent=2) + "\n"
    track_segment = f"/{manifest['track']}" if schema_version == 2 else ""
...
def _add_release_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="comma-separated module/profile selection; --services remains a control-plane alias",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.execute and args.from_sha:
            raise ReleaseError(
                "execute mode resolves the deployed track state automatically; --from-sha is plan-only"
            )
        if args.execute:
        ...
        if manifest.get("schema_version") == 2 and args.track == "gpu-execution":
            raise ReleaseError(
                "GPU track mutations must use the profile canary operator; release.py records plans and verification only"
            )
