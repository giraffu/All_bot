...
async def merge_users():
    source_id = 10000000003824 # Robin Green
    target_id = 10000000052318 # Kathleen Tus
        ...
        try:
            ...
            await session.execute(text(f"""
                ...
                SET 
                    ...
                    user_group = (SELECT user_group FROM users WHERE id = {source_id}),
                    current_identity = (SELECT current_identity FROM users WHERE id = {source_id}),
                    identity_expire_at = (SELECT identity_expire_at FROM users WHERE id = {source_id}),
                    is_channel_member = (SELECT is_channel_member FROM users WHERE id = {source_id}),
                    
                    invited_by = COALESCE(invited_by, (SELECT invited_by FROM users WHERE id = {source_id}))
                WHERE id = {target_id};
            """))
            ...
            # For invitee_id, we only update if target doesn't already have an entry
            target_invitee_check = await session.execute(text(f"SELECT id FROM referrals WHERE invitee_id = {target_id}"))
            if not target_invitee_check.fetchone():
                await session.execute(text(f"UPDATE referrals SET invitee_id = {target_id} WHERE invitee_id = {source_id};"))
            else:
                logger.info("Target user already has an inviter record, discarding source user's invitee record...")
                await session.execute(text(f"DELETE FROM referrals WHERE invitee_id = {source_id};"))
            ...
            await session.execute(text(f"""
                ...
                SET 
                    ...
                    google_id = NULL,
                    email = NULL,
                    username = COALESCE(username, 'user_' || id) || '_merged_to_{target_id}',
                    credits = 0,
                    checkin_count = 0,
                    referral_count = 0,
                    generation_count = 0
